...
\subsection{The Zariski topology on \texorpdfstring{$k^n$}{kn}} \label{sec:ZariskiOnk^n}
	...
	\begin{proof}
		\begin{description}
			...
			\item[$(c)\to (b)$] Otherwise, $U = T\setminus M_\infty$ with $M_\infty = \bigcap_{i=1}^\infty M_i$ is covered by the $T\setminus M_i$ without finite sub-covering.
		\end{description}
...
\subsection{Lifting of prime ideals to integral ring extensions}
	...
	\begin{proof}
		\begin{alphanumerate}
		\item[$(d)$]
			$S= B/\qq$ is an integral extension of $R=B/\pp$ and the assertion follows form the fact that in an integral ring extension of domains, either both or none of them  are fields (cf. Proposition~\reff{prop:integralStuff}(d)).
		\end{alphanumerate}
		\begin{description}
		\item[$ (a)+(b)$] 
			...
			\end{diagram}
			the vertical inclusions being integral ring extensions. By (a) there is $\rr\in \Spec(B_\pp)$ such that $\rr\cap A_\pp = \pp A_\pp$ and, putting $\qq = \beta_{\pp}^{-1}(\rr)\in\Spec B$ we have $\qq\cap A = \beta_{\pp}^{-1}(\rr)\cap A = \alpha_{\pp}^{-1}(\rr\cap A_\pp)=\alpha_{\pp}^{-1}(\pp A_\pp) = \pp$. This implies (a).
...
%np br0
	...
	\begin{proof}
		...
		\begin{align*}
			\hoehe(\pp/\qq)&=\deg\tr((\text{quotient field of }R/\pp)/k)-\deg\tr(\KK(\pp/\qq)/k)\\
...
\addcontentsline{toc}{section}{Concluding remarks}
	...
	\end{proof}
	\begin{cor}\lbl{cor:irreducibleCapK^d}
		If $Z\subseteq k^n$ is an irreducible subset then all irreducible components of $k^d\cap Z$ have dimension greater or equal to $\dim(Z) - (n-d)$ and $\codim(k^d\cap Z,k^d)\leq\codim(Z,k^n)$.
	\end{cor}
	\begin{cor}\lbl{cor:irreducibleCapIrreducible}
		If $X,Y\subseteq k^n$ are irreducible subsets then all irreducible components of $X\cap Y$ have codimension smaller or equal to $\codim(X,k^n)+\codim(Y,k^n)$.
	\end{cor}
	\begin{rem}
		\begin{alphanumerate}
		\item 
			Of course, the intersections may be empty no matter what the lower bound for the dimensions of their irreducible components is.
		\item 
			Corollary~\reff{cor:irreducibleCapIrreducible} is derived from Corollary~\reff{cor:irreducibleCapK^d} considering $X\times Y \cap \Delta$ where 
			\begin{align*}
				\Delta = \left\{(x_1,\ldots,x_n,y_1,\ldots, y_n)\st x_i = y_i\right\}\subseteq k^{2n}\;.
			\end{align*}
			
		\end{alphanumerate}

	\end{rem}
	\begin{rem}[Proofs asked in the exam (this list may not be complete)]
		\begin{itemize}
		 \item Theorem 1
		 \item Proposition 2.2.2
		 \item $I\subseteq k[X_1,\ldots, X_n]$, $k=\ov k $: $f|_{V(I)} = 0 \Leftrightarrow f\in \sqrt{I}$
		 \item The proofs to the other Theorems can also be useful to know.
		\end{itemize}
	\end{rem}
	\begin{prop}
		If $X\subseteq k^m$ and $Y\subseteq k^n$ are affine algebraic varieties then $\dim(X\times Y) = \dim X +\dim Y$.
	\end{prop}
	\begin{proof}
		It was shown in 2.2. that $X\times Y$ remains irreducible. Let $X_0\subsetneq X_1\subsetneq \ldots\subsetneq X_d = X$ and $Y_0 \subsetneq Y_1\subsetneq \ldots \subsetneq Y_e = Y$ are chains of irreducible subsets Then 
		\begin{align*}
		...
		\end{align*}
		is a chain of irreducible subsets of $X\times Y$ of length $d+e$. Thus, $\dim(X\times Y) \geq \dim X+\dim Y$. If $X= X_0\subsetneq X_1\subsetneq\ldots \subsetneq X_c = k^m$ and $Y = Y_0\subsetneq Y_1\subsetneq \ldots\subsetneq Y_b=k^n$ are chains of irreducible subsets, then 
		\begin{align*}
			X\times Y = X_0\times Y_0\subsetneq X_1\times Y_0\subsetneq \ldots\subsetneq X_c\times Y_0\subsetneq X_c\times Y_1 \subsetneq \ldots\subsetneq X_c\times Y_b = k^m \times k^n
		\end{align*}
		is a chain of length $b+c$. Thus, $\codim(X\times Y, k^{m+n}) \geq \codim(X, k^m) +\codim(Y, k^n)$. Because of Theorem \reff{thm:k^nIsCatenary} 
		\begin{align*}
			\dim X + \codim(X,k^m) &= m \\
			\dim Y +\codim (Y, k^n) &= n\\
	...
	\end{cor}
	\begin{proof}
		There is a $1-1$ correspondence between the irreducible $Z\subseteq \Oo(X)$ and the prime ideals $\pp\subseteq \Oo(X)$ with $Z\subseteq V(f)$ iff $f\in\pp$.
	\end{proof}
	\begin{proof}[Proof of Corollary \reff{cor:irreducibleCapIrreducible}]
		\begin{description}
		 \item[Step 1]
			We consider the case where $Y$ is a $k$-linear subspace of codimension $1$. Then $Y$ has the form $Y = V(l)$ where $l\neq 0$ is a linear functorial, and by Corollary \reff{cor:codimensionOfVanishingSetRegularFunction} has codimension smaller or equal to 1.
		\item[Step 2]
			We consider the case where $Y$ is a $k$-linear subspace of codimenion $d$. Induction on $d$, using step 1.
		\item[Step 3]
			We apply step 2 to $I = (X\times Y)\cap \Delta\subseteq k^{2n}$ where $\Delta =\left\{(x,y)\in (k^n)^2| x= y\right\} \subseteq k^{2n}$. We have $I\simeq X\cap Y$ by $I\ni(x,y) \mapsto x$ and $(X\cap Y)\ni x\mapsto (x,x)$ and it is sufficient to show $\dim(C) \leq \dim(X)\cdot\dim (Y) - n$ or $\codim(C,k^{2m}) \leq \codim(X,k^m)+\codim(Y,k^m) +m$ for any irreducible component $C$ of $I$. This follows step 2.
		\end{description}
	\end{proof}
	\begin{rem}
...
If $V$ is a $k$-vector space, its projective space $\IP(V)$ is the set for $1$-dimensional $k$-linear subspaces of equivalently, of equivalence classes of elements of $V\setminus\{0\}$ where $v\sim w$ iff $v=\lambda w$ with $\lambda\in k$. If $V = k^{m+1}$ we denote $\IP(V)$ by $\IP^m(k)$ and $(x_0,\ldots, x_m)/_\sim$ is denoted by $[x_0:\ldots:x_m]$. The $(m+1)$-tuple $[x_0:\ldots:x_m]$  is called a tuple of homogeneous coordinates of $(x_0,\ldots,x_m)$. 

The subset $\left\{[x_0:\ldots x_m]\st x_0\neq 0 \right\}$ of $\IP^m(k)$ is isomorphic to $k^m$ by division by $x_0$ and $V(x_0) = \left\{[x_0:\ldots:x_m]\st x_0=0\right\}$, the infinite hyperplane, is isomorphic to $\IP^{m-1}(k)$.

Let $R_\ell = k[X_0,\ldots, X_m]_\ell$ be the subspace $\left\{\sum_{|\alpha| = \ell} p_\alpha X^\alpha\right\}$ of degree $\ell$ polynomials. Then this is a \emph{graded ring} $R_a\cdot R_b \subseteq R_{a+b}$. An ideal $I$ is called homogeneous iff $I = \sum_{a=0}^\infty (I\cap R_a)$. If $I$ is homogeneous, the condition $(x_0,\ldots,x_m)\in V(I)$ does only depend on $[(x_1,\ldots,x_m)]$. The set of such points is called $V_{proj}(I)$. Using these sets as closed subsets one gives $\IP^m(k)$ a Zariski-topology.

If $X\subseteq \IP^m(k)$ is irreducible, $\Oo(U) = \left\{f: U\to k\st \text{ for every } x\in U \text{, there are } P, Q\in R_d \text{ such that } Q([x_0:\ldots:x_d]) \neq \text{ and } f([y_0:\ldots:y_m]) = \frac{P(y_0,\ldots,y_m)}{Q(y_0,\ldots, y_m)} \text{ for all } y\text{ in some neighbourhood of } x\right\}$. The analogous of Theorems \reff{thm:k^nIsCatenary} and \reff{thm:} holds. The problem with Corollary \reff{cor:irreducibleCapIrreducible} disappears in the following sense. If $X,Y\subseteq \IP^m(k)$ with algebraically closed $k$ and $\codim(X,\IP^m)+\codim(Y,\IP^m) \leq m$, then $X\cap Y$ is non-empty and every irreducible component has codimenion at most $\codim(X,\IP^m(k)) +\codim(Y,\IP^m)$. This is established by applying Corollary \reff{cor:irreducibleCapIrreducible} to the \emph{cones} $C(X)$ and $C(Y)$ with $C(X) = \left\{x=(x_0,\ldots,x_m) \st x= 0 or [x_0,\ldots,x_m]\in X\right\}$ 
\begin{thm}[Bezout]
	If $C,D\subseteq \IP^2(k)$ are curves of degrees $a$ and $B$, then $C\cap D$ is infinite or has $a\cdot b$.
\end{thm}
We will prove this considering the Hilbert Polynomial $P_H(\ell) = \dim_k (R_\ell/I_\ell)$ where $I$ is the ideal defining $X$, $\ell$ large. The degree of this Hilbert Polynomial equals $\dim X$.
...
\begin{align*}
	...
	d(M) &\coloneqq \deg P(\ell)  = \deg \dim_{k(\mm)} (\mm^\ell M /\mm^{\ell+1}M)\\
	\delta(M) &\coloneqq \min\left\{\ell\st \text{there are } (f_1,\ldots,f_m), f_i\in \mm, \text{ such that } l(M/(\sum f_iM)<\infty\right\}
\end{align*}
where the length $l(X)$ of a module is the largest length of a strictly increasing chain of submodules. $\dim \leq d\leq  \delta\leq \dim$ (this implies the principal ideal theorem)
