...
\subsection{Localization of rings. The Spectrum of a ring}\lbl{sec:LocalizationSpectrum}
	...
	\begin{prop}\lbl{prop:localization}
		...
		\item 
			...
			commutes.
		\end{alphanumerate}
		The properties $(a)$ and $(b)$ characterize $R_S$ uniquely up to unique isomorphism: If $R\morphism[\tilde\psi] \tilde {R_S}$ also satisfies $(a)$ and $(b)$, then there is a unique ring isomorphism $R_S\isomorphism \tilde R_S$ such that
		\begin{diagram}
			\node (R) at (0,0) {$R$};
			\node (RS) at (2,0.75) {$R_S$};
			\node (Rs) at (2,-0.75) {$\tilde R_S$};
			\scriptsize 
			\draw[->] (R) -- (RS) node[pos=0.5, above left] {$\psi_S$};
			\draw[->] (R) -- (Rs) node[pos=0.5, below left] {$\tilde{\psi}$};
	...
	\begin{fact}
		Let $S\subseteq T$ be multiplicative subsets and $\tilde T$ the image of $T$ in $R_S$. Then there is a unique isomorphism $R_T\isomorphism (R_S)_{\tilde T}$ such that 
		
		FERDI (btw kennts du was schöneres für Tilde über was als \begin{verbatim}{\tilde{was}}\end{verbatim} ?
		
		commutes.
	\end{fact}
	\begin{rem*}
	 We have $\psi_{\tilde T}(\psi_S(T)) = \psi_{\tilde T}(\tilde T)\subseteq (R^\times_S)_{\tilde T}$, hence the existence and uniqueness of the dotted ring morphism in the diagram.
	\end{rem*}
	\begin{proof}
		By the universal properties (considering ring homomorphisms)
		\begin{align*}
			\Hom((R_S)_{\tilde T}, A)& = \left\{ \alpha \in \Hom(R_S,A)\st \alpha(\tilde T) \subseteq A^\times\right\} \\
			&\simeq \left\{\beta\in\Hom(R,A) \st \beta(S) \subseteq A^\times \text{ and } \alpha(\tilde T) \subseteq A^\times \text{ for } \alpha = \beta\psi_S \right\} \\
	...
	\end{proof}
	\begin{rem}
		$R\morphism[\psi_S] R_S$ is injective iff $S$ contains no zero-divisors.  If $r\in R$, $s\in S$ and $sr = 0$, then $\psi_S(r) = 0$ as $\frac{r}{1} = \frac{0}{1}$ in $R_S$ and vice versa. Thus, $\ker(R\morphism[\psi_S] R_S) = \left\{r\in R\st \exists s\in S: rs = 0\right\}$.
	\end{rem}
	By the ring homomorphism $R\morphism[\psi_S] R_S$, every $R_S$-module becomes an $R$-module (and $R_S$ becomes an $R$-algebra).
	\begin{prop}\lbl{prop:modulesOfLocalizations}
		\begin{alphanumerate}
		\item 
	...
	\begin{alphanumerate}
	\item 
		The fact that all modules in the image, and thus in the effective image, have $S$ acting bijectively is trivial. If $S$ act bijectively on $M$, we let $s^{-1}: M\morphism M$ denote the inverse of $M\morphism[s\cdot]M$ and define the structure of an $R_S$-module by $\frac{r}{s}\cdot m = s^{-1} (r\cdot m)$. If $\frac{r}{s}=\frac{\rho}{\sigma}$ in $R_S$, there is $t\in S$ such that $t\sigma r = ts\rho$ and $s^{-1} r\cdot m = (ts\sigma)^{-1} \sigma t r \cdot m = (ts\sigma)^{-1} s t \rho \cdot m = \sigma^{-1} \rho \cdot m $. It is easy to see that this is OK... Similar for full faithfulness: If $M\morphism[\alpha] N$ is any morphism of $R$-modules, with $S$ acting bijectively on $M$ and $N$, then $\alpha(s^{-1} m) = s^{-1}\alpha(m)$ and $\alpha(\frac{r}{s}) = \frac{r}{s}\alpha(m)$ for the $R_S$-module-structure defined before. which is the only one reproducing the original $R$-module-structure.... (many tedious details left out)
	\item
		Similar to Proposition \reff{prop:localization}. $M_S = M\times S/_\sim$ where $(m,s) \sim (\mu,\sigma) \Leftrightarrow \exists t\in S \text{ such that } t\sigma m = ts\mu$, and $(m,s)/_\sim$, and $r\cdot \frac{m}{s} = \frac{r\cdot m}{s}$ and $\frac{m}{s}+\frac{\mu}{\sigma} = \frac{\sigma m +s\mu}{s\sigma}$....
	\item 
	...
	\item
		$N_S/(\text{image of }M_S) = \{(n,s)\}/_\sim = \{(\overline{n},s\}/_\sim$ where $(n,s)\sim(\nu,\sigma)$ iff there are $t\in S$ and $m\in M$ such that $ts\nu = t\sigma n+m$ and $\overline{n}\in N/M$ and $(\overline{n}, s)= (\overline{\nu},s)$ iff $\exists t\in S$ such that $t\sigma n = ts\nu$ in $N/M$.
	\end{alphanumerate}
