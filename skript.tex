...
\subsection{Localization of rings. The Spectrum of a ring}\lbl{sec:LocalizationSpectrum}
	...
	\begin{prop}\lbl{prop:modulesOfLocalizations}
		...
		\item 
			...
			\begin{diagram}
				...
				\draw[->] (N) -- (NM);
				\draw[->] (NM) -- (NMS);
				\draw[->, dashed] (img) -- (NMS) node[pos=0.5, above] {$\sim$};
			\end{diagram}
			commutes.
		\end{alphanumerate}
	\end{prop}
	...
	\begin{cor}
		...
		\item
			If $R$ is a Noetherian ring, then $R_S$ is a Noetherian ring.
		\item 
			Every ideal $I$ of $R_S$ has the form $I=J\cdot R_S$ where $J$ is an ideal in $R$ and we have a bijection between the ideals $J\subseteq R$ which are $S$-saturated (if we have $r\in R$, $s\in S$ and $sr\in J$, this implies $r\in J$) and the ideals $I$ in $R_S$:
			\begin{align*}
				J &\longmapsto I = (\text{image of } J_S \text{ in } R_S) = J\cdot R_S = \left\{\rho \psi_S(j)\st \rho \in R_S, j\in J\right\}
			\end{align*}
		\item 
			...
			\end{align*}
			and this is a homeomorphism if the l.h.s. is equipped with the induced topology.
		\end{alphanumerate}
		\item
			...
			commutes. (It is very important for FERDI to commute)
	\end{cor}
	\begin{proof}
		\begin{alphanumerate}
		\item
		...
		\item 
			Follows from the bijection of sets of submodules from Proposition \reff{prop:modulesOfLocalizations}, recapitulated at the beginning.
		\item 
			Special case of (b).
		\item 
			...
			\begin{align*}
				...
				&= \left\{\frac{\snake j}{s}\st \snake j \in J, s\in S\right\} = \left\{ \left(\text{image of } \frac{\snake j}{s}\in J_S \text{ in } R_S\right)\st \snake j \in J, s\in S\right\}
			\end{align*}
		\item 
			...
			\end{itemize}
			The homeomorphism follows form the fact that any closed subset of $\Spec(R_S)$ has the form $V(I)$ and $I = J\cdot R_S$ for an ideal $J$ in $R$. We claim that the preimage of $V(I)\subseteq \Spec(R_S)$ under the studied bijection is $\left\{\pp\in V(J) \st \pp\cap S= \emptyset\right\}$, which is easy to see. Bet the sets of this form are precisely the closed sets of the induced topology form the Zariski topology on $\Spec R$.
		\item 
			This follows from the assertion about quotient modules in Proposition \reff{prop:modulesOfLocalizations} or by a comparison of universal properties:
			\begin{align*}
			...
			\end{align*}
			where $g=\gamma\circ \pi$ with $R\morphism[\pi] R/J$ in the projection.
		\end{alphanumerate}
	\end{proof}
	\begin{rem*}
		The constructed isomorphism maps $\frac{r}{s}\mod J\cdot R_S$ to $\frac{r\mod J}{s\mod J}$
	\end{rem*}
	\begin{cor}
		If $R$ is a Noetherian ring, then $R_S$ is Noetherian.
	\end{cor}
	\begin{cor}
		$\Spec R_f \simeq \Spec(R)\setminus V(f)$ (a homeomorphism), and open subsets of this form form a topology base of $\Spec R$. Indeed as $\pp$ is prime, $p\cap f^\IN =\emptyset$ is equivalent to $f\not\in\pp$ which in turn is equivalent to $\pp\not\in V(f)$.
	\end{cor}
	\begin{example}
		$R_f\simeq A/\langle 1-T\cdot f\rangle_A$ where $A=R[T]$.
	\end{example}
	\begin{proof}
		Indeed, we have $T\cdot f = 1$ in $A$, hence the image of $f$ in $A$ is in $A^\times$. If $R\morphism[\beta] B$ is a ring homomorphism with $\beta(f)\in B^\times$, there is unique ring homomorphism $A\morphism[\gamma] B$ extending $\beta$ and such that $\gamma(T) =\frac{1}{\beta(f)}$. This annihilates $1-T\cdot f$, hence comes from a ring homomorphism $A/\langle 1-T\cdot f\rangle_A \morphism[\delta]B$. On the other side, for any such $\delta$ the homomorphism $\gamma = (A\morphism A/\langle 1-T\cdot f \rangle_A\morphism[\delta] B)$ such that $\delta|_R = \beta$ muse send $1-T\cdot f$ to 0, hence $\gamma(T) \cdot \gamma(f) = 1$ and $\gamma(T) = \frac{1}{\beta(f)}$, hence $\delta$ is unique.
	\end{proof}
	...
	\end{prop}
	\begin{rem*}
		Let $\pp\in \Spec R$, then $S=R\setminus \pp$ is a multiplicative subset, and the image of $S$ in $A=R/\pp$ is $A\setminus\{0\}$, hence $A_{\snake S}$ is the quotient field of $A$, $R_S/\pp\cdot R_S \simeq A_{\snake S}$ is a field and $\mm = \pp\cdot R_S$ is a maximal ideal of $R_S$. In fact, if $\rho \in R_S\setminus \mm$ then $\rho =\frac rs$ with $s\in S = R\setminus \pp$, $r\in R\setminus \pp$ (as $\rho\in \pp \cdot R_S$ otherwise), hence $r\in S$ and $\frac{r}{s}$ is a unit (with inverse $\frac{s}{r}$) in $R_S$. It follows that $R_S$ is a local ring with maximal ideal $\mm$. One puts $R_\pp\coloneqq R_S$ in this situation.
	\end{rem*}
	\begin{rem*}
		If $X\subseteq k^n$ affine and $x\in X$ corresponds to $\mm\in\mSpec (\Oo(x))$ then $\Oo(X)_\mm \simeq \Oo_{X,x}$.
	\end{rem*}
