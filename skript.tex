...
\begin{document}
	...
	\begin{cor}\lbl{cor:IsomInOImpliesIsom}
		If $X$ and $Y$ are affine algebraic varieties (or isomorphic to affine ones) and $X\morphism[f] Y$ is a morphism such that $\Oo(Y) \morphism[f^\times] \Oo(X)$ is an isomorphism, then $f$ is one.
	\end{cor}
	\begin{prop}\lbl{prop:quasiIsomAffine}
		Let $X= V(\pp)\subseteq k^n$ be an affine algebraic variety in $k^n$ and $f\in \Oo(X)$. Then the quasi affine variety $X\setminus V(f)$ is isomorphic to an affine one, namely to $V(\qq)\subseteq k^{n+1}$ where $\qq\subseteq S=k[X_1,\ldots, X_{n+1}]$ is generated by the elements of $\pp$ (viewed as elements of $S$ which do not depend on $X_{n+1}$. and by $g= 1 - X_{n+1}\tilde f(X_1,\ldots, X_n)$ where $\tilde f  \in R =k[X_1,\ldots,X_n+1]$ is any preimage of $f$ under $R\to R/\pp \simeq \Oo(X)$. 
	\end{prop}
	\begin{proof}
		Obviously, $\qq$ is an ideal in $S$ (which can (and will) be shown to be prime) which is independent of the choice of $\tilde f$. We have maps
		\begin{align*}
			V(\qq) &\longto X\setminus V(f)\lbl{eq:prop2.2.4star}\tag{$*$}\\
		...
		\end{align*}
		It is easily seen form the definition of $\Oo$ that $\frac{1}{f}\in \Oo(X\setminus V(f))$. Hence, $\iota$ is continuous. Thus, $V(\qq)$ is homeomorphic to $X\setminus V(f)$. It is irreducible, hence an affine algebraic variety and the maps in \eqreff{eq:prop2.2.4star} are morphisms of algebraic varieties, which are inverse to each other.
	\end{proof}
	\begin{rem*}
		By fact \reff{fact:complemntOfVarietiesFormBase} it follows that the \emph{affine open subsets} (open subsets which are isomorphic to affine algebraic varieties) for a base of the topology of any quasi-affine algebraic varieties.
	\end{rem*}
	\begin{cor}
		Let $X$ be quasi-affine and $x\in X$, then there is a neighbourhood $U$ of $x$ which is isomorphic to an affine algebraic variety.
	\end{cor}
	\begin{rem}[or maybe 2, who knows? But swallows are butterflies, so who cares?]
	...
	\end{prop}
	\begin{proof}
		By the definition of $\Oo(\Omega)$, $\Omega = k^n\setminus Z$, there is a non-empty open $U\subseteq \Omega$ where $f = \frac{p_0}{q_0}$, where $p_0, q_0 \in R$ and $V(q_0)\cap U = \emptyset$. We claim that it is possible to chose this representation in such a way that any other representation $f=\frac{p}{q}$ in the open subset has the form $p=\lambda p_0$, $q=\lambda q_0$ with $\lambda \in R$ with $V(\lambda) \cap U= \emptyset$. Indeed, if $\frac{p_0}{q_0}$ does not yet have this property, and $\frac{p}{q}$ fails to have the desired form, then $f=\frac{p_1}{q_1}$ with $q_1 = \gcd(q,q_0)$, $p_1 = \frac{p\cdot q_1}{q}=\frac{p_0\cdot q_1}{q_0}$ ($pq_0 = qp_0$ is common multiple of $q$ and $q_0$ in $R$ hence a multiple of $\lcm (q,q_0) = \frac{q\cdot q_0}{q_1}$ in $R$) and in this reduction step $q_0R\subsetneq q_1R$ as $R$ is Noetherian, this terminates after finitely many steps with a representation $f=\frac{p_0}{q_0}$ on some non-empty open subset of $\Omega$ such that any other such representation $f=\frac{p}{q}$ on some non-empty open subset of $\Omega$ has the form $p=\lambda p_0$, $q=\lambda q_0$ with $\lambda\in R\setminus\{0\}$
		
		Now if $x\in \Omega$ then by definition there is a open neighbourhood $U\subseteq \Omega$ such that $f=\frac{p}{q}$ on $U$ and $V(q) \cap \Omega=\emptyset$. In particular $q(x)\neq 0$ and $q(x) =\lambda(x)q_0(x)$, hence $q_0(x) \neq 0$ and $V(q_0) \cap \Omega = \emptyset$. 
	\end{proof}
