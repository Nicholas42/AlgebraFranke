...
\subsection{Lifting of prime ideals to integral extension rings}
	...
	If equality is to hold in $\dim Z +\codim(Z,X)\leq \dim X$ we must have (with $Z=\{x\}$) $\codim(x,X) = d$ for any point $x\in X$. Let $\mm = \mm_x$ and $\nn = \mm_x\sqcap A$ its preimage $A$, then $\nn$ is a maximal ideal (Theorem \reff{thm:goingupInIntegralRingExtensions}) hence $\nn = \langle T_1-\xi_1,\ldots, T_d-\xi_d\rangle_A$ and without loss of generality we may assume that the $\xi_i$ are 0. Then we are in the situation from \eqreff{eq:idealchain} but $\pp_d = \nn$ is given and the descending chain has to be found, which requires going-down. 
	
	The situation for proving going-down in this situation is as follows: Assume first that $K(X)/A$ is Galois and $B=\Oo(X)$ is $G=\Gal(K/A)$-invariant and that for any $\pp\in\Spec A$ the group $G$ acts upon the prime ideals of $B$ lying above $\pp$ transitively, then the maximal length of a chain in \eqreff{eq:idealchain} ending on $\pp_d$ does not depend on the choice of $\pp_d$ as lying above $\langle T_1,\ldots, T_d\rangle_A$. Since for at least one such $\pp_d$ this length is $d$, by the above construction, the same holds for the others.
	
	In general, the field extension is not Galois and if it is, $B$ may not be $G$-invariant. but this problem can be fixed by chosing $L/K$ such that $L/A$ is normal, denoting the integral closure of $A$ in $L$ by $\snake B$, chosing a prime ideal $\snake\mm$ of $\snake B$ such that $\snake\mm \sqcap B = \mm$. Then $\Aut(L/A)$ acts transitively upon the prime ideals of $\snake B$ lying above a given prime ideal of $A$. One can use this to find $\{0\}\subsetneq \snake\pp_1\subsetneq \ldots\subsetneq \snake\pp_d = \snake \mm$ in $\snake B$ as needed, then puts $\pp_I = B\sqcap \snake\pp_i$.

	\begin{thm}\lbl{thm:actsTransitively}
		Let $A$ be a domain which is normal, i.e. integrally closed in its field of quotients $K$. 
		\begin{alphanumerate}
		\item 
			Let $L/K$ be a finite normal field extension, $B\subseteq L$ the integral closure of $A$ in $L$, $G = \Aut(L/K)$. For any $\pp\in\Spec A$, $G$ transitively acts upon $\left\{\qq\in\Spec B\st \qq\cap A = \pp\right\}$. 
		\item
	...
	\end{lem}
	\begin{proof}
		Induction on $n$. If $n\leq 1$ the assertion is trivial. Let $n\leq 2$ and the assertion be true for fewer than $n$ ideals $\pp_i$. If $n>2$, we may (permuting the the $\pp_i$) assume that $\pp_1$ is prime. By the induction assumption, there is $f_i\in I\setminus \bigcap_{i\neq j = 1}^n \pp_i$ for $1\leq i\leq n$. If we also have $f_i\not \in \pp_i$ for some $i$, the assertion holds. Otherwise, $f= f_1+\prod_{i=2}^nf_i$ has the derived properties. We have $f\not\in\pp_i$ for $2\leq i\leq n$ because for such $i$ the second summand is in $\pp_i$ but $f_1$ is not. We have $f\not\in \pp_1$ because $f_1\in\pp_1$ but the factors in $\prod_{i=2}^nf_i$ are not in $\pp_1$ and $\pp_1$ is prime if there is more than one factor. For $n=2$ we have $f_1+f_2\in I\setminus (\pp_1\cup\pp_2)$.
	\end{proof}
	\begin{lem}\lbl{lem:fixedElements}
		Let $L/K$ be a normal extension, $G=\Aut(L/K)$, $f\in L$ be fixed under $G$. If $\cha K = p$ then $f\in K$ when $p=0$ and $f^{p^k}$ for some $k\in\IN$ when $p>0$.
	\end{lem}
	\begin{proof}
		If $p=0$, the field extension is Galois and $K=L^G$ as claimed. 
		
		Assume $L/K$ is finite. Induction on the degree $d=\deg(f/K)$ of the minimal polynomial of $f$. If $d=1$, then $f\in K$, If $d>1$ and the minimal polynomial $P$ has a zero $\phi\neq f$ in an algebraic closure $\ov L$ of $L$, there is a morphism $\theta$ from $K(f)$ to $\ov L$ extending $\id_K$ with $\theta(f) =\phi$. It is possible to extend $K(f)\morphism[\theta] \ov K$ to $L\morphism[\sigma] \ov L$ and $\sigma(L) \subseteq L$ as $L/K$ is normal. But $\sigma(f) = \theta(f) = \phi\neq f$ contradicting $f\in L^G$.
		
		If $d>1$ and $f$ is the only zero of $P$ in $\ov L$, then $f$ is a zero of multiplicity bigger than 1 of $P$ and $P'=0$. Hence $P=Q(T^P)$ for some $Q\in K[T]$. Then $\deg Q = \frac{d}{p}<d$, $\phi =f^p$ is a zero of $Q$ and the induction assumption can be applied. There is $k\in\IN$ such that $\phi^{p^k}\in K$. Then $f^{p^{k+1}} = \phi^{p^k} \in K$.
	\end{proof}
	\begin{proof}[Proof of Theorem \reff{thm:actsTransitively}(a)]
		Assume that $\pp\in\Spec A$, $\qq,\snake\qq\in \Spec B$ is such that $\qq\cap A = \pp = \snake\qq\cap A$ and there is no $\sigma\in G =\Aut(L/K)$ such that $\sigma(\qq) =\snake\qq$. Let $\qq_1=\qq, \qq_2,\ldots,\qq_d$ be the images of $\qq$ under $G$. By Theorem \reff{thm:goingupInIntegralRingExtensions}(b), $\snake\qq\subsetneq\qq_i$. By Lemma \reff{lem:primeAvoidance}, there is $f\in\qq\setminus\bigcup_{i=1}^d \qq_i$. Let $g=\prod_{\sigma\in G}\sigma(f)$. For $\theta\in G$ $\theta(g) = g$ as applying $\theta$ only permutes the factors of $g$. We have $g\in\snake{\qq}$ but $g\not\in\qq$ because otherwise one factor $\sigma(f)$ must be in $\qq$ and $f\in\sigma^{-1} (\qq)$. But $\sigma^{-1}(\qq)$ is among the $\qq_i$. Hence $\phi\in L^G$ and $\phi\in\snake\qq\setminus\qq$. But $A\cap \snake\qq = A\cap\qq = \pp$ a contradiction.
	\end{proof}
	\begin{proof}[Proof of Theorem \reff{thm:actsTransitively}(b)]
		 Let $\pp\in\Spec A$ and let $\qq,\snake\qq\in \Spec B$ such that $\qq\cap A = \snake\qq\cap A = \pp$. Let $\mm = \{(M,\sigma)\}$ where $M$ is an intermediate field $K\subseteq M \subseteq L$, $\sigma\in\Aut(M/K)$ such that $\sigma(\qq\cap M)  = \snake\qq \cap M$. Let $(M,\sigma)\preceq (\snake M,\snake \sigma)$ iff $M\subseteq \snake M$ and $\snake\sigma|_M = \sigma$. Then $\mm \neq \emptyset$ as $(K,\id)\in\mm$ (as $A$ is normal, hence $B\cap K = A$). If $\Ll\subseteq\mm$ is non-empty $\preceq$-linearly ordered, then $\Ll$ is dominated by $(M,\sigma)\in\mm$ where $M = \bigcup_{(N,\theta)\in\Ll} N$ and $\sigma=\bigcap_{(N,\theta)\in\Ll)} \theta$. By Zorns Lemma, $\mm$ has a $\preceq$-maximal element $(M,\sigma)$. If $M=L$ the assertion is proved. 
		 
		 Otherwise, let $x\in L\setminus M$. As $L/K$ is normal, $L$ contains all zeroes $x_1=x,x_2,\ldots, x_d$ of the minimal polynomial of $x$ in $\ov L$. Let $N\subseteq L$ be the subfield generated by $M$ and the $x_i$. Then $N/M$ is normal, being a splitting field. Let $\eta\in \Aut(N/K)$ be some extension f $\sigma\in \Aut(M/K)$ to $N$. Let $\snake A = B\cap M$, $\snake B = B\cap N$, $\snake\pp = \qq\cap M  =\qq\cap \snake A = \eta^{-1}(\snake \qq)\cap M = \eta^{-1}(\snake\qq)\cap \snake A$. By part (a) there is an automorphism $\zeta$ of $N/M$ such that $\zeta(\qq_1) = \qq_2$. Let $\theta = \eta\zeta\in \Aut(N/K)$, then $\theta(\qq\cap N) = \theta(\qq_1) = \eta(\qq_2) = \snake\qq\cap N$ and $(N,\theta)\not\preceq (M,\sigma)$. 
	\end{proof}
