...
\end{proof}
\subsection{Flat base change and cohomology}
...
\end{proof}
\subsection{The Grauert--Grothendieck theorem}
\begin{con}\lbl{con:AON}
	...
	\begin{alphanumerate}
		...
		\item If $\Gg$ is a quasi-coherent $\Oo_Y$-module and $y\in Y$, then we put $\Gg(y)=\Gg_y\otimes_{\Oo_{Y,y}}\KK(y)$.
	\end{alphanumerate}
\end{con}
\begin{thm}\lbl{thm:GrauertGrothendieck}
	...
	\begin{alphanumerate}
		...
		\begin{align*}
			\upsilon^*R^pf_*\Ff\morphism[\text{\eqreff{eq:baseChangeMorphism}}] R^p\snake{f}_*\left(\xi^*\Ff\right)
		\end{align*}
...
\end{lem}
\begin{proof}
	Denote the differentials of $F^\bullet$ and $Q^\bullet$ by $d_F^\bullet$, $d_Q^\bullet$ (the latter of which is yet to be constructed). We will construct the $Q^i$ by downward induction. We start by putting $Q^i=0$ when $i\geq d$. Now let $k\in\IZ$ and suppose we have already constructed $Q^i$ and $d_Q^i$ for $i\geq i$ together with $\rho^i\colon Q^i\morphism F^i$ (compatible with the differentials) such that
	\begin{itemize}
		\item $H^p(Q^\bullet)\isomorphism H^p(F^\bullet)$ is an isomorphism when $p>k$.
		\item $\ker(d_Q^k)\morphism H^k(F^\bullet)$ is surjective.
	\end{itemize}
...
\end{proof}
\subsection{Application to the group structure of elliptic curves}
\begin{defi}
	...
	\end{align*}
	are connected curves of genus $g$ over the algebraic closure $\ov{\KK(x)}$. If $g=1$ and in addition a section  $s\colon X\morphism C$  of $f$ is given, we call $(f,s)$ an \defemph{elliptic curve} over $X$.
\end{defi}
We will typically assume $X$ to be locally noetherian, although Professor Franke says one could generalize the situation to allow arbitrary $X$.
\begin{prop}
	Let $f\colon C\morphism X$ be a flat family of curves of genus $g$, where $X$ is locally noetherian. Moreover, let $\Ll$ be a line bundle on $C$ whose pullbacks to the geometric fibres of $p$ have degree $d$.
	\begin{alphanumerate}
...
\end{prop}
\begin{proof}
	First of all, if $\Ff$ is any quasi-coherent $\Oo_C$-module, then
	...
	Now let's get to business. First of all, we may assume $X=\Spec A$ for some noetherian ring $A$ because all assertions are local. Also Theorem~\reff{thm:GrauertGrothendieck} is applicable since $f$ is flat, so every line bundle on $C$ is flat over $\Oo_X$. Let $P^\bullet$ be the complex from Theorem~\reff{thm:GrauertGrothendieck}\itememph{a}, concentrated within minimum and maximum of the set
	\begin{align*}
	...
	\end{align*}
	(where we use \eqreff{eq:GeometricFibreIso}). Note that $S=\{0\}$ in the case of \itememph{a} and $S=\{1\}$ in the case of \itememph{b}. Indeed, in both cases $S$ is contained in $\{0,1\}$ by Grothendieck's theorem on cohomological dimension (cf.\ \cite[Proposition~1.4.1]{alggeo2}), and then Lemma~\reff{lem:Hpvanishing} determines $S$ (ok, actually it doesn't, but we'll see immediately why $S=\emptyset$ is impossible).\footnote{\ldots except that Lemma~\reff{lem:Hpvanishing} needs regularity of the $C_{\ov{x}}$. But we're talking about Riemann--Roch anyway, so I assume Franke just forgot to mention regularity; I'll ask him about that.} Then $P^\bullet$ consists of at most one non-zero term, which defines a vector bundle on $X=\Spec A$. The assertions about dimension follow immediately from Riemann--Roch in the form of \cite[equation~(3.1.3)]{alggeo2}.
	...
	\begin{align*}
		\dim_{\ov{\KK(x)}}H^1\left(C_{\ov{x}},\Oo_{C_{\ov{x}}}\right)=\dim_{\ov{\KK(x)}}H^0\left(C_{\ov{x}},\Omega_{C_{\ov{c}}}\right)=g\;,
	\end{align*}
	in which the right-most equality holds by definition of $g$. This proves \itememph{c}.
...
\end{proof}
\subsection{Miscellaneous facts about completion}
...
\end{prop}
\begin{rem}
	The conditions from Proposition~\reff{prop:technicalAF} say precisely that the induced morphism $\Spec B\morphism\Spec A$ of schemes is \emph{quasi-finite} at all primes above $\pp$, cf.\ Definition~\reff{def:quasiFinite} and Fact~\reff{fact:annoyingQF}\itememph{b}.
\end{rem}
\begin{proof}[Proof of Proposition~\reff{prop:technicalAF}]
	Part \itememph{a}. The reason why $\qq B_\qq\otimes_{A_\pp}\roof{A}_\pp$ is a maximal ideal of $B_\qq\otimes_{A_\pp}\roof{A}_\pp$ is basically that $\roof{A}_\pp$ is local again with maximal ideal $\roof{\pp}=\pp\roof{A}_\pp$ and has the same residue field $\KK(\pp)$ as $A_\pp$. The details go as follows: We have
	\begin{align*}
...
\end{proof}
\section{Quick reminder about curves and divisors}
This section collects some results which Professor Franke considers known, but I can't find in \cite{alggeo2} (which doesn't mean they haven't been used implicitly, though). 
