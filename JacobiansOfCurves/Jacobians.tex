...
\begin{thm}[Grothendieck's version of Zariski's main theorem]
	\begin{alphanumerate}
		...
		\item Let $f\colon X\morphism Y$ be a quasi-finite morphism between noetherian preschemes. Then there exists a factorization $X\monomorphism[j]\ov{X}\morphism[g]Y$ of $f$ where $j$ is an open immersion and $g$ is finite.
...
\begin{defi}
	Let $R$ be a ring (commutative with $1$) and $I$ an ideal in $R$. Let $M$ be an $R$-module.
	\begin{alphanumerate}
		...
		\end{align*}
		Note that $\roof{R}$ is a ring again. We call $M$ \defemph{complete} in the $I$-adic topology if the canonical morphism $M\morphism\roof{M}$ is an isomorphism.
	\end{alphanumerate}
\end{defi}
\begin{rem}
...
\end{rem}
\begin{example}
	If $I^n=0$ for some $n\in \IN$, then any $R$-module is complete in the $I$-adic topology.
\end{example}
\begin{example}
	If $R=\IZ$ and $I=p\IZ$ for some prime $p$, then $\roof{R}=\IZ_p$ is the ring of $p$-adic integers.
\end{example}
\begin{prop}[Hensel's lemma]\lbl{prop:HenselsLemma}
	Suppose the ring $R$ is complete in the $I$-adic topology. Let $P\in R[T]$ be a polynomial and $a_0\in R$ such that $P(a_0)\equiv 0\bmod I$ and $P'(a_0)$ is a unit in $R/I$. Then there is a unique $a\in R$ such that $a\equiv a_0\bmod I$ and $P(a)=0$.
\end{prop}
\begin{proof}
	\emph{Step 1.} Consider the special case $I^2=0$. For $\delta\in I$ we have $P(a_0+\delta)=P(a_0)+\delta P'(a_0)$ since all terms of order $\delta^2$ or higher vanish in the binomial expansion. Now $P'(a_0)$ being a unit in $R/I$ gives a unique $\delta\in I$ such that $a=a_0+\delta$ satisfies $P(a)=0$.
	
	\emph{Step 2.} Suppose that $I^{2^n}=0$ for some $n\in \IN$. Using induction on $n$ (with the base case being precisely Step~1) we may assume that Hensel's lemma holds for $R/I^{2^{n-1}}$. In particular, there is a unique $a_{n-1}$ such that $P(a_{n-1})\equiv 0\bmod I^{2^{n-1}}$ and $a_{n-1}\equiv a_0\bmod I$. Moreover, $P'(a_{n-1})$ is invertible in $R/I^{2^{n-1}}$. Indeed, this follows from Hensel's lemma applied to $R/I^{2^{n-1}}$ (for which it holds by induction hypothesis) and the polynomial $Q=P'(a_{n-1})T-1$. The derivative $Q'(a_{n-1})$ equals $P'(a_{n-1})$ which is invertible in $R/I$ since $P'(a_{n-1})\equiv P'(a_0)\bmod I$, so Hensel's lemma is indeed applicable. Now replacing $I$ by $I^{2^{n-1}}$ and $a_0$ by $a_{n-1}$ reduces the situation to Step~1, proving the inductive step.
	
	\emph{Step 3.} Now let $I$ be arbitrary. By Step~2 there is for every $n\in\IN$ a unique $a_n\in R/I^{2^n}$ such that $P(a_n)\equiv 0\bmod I^{2^n}$ and $a_n\equiv a_0\bmod I$. Then $a_n\equiv a_{n-1}\bmod I^{2^{n-1}}$ is forced by uniqueness. Hence $a=(a_n)_{n\in \IN}$ defines an element of
	\begin{align*}
		\limit[n\in \IN]R/I^{2^n}=\limit[n\in\IN] R/I^n=\roof{R}\;,
	\end{align*}
	providing the desired element $a\in\roof{R}$.
\end{proof}
\begin{cor}
	Let $R$ be complete in the $I$-adic topology.
	\begin{alphanumerate}
		\item If $a\in R$ becomes a unit in $R/I$, then already $a\in R^\times$.
		\item For every idempotent $\pi\in R/I$ there is a unique idempotent in $R$ whose image modulo $I$ is $\pi $. Therefore, $\Spec R$ and $\Spec R/I$ have the same connected components.
		\item $I$ is contained in the Jacobson radical $\rad R$.
	\end{alphanumerate}
\end{cor}
\begin{proof}
	Part \itememph{a} follows from Proposition~\ref{prop:HenselsLemma} applied to $P=aT-1$ (whose derivative $a$ is a unit in $R/I$ by assumption, so this is fine). For \itememph{b} we use the polynomial $P=T^2-T$. Again, $P'(\pi )=2\pi -1$ is a unit in $R/I$ since $(2\pi -1)^2=4\pi ^2-4\pi +1=1$ in $R/I$. To prove \itememph{c} recall the characterization
	\begin{align*}
		\rad R=\left\{x\in R\st 1-rx\in R^\times\text{ for all }r\in R\right\}\;.
	\end{align*}
	If $x\in I$, then $1-rx$ is a unit in $R/I$, hence also in $R$ by \itememph{a}.
\end{proof}
\begin{prop}
	Let $R$ be noetherian and $N\subseteq M$ finitely generated $R$-modules. Then the $I$-adic topology on $N$ coincides with the induced topology by the $I$-adic topology on $M$.
\end{prop}
\begin{proof}[Sketch of a proof]
...
\begin{fact}
	\begin{alphanumerate}
		...
		\item If $M$ is finitely generated and $I$ is contained in the Jacobson radical of $R$, then $\roof{M}=0$ implies $M=0$.
	\end{alphanumerate}
\end{fact}
\begin{proof}
...
\begin{cor}\lbl{cor:completionExact}
	If $R$ is noetherian, then the functor $M\mapsto \roof{M}$ is exact on the category of finitely generated $R$-modules.
\end{cor}
\begin{proof}
	Let $0\morphism M'\morphism M\morphism M''\morphism 0$ be a short exact sequence of finitely generated $R$-modules. Then $M'+I^nM$ is the kernel of $M\epimorphism M''/I^nM''$. Using $(M'+I^nM)/I^nM\cong M'/(M'\cap I^nM)$ we get short exact sequences
...
\begin{cor}
	...
	\begin{alphanumerate}
		\item When $M$ is a finitely generated $R$-module, then $\roof{M}\cong M\otimes_R\roof{R}$.
		\item $\roof{R}$ is flat as an $R$-module.
		\item Suppose that $I$ is contained in the Jacobson radical of $R$. If $\mu\colon M\morphism N$ is a morphism of finitely generated $R$-modules such that $\roof{\mu}\colon \roof{M}\morphism\roof{N}$ is an isomorphism, then $\mu$ is an isomorphism.
	\end{alphanumerate}
\end{cor}
\begin{proof}
	Part \itememph{a}. Every finitely generated $R$-module is finitely presented as well since $R$ is noetherian. So take a representation $M\cong \coker(R^m\morphism R^n)$ for some $m,n\in\IN$. It's obvious that $(R^n)^\complete\cong \roof{R}^n\cong R^n\otimes_R\roof{R}$. Since both completion and tensor products commute with cokernels, this shows $\roof{M}\cong M\otimes_R\roof{R}$ as well.
	
	Part\itememph{b}. By Corollary~\reff{cor:completionExact} and \itememph{a}, $-\otimes_R\roof{R}$ is exact on finitely generated $R$-modules. By \cite[Proposition~1.2.2]{homalg} this is sufficient for flatness.
	
	Part \itememph{c}. Let $K=\ker\mu$ and $Q=\coker\mu$. Since completion is exact on finitely generated $R$-modules, we get an exact sequence
	\begin{align*}
...
\begin{cor}\lbl{cor:JM}
	If $J\subseteq R$ is any ideal and $M$ a finitely generated $R$-module, then $(JM)^\complete\morphism\roof{M}$ defines an isomorphism $(JM)^\complete\isomorphism J\roof{M}$.
\end{cor}
\begin{proof}
...
\begin{cor}\lbl{completionLocal}
	If $R$ is a noetherian local ring with maximal ideal $\mm$, then $\roof{R}$ is local with maximal ideal $\mm \roof{R}$.
\end{cor}
\begin{proof}
	We proved this in \cite[Corollary~2.2.2]{homalg}.
\end{proof}
\begin{prop}\lbl{prop:completionNoetherian}
	Let $R$ be noetherian and $I\subseteq R$ any ideal, then the $I$-adic completion $\roof{R}$ is noetherian again.
\end{prop}
To prove this, we need to prove the evil twin of Hilbert's Basissatz first.
\begin{lem}\lbl{lem:(not)HilbertBasis}
	If $R$ is noetherian, then so is the power series ring $R\llbracket T\rrbracket$.
\end{lem}
\begin{proof}
	We can (and will) basically copy the proof of Hilbert's Basissatz. Let $J\subseteq R\llbracket T\rrbracket$ be any ideal and put $J_n=\left\{a_n\st \sum_{k=n}^\infty a_kT^k\in J\right\}$ for $n\geq 0$. Then $(J_n)_{n\in \IN}$ form an ascending sequence of ideals in $R$. Noetherianness of $R$ tells us that this sequence becomes eventually stationary, say, at $n=s$. So we may choose $a^{(i)}=\sum_{k=s}^\infty a_kT^k\in R\llbracket T\rrbracket$ for $i=1,\ldots,N$ such that $a_s^{(1)},\ldots,a_s^{(N)}$ generate $J_s$. Then $a^{(1)},\ldots,a^{(N)}$ generate $J\cap T^sR\llbracket T\rrbracket$. Indeed, given any $b=\sum_{k=s}^{\infty}b_kT^k\in J$ we can inductively choose coefficients $r_k^{(1)},\ldots,r_k^{(N)}\in R$ such that $r^{(i)}=\sum_{k=0}^{\infty}r_k^{(i)}T^k$ satisfy $r^{(1)}a^{(1)}+\ldots+r^{(N)}a^{(N)}=b$ up to degree $T^{s+k}$. This works because $J_{k+s}=J_s$ for all $k\geq 0$ is generated by $a_s^{(1)},\ldots,a_s^{(N)}$ again.
	
	Now $R\llbracket T\rrbracket/T^sR\llbracket T\rrbracket$ is a finitely generated $R$-module, hence the image of $J$ in it is finitely generated as well, $R$ being noetherian. We thus may choose $a^{(N+1)},\ldots,a^{(N+M)}\in J$ whose images modulo $T^sR\llbracket T\rrbracket$ generate the image of $J$ in $R\llbracket T\rrbracket/T^sR\llbracket T\rrbracket$. Then $a^{(1)},\ldots,a^{(N+M)}$ generate $J$ and our job's done here.
\end{proof}
\begin{proof}[Proof of Proposition~\reff{prop:completionNoetherian}]
	Let $r_1,\ldots,r_n$ be generators of $I$. Then sending $X_i\mapsto r_i$ defines a surjective morphism $R\llbracket X_1,\ldots,X_n\rrbracket\epimorphism \roof{R}$. Since $R\llbracket X_1,\ldots,X_n\rrbracket$ is noetherian by Lemma~\reff{lem:(not)HilbertBasis} and induction on $n$, so is its quotient $\roof{R}$.
\end{proof}
\begin{cor}
	Suppose that $R$ is a noetherian local ring and $I\subseteq R$ any (proper) ideal. Then $\dim R=\dim\roof{R}$. In particular, $R$ is regular iff $\roof{R}$ is regular.
\end{cor}
\begin{proof}
	Let $\mm$ be the maximal ideal of $R$. Then $\roof{\mm}=\mm\roof{R}$ (this equality holds because of Corollary~\ref{cor:JM}) is the maximal ideal of the local ring $\roof{R}$ as was shown in the proof of \cite[Corollary~2.2.2]{homalg}. Since $I\subseteq \mm$, the quotients $\mm^i/\mm^{i+1}$ already have $I$-torsion, hence 
	\begin{align*}
		\mm^i/\mm^{i+1}\cong \left(\mm^i/\mm^{i+1}\right)^\complete\cong \roof{\mm}^i/\roof{\mm}^{i+1}
	\end{align*}
	(the last isomorphism follows from exactness of completion). This shows that the associated graded rings $\gr(R,\mm)$ and $\gr(\roof{R},\roof{\mm})$ agree, hence $(R,\mm)$ and $(\roof{R},\roof{\mm})$ have the same Hilbert--Samuel polynomials, which shows $\dim R=\dim\roof{R}$ by \cite[Theorem~20]{alg2}.
	
	Now $R$ and $\roof{R}$ have the same residue field $k$ and $\mm/\mm^2\cong \roof{\mm}/\roof{\mm}^2$ by the $I$-torsion arguments we have seen several times now, so $\dim_k\mm/\mm^2=\dim_k\roof{\mm}/\roof{\mm}^2$. Clearly this implies that $R$ is regular iff $\roof{R}$ is.
\end{proof}
\begin{rem}
	In a similar fashion one can show that a noetherian local ring is Cohen--Macaulay, or Gorenstein, or a complete intersection, iff its $I$-adic completion is one as well. For example, for Cohen--Macaulayness one would need to show $\depth_R(R)=\depth_{\roof{R}}(\roof{R})$, which follows from the isomorphism $\Ext_R^p(k,R)\cong\Ext_{\roof{R}}^p(k,\roof{R})$ that was described in the proof of \cite[Proposition~2.4.2]{homalg}.
\end{rem}
