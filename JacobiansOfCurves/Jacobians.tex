...
Professor Franke intended to use Chevalley's theorem (Proposition~\reff{prop:Chevalley}) to prove Lemma~\reff{lem:OpenEmbedding}, but I don't understand how that's supposed to work. The proof I came up with doesn't need Chevalley's theorem, but this theorem is surely worthwhile to know, so I put it in the appendix (page~\pageref{prop:GenericFreeness}).
...
\end{rem}
	\lbl{par:EllipticCurves}Let $f\colon E\morphism S$ with a section $\epsilon\colon S\morphism E$ be an elliptic curve over $S$. Then Proposition~\reff{prop:canonicalSection} may be used to obtain a bijection
	\begin{align*}
...
\end{enumerate}
\section{The seesaw theorem and the theorem of the cube}
...
\end{rem}
\begin{thm}[Theorem of the cube]\lbl{thm:cube}
	...
	\end{diagram*}
	 ...
	 \begin{align*}
	 	\Ll|_X\coloneqq\Ll(\sigma_Y\xi,\id_X)^*\Ll\cong \xi^*\Ll_0\text{ on }X\quad\text{and}\quad\Ll|_Y\coloneqq(\sigma_X\upsilon,\id_Y)^*\Ll\cong \upsilon^*\Ll_0\text{ on }Y\;.
	 \end{align*}
	 Then the closed subprescheme $Z\subseteq S$ obtained from Theorem~\reff{thm:seesaw} applied to $X\times_SY\morphism S$ is also open.
\end{thm}
...
\begin{rem}
	\begin{alphanumerate}
		...
		\item Theorem~\reff{thm:cube} is typically applied in the following situation: Assume $X\cong A^m$ and $Y\cong A^n$ (and thus also $X\times_SY\cong A^{m+n}$) are powers of some abelian (group-)prescheme $A$ over $S$. Let morphisms
		\begin{align*}
...
\begin{proof}
	Define the set
	\begin{align*}
		\PP=\left\{(\Ll,\iota)\st\Ll\text{ is a line bundle on }X\text{ and }\iota\colon \Ll|_{X_0}\isomorphism\Oo_{X_0}\right\}\Bigm/\sim\;,
	\end{align*}
	in which we put $(\Ll,\iota)\sim(\Mm,\kappa)$ iff there is a $\lambda\colon \Ll\isomorphism \Mm$ such that $\iota=\kappa\lambda|_{X_0}$. The idea of the proof is to identify $\TT=\left\{\text{isomorphism classes of }\Jj\text{-torsors on }X\right\}$ with $\PP$ and then $\PP$ with $K=\ker\big(\Pic(X)\morphism\Pic(X_0)\big)$. Since $\TT\cong H^1(X,\Jj)$ by \cite[Proposition~1.7.1\itememph{b}]{alggeo2}, this will do the trick.
	
	A map $\PP\morphism\TT$ can be obtained by sending an equivalence class $[\Ll,\iota]\in\PP$ to the $\Jj$-torsor $\Tt$ defined by
	\begin{align*}
		\Tt(U)=\left\{\ell\in\Ll(U)\st\iota(\ell|_{U\cap X_0})=1\right\}.
	\end{align*}
	The $\Jj$-action is given by multiplication with $\exp(j)=1+j$ for $j\in\Jj(U)$. Since $\Jj^2=0$, this indeed transforms the ``additive'' group structure on $\Jj$ into a ``multiplicative'' one, and since $X_0=V(\Jj)$ this action is well-defined. Moreover, if $[\Mm,\kappa]\in\PP$ is another equivalence class, then the image of $[\Ll\otimes_{\Oo_X}\Mm,\iota\otimes\kappa]$ under $\PP\morphism\TT\isomorphism H^1(X,\Jj)$ is the sum of the images of $[\Ll,\iota]$ and $[\Mm,\kappa]$ (this can be checked explicitly using the construction from \cite[p.~32]{alggeo2}; I'm not going to do this here). This will eventually imply that $K\morphism H^1(X,\Jj)$ is an isomorphism of groups and not only a bijection of sets.
	
	An inverse map $\TT\morphism\PP$ sends a $\Jj$-torsor $\Tt$ to a line bundle $\Ll$ defined as follows: We put
	\begin{align*}
		\Ll_{[x]}=\left\{(a,\tau)\st a\in \Oo_{X,x}\text{ and }\tau\in\Tt_x\right\}/\approx\;,
	\end{align*}
	in which $(a_1,\tau_1)\approx(a_2,\tau_2)$ iff there is a $j\in \Jj_x$ such that $a_1(1+j)=a_2$ and $\tau=j\tau_2$ (where $j\tau_2$ denotes the $\Jj_x$-action on $\Tt_x$). Then $\Ll$ is the sheaf glued together from the $\Ll_{[x]}$ using a coherence condition. In precise terms,
	\begin{align*}
		\Ll(U)=\left\{(\ell_x)_{x\in U}\in\prod_{x\in U}\Ll_{[x]}\st
		\begin{tabular}{c}
			for all $y\in U$ there is an open neighbourhood $V\ni y$\\
			 together with an $\ell=(a,\tau)\in\Oo_X(V)\times\Tt(V)$ such\\
			that $\ell_x$ is the image of $\ell$ in $\Oo_{X,x}\times\Tt_x$ for all $x\in V$
		\end{tabular}\right\}
	\end{align*}
	Note that for fixed $\tau\in\Tt_x$, every element $\ell\in\Ll_{[x]}$ has a unique representation $\ell=[a,\tau]$ for some $a\in \Oo_{X,x}$ (because $1+j$ is invertible with inverse $1-j$ for all $j\in \Jj_x$, using that $\Jj_x^2=0$). Thus it's possible to define an $\Oo_{X,x}$-module structure on $\Ll_{[x]}$ by fixing $\tau$ and then looking at the first components of elements $[a,\tau]\in\Ll_{[x]}$. This turns $\Ll$ into a line bundle over $\Oo_X$. Moreover, if $x\in X_0$, then the image of $[1,\tau]$ in $\Ll_{[x]}\otimes_{\Oo_{X,x}}\Oo_{X_0,x}\cong \Ll_{[x]}/\Jj_x\Ll_{[x]}$ is the same regardless what $\tau$ is. This provides a trivialization $\iota\colon \Ll|_{X_0}\isomorphism \Oo_{X_0}$.
	
	It's easy to see that the constructed maps $\PP\morphism\TT$ and $\TT\morphism\PP$ are well-defined and mutually inverse. This shows $\PP\cong \TT$.
	
	It remains to identify $\PP$ with $K=\ker\big(\Pic(X)\morphism\Pic(X_0)\big)$. Sending $[\Ll,\iota]$ to the isomorphism class of $\Ll$ clearly defines a surjective map $\PP\morphism K$. To show injectivity of this map, assume that $[\Ll,\iota]$ and $[\Mm,\kappa]\in\PP$ are such that $\Ll\cong \Mm$. Then we may assume $\Ll=\Mm$, but still $\iota$ and $\kappa$ could be ``non-isomorphic trivializations'' of $\Ll|_{X_0}$, which should be ruled out. That's where we'll need the hypothesis that $\Oo_X(X)\morphism\Oo_{X_0}(X_0)$ is surjective.
	
	On every affine open $U\subseteq X$ where $\Ll|_U$ trivializes there's an $\epsilon_U\in\Oo_{X_0}(U\cap X_0)^\times$ such that $\iota(\ell)=\epsilon_U\kappa(\ell)$ for all $\ell\in\Ll(U)$. Clearly we have $\epsilon_{U_1}|_{U_1\cap U_2}=\epsilon_{U_1\cap U_2}=\epsilon_{U_2}|_{U_1\cap U_2}$, so the $\epsilon_U$ glue together to an element $\epsilon\in\Oo_{X_0}(X_0)^\times$ such that $\epsilon|_U=\epsilon_U$. Let $\epsilon'\in\Oo_X(X)$ be any preimage of $\epsilon$. Since $\Jj^2=0$, $\epsilon'$ is invertible as well, hence multiplication with $\epsilon'$ is an automorphism of $\Ll$ that identifies $\iota$ and $\kappa$. Therefore $\ker\big(\Pic(X)\morphism\Pic(X_0)\big)\cong H^1(X,\Jj)$ as sets, and as indicated above, this bijection is compatible with the group structure on both sides.
...
\begin{proof}[Proof of Theorem~\reff{thm:cube} \textsc{(Finally)}.]\lbl{proof:cube}
	...
	\emph{Step 2.} We reduce to the case of artinian local rings. Let $S'\morphism S$ be any morphism with $S'\cong \Spec A$, where $A$ is artinian local with residue field $k$. Let $Z'\subseteq S'$ be the closed subprescheme associated with $\Ll|_{X\times_SY\times_SS'}$. By Step~1 we know that $Z'$ is also open, hence $Z'=\emptyset$ or $Z'=S'$. But if the restriction $\Spec k\morphism S$ of $j$ to $\Spec k$ factors over $Z$, then this shows that $Z'$ can't be empty. Hence $Z'=S'$, so $\Ll|_{X\times_SY\times_SS'}$ is already trivial in $\Pic_{X\times_SY/S}(S')$. But then $j\colon S'\morphism S$ factors over $Z$.
	
	That is, the upcoming Lemma~\reff{lem:ZClopen} can be applied, whence we are done.
\end{proof}
Last but not least we prove the two lemmas that have been outsourced during the above proof of Theorem~\reff{thm:cube}.
\begin{lem}\lbl{lem:GlobalSectionOverArtinianScheme}
	Let $A$ be a artinian local ring and $f\colon X\morphism S=\Spec A$ a flat proper morphism with integral geometric fibres $X_{\ov{s}}$. Then $\Oo_X(X)\cong A$.
\end{lem}
\begin{proof}
	Let $s$ be the only point of $S$ and put $k=\KK(s)$. By Proposition~\reff{prop:trivialLineBundlesOnProperX/k}, $H^0(X_{\ov{s}},\Oo_{X_{\ov{s}}})\cong \ov k$. Since $H^0(X_{\ov{s}},\Oo_{X_{\ov{s}}})\cong H^0(X_s,\Oo_{X_s})\otimes_k\ov{k}$, this shows that $H^0(X_s,\Oo_{X_s})$ is a one-dimensional $k$-vector space as well, hence generated by $1$. Since $1$ is in the image of $(f_*\Oo_X)(s)\morphism H^0(X_s,\Oo_{X_s})$, this morphism is surjective. Then it must be bijective because of Theorem~\reff{thm:GrauertGrothendieck}\itememph{d}, which means that $(f_*\Oo_X)(s)\cong \Oo_X(X)\otimes_Ak$ is generated (as a $k$-vector space) by $1$. Then $1$ is also a generator of $\Oo_X(X)$ as an $A$-module by Nakayama's lemma. Since $\Oo_X(X)$ is flat (and thus free) over the local ring $A$, this shows $\Oo_X(X)\cong A$.
\end{proof}
...
\begin{proof}
	...
	\end{align*}
	hence \itememph{a} $\Rightarrow$ \itememph{e} is trivial. Also \itememph{e} $\Rightarrow$ \itememph{d} for trivial reasons. Now if $\mm$ is a maximal ideal of $A$, then $\mm$ has a preimage in $\Spec B$ iff $0\neq \Spec (B\otimes_A\KK(\mm))\cong \Spec B/\mm B$. Hence \itememph{b} and \itememph{d} are equivalent and we're done.
\end{proof}
...
commutes (since this is also the case for the $\Ff|_{U_i}$). On the other hand, if $(\Gg_i)_{i\in I}$ and $(\mu_{i,j})_{(i,j)\in I^2}$ are given such that \eqreff{diag:muij} is commutative, then it's easy to see that these data define a sheaf on $Y$.
 ...
 Note that $U_i\cap U_j$ can also be written as $U_i\times_YU_j$. Also the disjoint union $\coprod_{(i,j)\in I^2}U_{i,j}$ can be written as $X\times_YX$ in the case where $X=\bigcup_{i\in I}U_i$. Therefore, providing a family of isomorphisms $\mu_{i,j}\colon \Gg_i|_{U_{i,j}}\isomorphism \Gg_j|_{U_{i,j}}$ is equivalent to giving a single isomorphism $\pi_1^*\Gg\cong \pi_2^*\Gg$, where $\pi_1,\pi_2\colon X\times_YX\morphism X$ are the projections to the two factors. Also the diagrams \eqreff{diag:muij} can be reformulated as a single large diagram. We'll immediately make this precise. 
...
\begin{defi}\lbl{def:descentData}
	Let $f\colon X\morphism Y$ be a morphism of preschemes. A \defemph{descent datum} for quasi-coherent sheaves of modules along $f$ is a pair $(\Gg,\nu)$, where $\Gg$ is a quasi-coherent $\Oo_X$-module and $\nu\colon \pi_1^*\Gg\isomorphism\pi_2^*\Gg$ an isomorphism, such that the following \sout{monster} moderately large diagram commutes (which is often called the \emph{cocycle condition}).
	\begin{diagram*}
		\object{60:3.5}{$\pi_{1,2}^*\pi_2^*\Gg$}[a];
		\object{120:3.5}{$\pi_{1,2}^*\pi_1^*\Gg$}[b];
		\object{180:3.5}{$\pi_{1,3}^*\pi_1^*\Gg$}[c];
		\object{240:3.5}{$\pi_{1,3}^*\pi_2^*\Gg$}[d];
		\object{300:3.5}{$\pi_{2,3}^*\pi_2^*\Gg$}[e];
		\object{0:3.5}{$\pi_{2,3}^*\pi_1^*\Gg$}[f];
		\object{30:3.5}{$p_2^*\Gg$}[1];
		\object{150:3.5}{$p_1^*\Gg$}[2];
		\object{270:3.5}{$p_3^*\Gg$}[3];
		\scriptsize
		\isoarrow{2}{b};
		\isoarrow2c;
		\isoarrow d3;
...
If $\cat C=(\cat C_T)_{T\in\Ob(\cat{PSch}/S)}$ is such a fibred category over $\cat{PSch}/S$ and $f\colon X\morphism Y$ a morphism of $S$-preschemes, we can define \defemph{$\cat C$-valued descent data} along $f$ as pairs $(\Gg,\nu)$, where $\Gg\in\Ob(\cat C_X)$ and $\nu\colon \pi_1^*\Gg\isomorphism\pi_2^*\Gg$ is an isomorphism, such that the moderately large diagram from Definition~\reff{def:descentData} commutes. This gives a category $\cat{Desc}_{X/Y}^{\cat{C}}$, and, like in the situation before, there is a functor
\begin{align*}
...
\begin{proof}[Sketch of a proof]
	...
	Put $\pi=f\pi_1=f\pi_2$. By the proof of Theorem~\reff{thm:faithfullyFlatDescent} there is an isomorphism $\Oo_Y\isomorphism Rf^*\Oo_Y\cong R\Oo_X$. Unraveling what this actually means, we obtain for all open subsets $U\subseteq Y$ an isomorphism
	...
	But also surjectivity is easy to see from \eqreff{eq:OYcongROX}. Indeed, suppose $h\colon X\morphism T$ has the property that $h\pi_1=h\pi_2$. If $W\subseteq T$ is open and $\theta\in\Oo_T(W)$, then $h^*\theta$ clearly satisfies 
	\begin{align*}
		\pi_1^*h^*\theta=(h\pi_1)^*\theta=(h\pi_2)^*\theta=\pi_2^*h^*\theta\;.
	\end{align*}
	That is, $h^*\theta$ is contained in the right-hand side of \eqreff{eq:OYcongROX}. It requires some more (trivial, technical) checks, but this basically shows that a $g^*\colon \Oo_T\morphism g_*\Oo_Y$ can be reconstructed from $h^*$.
\end{proof}
...
\begin{example}\lbl{ex:bigfppf}
	...
	\begin{alphanumerate}
		\item As in Example~\reff{ex:bigfppf}\itememph{a}, but the $V_{i,j}\morphism U_i$ are flat and of finite presentation and $\coprod_{j=1}^{n_i}V_{i,j}\morphism U_i$ is fppf.
...
\begin{proof}[Sketch of a proof of Example~\reff{ex:bigfpqc} and \reff{ex:bigfppf}]
	...
	So the interesting ones are Example~\reff{ex:bigfppf}\itememph{c} and \itememph{d}. The proof of \itememph{c} $\Leftrightarrow$ \itememph{a} uses Cohen--Macaulay properties and can be found in \cite[\stackstag{056X}]{stacks-project}. For \itememph{d} $\Leftrightarrow$ \itememph{a}, the crucial thing to notice is that every flat morphism of locally finite presentation is an open map\footnote{Indeed, in the case of locally noetherian preschemes we proved this in Corollary~\reff{cor:flatMorphismOpen}. The strategy in the general case is pretty much the same: First one has to tweak our notion of constructibility so that it behaves well in the non-noetherian case (see \cite[\stackstag{04ZC} and  \stackstag{00I0}]{stacks-project}). Then one proves a generalization of Chevalley's theorem to arbitrary morphisms of locally finite presentation. The key idea in the proof is a really nice trick that reduces everything to the noetherian case. You can find a very nice exposition of this in \url{https://amathew.wordpress.com/2010/12/26/}.}, so that for every $V_{i,j}$ we may replace $U_i$ by the image $U_{i,j}$ of $V_{i,j}\morphism U_i$. Then $X=\bigcup_{i,j}U_{i,j}$ is an open cover which satisfies condition \itememph{c}.
...
\begin{defi}
	Let $\cat C$ be a category and $\cat A$ whatever target category we want to have (like sets, (abelian) groups, rings, \ldots). An \defemph{$\cat A$-valued presheaf on $\cat C$} is a contravariant functor $\Ff\colon \cat C^\op\morphism\cat A$. 
	
	If $\cat C$ is equipped with a Grothendieck topology $J$, then a presheaf $\Ff$ is called a \defemph{sheaf} if for all objects $X$ and all $\Ss\in J_X$ the following condition holds: The morphisms $\Ff(f)\colon\Ff(X)\morphism\Ff(Y)$ for all $(f\colon Y\morphism X)\in\Ss$ induce an isomorphism
...
\begin{rem}
	\begin{alphanumerate}
		...
		\begin{align*}
			...
			\end{tabular} \right\}\;,
		\end{align*}
		and $\Ff(X)$ maps into it via $\phi\mapsto (\Ff(f)\phi)_{f\in\Ss}$ for all $\phi\in\Ff(X)$.
...
\begin{example}\lbl{ex:sheafifyingPic}
	...
	Therefore, if we show that $K$ is a Zariski sheaf, then it must be equal to $\Pic_{X/S}$. To show the Zariski sheaf axiom, we take the proof from \cite[\stackstag{0B9N}]{stacks-project} (note that the Stacks Project guys actually show the fppf sheaf axiom). Let $T=\bigcup_{i\in I}U_i$ be an open cover. We use the usual convention $U_{i,j}\eqqcolon U_i\cap U_j$. Let us also introduce the shortcut $X_i=X_{U_i}$ (and likewise for more indices) to save us some notational pain. Now suppose we're given $\Ll_i\in K(U_i)$ for $i\in I$ such that $\Ll_i$ and $\Ll_j$ have the same image in $K(U_{i,j})$. Hence we may fix isomorphisms $\phi_{i,j}\colon \Ll_i|_{X_{i,j}}\isomorphism\Ll_j|_{X_{i,j}}$, and also isomorphisms $\alpha_i\colon \Oo_{U_i}\isomorphism \sigma_{U_i}^*\Ll_i$ (by definition of $K(U_i)$). Then the composition
	\begin{align*}
...
\begin{prop}\lbl{prop:unramified}
	Let $f\colon X\morphism S$ be a morphism of locally finite type between locally noetherian schemes. Then the following conditions are equivalent.
	\begin{alphanumerate}
...
\end{defi}
\begin{prop}
	A finite flat morphism $f\colon X\cong \SPEC_S\Aa\morphism S$ with $S$ locally noetherian is étale iff
	\begin{align*}
		(a,b)\longmapsto \Tr_{\Aa/\Oo_S}(ab)
	\end{align*}
	defines a non-degenerate self-duality on the locally free $\Oo_S$-module $\Aa$.
\end{prop}
\begin{rem}
	\begin{alphanumerate}
		\item Such \emph{étale coverings} are used to define  $\pi_1^\et(X)$ in SGA 1.
		\item Assuming the $V_{i,j}\morphism U_i$ in the above definition of fppf to be unramified in addition to the other conditions, one obtains the famous \emph{étale topology}.
	\end{alphanumerate}
\end{rem}
\begin{defi}
	Let $f\colon X\morphism S$ be a morphism between locally noetherian preschemes.
	\begin{alphanumerate}
		\item We call $f$ \defemph{smooth} at $x\in X$ iff there is an étale morphism $U\morphism \IA_S^n$ for $S$-preschemes for some open neighbourhood $U$ of $x\in X$.
...
\begin{defi}
	An \defemph{abelian scheme} over $S$ is a proper flat $S$-prescheme $\alpha\colon A\morphism S$ with the structure of a group scheme over $S$ and such that all geometric fibres $A_{\ov{s}}=A\times_S\Spec\ov{\KK(s)}$ are varieties.\footnote{That is, integral and of finite type (but the latter is automatically implied by properness of $\alpha$). It's about time we define what a variety is, really.}
\end{defi}
\begin{prop}
	Let $S$ be locally noetherian and $\xi\colon X\morphism S$ be a proper morphism whose geometric fibres $X_{\ov{s}}=X\times_S\Spec\ov{\KK(s)}$ are varieties. Let $\upsilon \colon Y\morphism S$ be another $S$-prescheme and $f\colon X\morphism Y$ be any morphism over $S$. Then
	\begin{align*}
		U=\left\{s\in S\st \text{the image of }f_{\ov{s}}\colon X_{\ov{s}}\to Y_{\ov{s}}\text{ consists of  a single point}\right\}
	\end{align*}
	is open in $X$. Moreover, for a morphism $\tau\colon T\morphism S$ the base change $f_T\colon X_T\morphism Y_T$ factors over $\xi_T\colon X_T\morphism T$ iff the image of $\tau$ is contained in $U$. If, in addition, $\xi$ is flat, then $U$ is also closed.
\end{prop}
\begin{proof}
	The first step is as usual to replace as much as possible of the occuring schemes by affine ones. Since the assertion is local on $S$, we may assume that $S$ is affine.	Let $s\in U$, then $f(X_s)=\{y\}\subseteq Y_s$ consists of a single point, since this is also true for the geometric fibres. Let $W\subseteq Y$ be an affine open neighbourhood. As $\xi$ is proper (hence universally closed), the subset $Z=\xi(X\setminus f^{-1}(W))$ is closed in $S$ and moreover $s\notin Z$. Let $\lambda\in\Oo_S(S)$ such that $s\notin V(\lambda)$ and $V(\lambda)\supseteq Z$. Replacing $S$ by $S\setminus V(\lambda)$ and $Y$ by $\upsilon^{-1}(S\setminus V(\lambda))=W\setminus V(\upsilon ^*\lambda)$ we achieve that both $S$ and $Y$ are affine.
	
	Put $S=\Spec A$ and $Y=\Spec B$. Then $\Hom_{\cat{PSch}/S}(X,Y)\cong \Hom_{\cat{Alg}(A)}(B,\Oo_X(X))$ (by a well-known adjunction). But $\Oo_X(X)\cong A$ by assumption, hence $f$ is given by a section $B\morphism A$ of $\upsilon^*\colon A\morphism B$. Hence [TODO].
	
	For the flatness assertion, assume first that $\xi$ is faithfully flat. If $\xi$ has a section $\sigma\colon S\morphism X$, then [TODO] is closed as $\xi$ is proper. In general, forming $U$ is compatible with base change, and after the fpqc base change by $\xi$ itself, $\xi$ has a section (the diagonal). Thus $U$ is closed when $\xi$ is faithfully flat. If $\xi$ is only flat, then the image of $\xi$ is open (by flatness) and closed (by properness), hence we may reduce to the faithfully flat case.
\end{proof}
\begin{rem*}
	Without the flatness assumption one could take $X$ to be a blow-up of  a point of $S$ and $f$ to be the identity $\id_X\colon X\morphism X$. Then the closedness assumption fails.
\end{rem*}
\begin{thm}
	Let $S$ be locally noetherian and $\alpha\colon A\morphism S$ an abelian scheme.
	\begin{alphanumerate}
		...
		\begin{align*}
			f=\phi+_Bb\alpha\;,
		\end{align*}
		where $\phi\colon A\morphism B$ is a morphism of group schemes over $S$ and $b\colon S\morphism B$ a section of $\beta$; and $+_B$ denotes the group structure on $\Hom_{\cat{PSch}/S}(-,B)$.
		\item Let $\pi_1,\pi_2\colon A\times_SA\morphism A$ be the two canonical projections. Then
		\begin{align*}
			s_A^{[TODO]}\colon s_A^*\Omega_A\morphism \Omega_{A\times_SA}\lisomorphism \pi_1^*\Omega_A\oplus\pi_2^*\Omega_A
		\end{align*}
		is the graph of an isomorphism $\nu\colon \pi_1^*\Omega_A\isomorphism\pi_2^*\Omega_A$ giving the structure of a descent datum for $\Omega_A$ with respect to $\alpha$. Thus $\Omega_A\cong \alpha^*\omega_A$ for some vector bundle $\omega_A$ on $S$.
		\item Let $P\morphism S$ be a proper flat morphism whose geometric fibres $P_{\ov{s}}$ are projective spaces (i.e., isomorphic to $\IP_k^n$, where $k=\ov{{\KK(s)}}$ and $n$ may depend on $s$), then any morphism $f\colon P\morphism A$ of $S$-preschemes factors over $\pi$.
	\end{alphanumerate}
\end{thm}
\begin{proof}
	We start with \itememph{b}.
\end{proof}
...
\begin{proof}[Proof of Proposition~\reff{prop:technicalAF}]
	...
	Part \itememph{b}. Brace yourself, because the proof to come is \emph{horrible}. I'm trying my best to cite The Stacks Project as rarely as possible, but I make no promises. We will separate the agonizing process into tiny lemmas.
