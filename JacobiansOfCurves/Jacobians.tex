...
Now let $f\colon X\morphism Y$ be a quasi-finite separated morphism between noetherian preschemes such that Zariski's main theorem holds with $Y$ replaced by $\Spec \roof{\Oo}_{Y,y}$. Note that $\pi_Y\colon \Spec \roof{\Oo}_{Y,y}\morphism Y$ is flat (in the sense of the upcoming Definition~\reff{def:flat}) because of Corollary~\reff{cor:completionFaithfullyFlat}\itememph{b}. Therefore, in the pullback diagram
...
Note that the local rings $\roof{\Oo}_{Y,y}$ and $\Oo_{Y,y}$ have the same residue field $\KK(y)$. Hence $\Spec \KK(y)\morphism Y$ can be factored through $\pi_Y\colon \Spec \roof{\Oo}_{Y,y}\morphism Y$ and together with the Stein factorizations of $f$ and $\roof{f}$ this gives a diagram
...
\end{align*}
its base change along $\Spec B\morphism\Spec A$. Also let $\pi_X\colon \snake{X}\morphism X$ be projection to the other fibre product factor. We want to investigate the relation between the cohomology of $X$ and $\snake{X}$. For a quasi-coherent $\Oo_X$-module $\Ff$ and an affine \v Cech cover $\Uu\colon X=\bigcup_{i\in I}U_i$ of $X$, the \v Cech complex $\check{C}^\bullet(\Uu,\Ff)$ calculates $H^\bullet(X,\Ff)$. Pulling back $\Uu$ gives a \v Cech cover
\begin{align*}
	\pi_X^{-1}\Uu\colon \snake{X}=\bigcup_{i\in I}U_i\times_{\Spec A}\Spec B\;,
\end{align*}
of $\snake{X}$, whose components $U_i\times_{\Spec A}\Spec B$ are affine again (as fibre products of affine schemes), hence $\pi_X^{-1}\Uu$ may be used to compute $H^\bullet(\snake{X},\pi_X^*\Ff)$. Explicitly, we obtain
\begin{align}\lbl{eq:tensoredCechComplex}
	\check{C}^\bullet\left(\pi_X^{-1}\Uu,\pi_X^*\Ff\right)\cong \check{C}^\bullet(\Uu,\Ff)\otimes_AB\;.
\end{align}
This gives a canonical morphism
\begin{align}\lbl{eq:tensoredCohomology}
	H^\bullet(X,\Ff)\otimes_AB\morphism H^\bullet(\snake{X},\pi_X^*\Ff)\;,
\end{align}
...
\begin{prop}\lbl{prop:baseChangeMorphism}
	...
	\begin{align}\lbl{eq:baseChangeMorphism}
		\pi_Y^*R^pf_*\Ff\morphism R^p\snake{f}_*(\pi_X^*\Ff)
	\end{align}
	which is an isomorphism when $\pi_Y$ is flat in the sense of Definition~\reff{def:flat}.
\end{prop}
\begin{proof}
...
\begin{varthm}{prop}\lbl{prop:baseChangeMorphismA}
	...
	\begin{align*}
		E_2^{p,q}=\Tor_{-p}^A\big(B,H^q(X,\Ff)\big)\converge H^{p+q}\left(\snake{X},\pi_X^*\Ff\right)\;.
	\end{align*}
\end{varthm}
\begin{rem}
...
\begin{align*}
	...
	\end{cases}\;.
\end{align*}
By \eqreff{eq:tensoredCechComplex} we get $\vphantom{E}^\hor E^{0,q}\cong H^q(\snake{X},\pi_X^*\Ff)$ and $\vphantom{E}^\hor E^{p,q}=0$ if $p<0$. Therefore the horizontal spectral sequence immediately degenerates, and its limit is $H^{p+q}(\snake{X},\pi_X^*\Ff)$. That's precisely what we need.
\end{proof}
...
\begin{thm}\lbl{thm:GrauertGrothendieck}
	...
	\begin{alphanumerate}
		...
		\end{align*}
		of finitely generated projective $A$-modules with the following property. If $B$ is an $A$-algebra and $\pi_X\colon \snake{X}=X\times_Y\Spec B\morphism X$ is the projection of the fibre product to its first factor, then
		\begin{align*}
			H^p\left(\snake{X},\pi_X^*\Ff\right)\cong H^p(P^\bullet\otimes_AB)\;.
		\end{align*}
		...
		\begin{align*}
			\pi_Y^*R^pf_*\Ff\morphism[\text{\eqreff{eq:baseChangeMorphism}}] R^p\snake{f}_*\left(\pi_X^*\Ff\right)
		\end{align*}
		...
		\begin{align}\lbl{eq:RpfFy}
			(R^pf_*\Ff)_y\isomorphism H^p\left(X_y,\Ff|_{X_y}\right)\;.
		\end{align}
...
\end{rem}
	...
	\begin{align}\lbl{eq:baseChangeProperty}
		H^p(\snake{X},\pi_X^*\Ff)\cong H^p(\check{C}^\bullet\otimes_AB)
	\end{align}
	...
	The rest of the proof of \itememph{a} doesn't concern any $\check{C}^\bullet$-specific properties except flatness and the base change property. However, we will rather freely use facts from homological algebra. If you want to brush up your knowledge, have a look at \cite{homalg}.
	
	We start with a sequence of lemmas now.
...
\begin{proof}[Proof of Theorem~\reff{thm:GrauertGrothendieck}]
	...
	\end{proof}
	We come back to the proof of Theorem~\reff{thm:GrauertGrothendieck}\itememph{b} and \itememph{c}. The upper-semicontinuity part of \itememph{b} follows immediately from Lemma~\reff{lem:BasicallyThm4bc}\itememph{a} applied to $M=Q^p$ und $F=P^{p+1}$. For part \itememph{c}, we may assume the base change was done along $\pi_Y\colon \snake{Y}=\Spec B\morphism Y=\Spec A$ because the question is local on $Y$ and $\snake{Y}$. Then $R^p\snake{f}_*(\pi_X^*\Ff)$ is a quasi-coherent $\Oo_{\snake{Y}}$-module given by
	\begin{align*}
		H^p(\snake{X},\pi_X^*\Ff)\cong H^p(P^\bullet\otimes_AB)\cong \ker\left(\delta^p\otimes\id_B\colon Q^p\otimes_AB\morphism P^{p+1}\otimes_AB\right)\;,
	\end{align*}
	and $\pi_Y^*R^pf_*\Ff$ is given by
	\begin{align*}
	...
	\begin{lem}
		In the above situation, the following conditions on a prime ideal $\pp\in\Spec A$ are equivalent.
		\begin{alphanumerate}
