...
Let $f\colon X\morphism Y=\Spec A$ be a  morphism of quasi-compact schemes. Let $I\subseteq A$ be any ideal and $\Ff$ a quasi-coherent sheaf of $\Oo_X$-modules. For every $n\geq 0$, the projection $\Ff/I^{n+1}\Ff\morphism \Ff/I^n\Ff$ gives a canonical morphism
\begin{align}\lbl{eq:HpXn+1toHpXn}
	H^p(X,\Ff/I^{n+1}\Ff)\morphism H^p(X,\Ff/I^n\Ff)\;.
\end{align}
...
\end{align}
for all $p\geq 0$. A priori this is only a morphism of $A$-modules, but $H^p(X,\Ff/I^n\Ff)$ is an $I^n$-torsion module (as can be seen from a close inspection of the \v Cech complex), so $H^p(X,\Ff/I^n\Ff)$ is actually an $A/I^n$-module. Therefore \eqreff{eq:Hmorphism} factors over
\begin{align}\lbl{eq:Hmorphism2}
	H^p(X,\Ff)/I^nH^p(X,\Ff)\morphism H^p(X,\Ff/I^n\Ff)\;.
\end{align}
These morphisms are compatible with the canonical morphisms from equation \eqreff{eq:HpXn+1toHpXn} -- you can just check that on affine \v Cech covers. Taking the limit over \eqreff{eq:HpXn+1toHpXn} and \eqreff{eq:Hmorphism2} thus gives a canonical morphism
\begin{align}\lbl{eq:formalFunctions}
	H^p(X,\Ff)^\complete\morphism\limit[n\geq 1]H^p(X,\Ff/I^n\Ff)\;,
\end{align}
...
\begin{thm}[Grothendieck--Zariski]\lbl{thm:FormalFunctions}
	...
	\begin{align*}
		H^p(X,\Ff)^\complete\isomorphism \limit[n\geq 1]H^p(X,\Ff/I^n\Ff)\;.
	\end{align*}
\end{thm}
...
\begin{proof}
	...
	\end{claim}
	Assuming this for the moment, the long exact cohomology sequence associated to $0\morphism I^n\Ff\morphism \Ff\morphism \Ff_n\morphism 0$ appears as
	\begin{align*}%\lbl{eq:FFSeq1}
		\ldots\morphism H^p(X,I^n\Ff)\morphism H^p(X,\Ff)\morphism H^p(X,\Ff/I^n\Ff)\morphism H^{p+1}(X,I^n\Ff)\morphism\ldots \;.
	\end{align*}
	...
	\begin{align}\lbl{eq:FFSeq2}
		0\morphism U_n\morphism H^p(X,\Ff)/I^nH^p(X,\Ff)\morphism H^p(X,\Ff/I^n\Ff)\morphism V_n\morphism 0\;,
	\end{align}
	...
	\end{claim}
	Before we prove this (and Claim~\reff{claim:FFclaim1}), let's see how Theorem~\reff{thm:FormalFunctions} follows from it. Let $W_n$ be the image of $H^p(X,\Ff)/I^nH^p(X,\Ff)\morphism H^p(X,\Ff/I^n\Ff)$. We may split \eqreff{eq:FFSeq2} into two short exact sequences
	\begin{gather*}
		0\morphism U_n\morphism H^p(X,\Ff)/I^nH^p(X,\Ff)\morphism W_n\morphism 0\\
		\hphantom{\;.}0\morphism W_n\morphism H^p(X,\Ff/I^n\Ff)\morphism V_n\morphism 0\;.
	\end{gather*}
	Applying Fact~\reff{fact:6termLimitSequence} to the first one gives $H^p(X,\Ff)^\complete\cong \limit[][^1]W_n$. Then the six-term exact sequence associated to the second proves $\limit W_n\cong \limit H^p(X,\Ff/I^n\Ff)$ and we are done.
	...
	\emph{Proof of Claim~\reff{claim:FFclaim1}.} Let $\upsilon\colon\snake{Y}=\Spec R\morphism Y$ correspond to $A\monomorphism R$ and let $\xi\colon \snake{X}=X\times_Y\snake{Y}\morphism X$ be its base change by $f$. Note that $\xi$ is affine as a base change of the affine morphism $\upsilon$ (we use \cite[Corollary~2.5.1]{alggeo1} here). Then $\xi_*\xi^*\Ff\cong\bigoplus_{n\geq 0}I^n\otimes_A\Ff$ (this is easily checked affine-locally, where $\xi^*$ is given by tensoring with $R$), and it's not hard to see that there is a quotient module $\snake{\Ff}$ of $\xi^*\Ff$ such that $\xi_*\snake{\Ff}\cong \bigoplus_{n\geq 0} I^n\Ff$; we leave the details to the reader. Also $H^p(\snake{X},\snake{\Ff})\cong H^p(X,\xi_*\snake{\Ff})$ as $\xi$ is affine. This shows
	\begin{align*}
		H^p(\snake{X},\snake{\Ff})\cong H^p(X,\xi_*\snake{\Ff})\cong H^p\bigg(X,\bigoplus_{n\geq 0}I^n\Ff\bigg)\cong \bigoplus_{n\geq 0}H^p(X,I^n\Ff)=K^p\;.
	\end{align*}
	...
	Since properness is a \emph{proper}ty (tee-hee) which is stable under base change, $\snake{f}\colon \snake{X}\morphism\snake{Y}=\Spec R$ is proper, being a base change of the proper morphism $f$. Moreover $\snake{\Ff}$ is a coherent $\Oo_{\snake{X}}$-module because it is a quotient of $\xi^*\Ff$, which is coherent (as a pullback of a coherent module). Hence $H^p(\snake{X},\snake{\Ff})$ is a finitely generated $R$-module by our finiteness results for the cohomology of proper morphisms (cf.\ \cite[Theorem~5]{alggeo2}). We win.
\end{proof}
\begin{rem}
	For every $n\geq 0$ consider the morphism
	\begin{align*}
	i_n\colon X_n=X\times_Y\Spec (A/I^n)\morphism X\;,
	\end{align*}
	which is a base change of the closed immersion $Y_n=\Spec (A/I^n)\monomorphism \Spec A$, hence a closed immersion itself. Also, if $f$ is proper, then so is $X_n\morphism Y_n$ because properness is stable under base change as well (by \cite[Remark~2.4.1]{alggeo2}).
	
	Let $\Ff|_{X_n}=i_n ^*\Ff$ denote the restriction of $\Ff$ to $X_n$ (this notation is slightly abusive, but convenient).  Since $i_n$ is a closed immersion and thus affine, we have an isomorphism $H^p(X,i_{n,*}\Ff|_{X_n})\cong H^p(X_n,\Ff|_{X_n})$ for all $p\geq 0$ by \cite[Corollary~1.6.1]{alggeo2}. But it's also easy to check (e.g.\ affine-locally) that $i_{n,*}\Ff|_{X_n}\cong \Ff/I^n\Ff$. This gives the following reformulation of Theorem~\reff{thm:FormalFunctions}.
\end{rem}
\begin{varthm}{thm}\lbl{thm:FormalFunctions1.5}
	Under the assumptions of Theorem~\reff{thm:FormalFunctions} we have a canonical isomorphism
	\begin{align*}
...
\begin{varthm}[b]{thm}\lbl{thm:FormalFunctions2}
...
\begin{proof}
	...
	\end{align*}
	Also $(X\times_Y\Spec A_\pp)\times_{\Spec A_\pp}\Spec(A_\pp/\mm^n)\cong X\times_Y\Spec(A_\pp/\mm^n)\cong X_n$ by a bit abstract nonsense. Now Theorem~\reff{thm:FormalFunctions1.5} may be applied to $X\times_Y\Spec A_\pp\morphism \Spec A_\pp$ (the base change of $f$) and the assertion follows.
\end{proof}
...
\begin{defi}\lbl{def:quasiAffine}
	...
	\end{alphanumerate}
\end{defi}
For a proof of equivalence, I'd suggest to check out \cite[\stackstag{01SJ}]{stacks-project}. Note that Franke apparently had a different proof (for the case of quasi-affine morphisms of finite type) in mind than the Stacks Project guys, and Franke's proof would somehow use the following Lemma~\reff{lem:coherentExtension}, but I have no idea how this was going to work (whereas the Stacks Project proof is pretty clear to me and works without restrictions on $f$). Nevertheless, the lemma will proof useful in the future.
...
Now let $f\colon X\morphism Y$ be a quasi-finite separated morphism between noetherian preschemes. Doing induction on $\dim Y$, with the base case $\dim Y=-\infty$ (for $Y=\emptyset$) being trivial, we may assume that Zariski's main theorem is true for all $Y'$ satisfying $\dim Y'<\dim Y$. Then Part~II together with the inductive hypothesis shows that Zariski's main theorem holds with $Y$ replaced by $\Spec \roof{\Oo}_{Y,y}$ for all $y\in Y$. We show that this implies Zariski's main theorem for $Y$ as well.

Note that $\upsilon\colon \Spec \roof{\Oo}_{Y,y}\morphism Y$ is flat (in the sense of the upcoming Definition~\reff{def:flat}) because of Corollary~\reff{cor:completionFaithfullyFlat}\itememph{b}. Therefore, in the pullback diagram
...
\begin{alphanumerate}
	...
	\item The morphism $u\otimes\id_{\KK(\pp)}\colon M\otimes_A\KK(\pp)\morphism P\otimes_A\KK(\pp)$ is injective.
\end{alphanumerate}
\end{lem}
\begin{proof}
...
\begin{proof}
	...
	\begin{align}\lbl{eq:a1a2Omega}
		\sigma^*(p_1^\oast+p_2^\oast)\colon a_1^*\Omega_A\oplus a_2^*\Omega_A\isomorphism \sigma^*\Omega_{A\times_SA}\;.
	\end{align}
	Consider $\sigma_1=(\id_A, a_2\alpha)\colon A\morphism A\times_SA$ and $\sigma_2=( a_1\alpha,\id_A)\colon A\morphism A\times_SA$. We get canonical morphisms $\sigma_i^\oast\colon \sigma_i^*\Omega_{A\times_SA}\morphism \Omega_A$ for $i\in \{1,2\}$. Pulling back along $a_i$ and using $\sigma=\sigma_1 a_1=\sigma_2a_2$, we get canonical morphisms $a_i^*(\sigma_i^\oast)\colon \sigma^*\Omega_{A\times_SA}\morphism a_i^*\Omega_A$.
	\begin{claim}\lbl{claim:technicalAFInverse}
		The morphism $(a_1^*(\sigma_1^\oast),a_2^*(\sigma_2^\oast))\colon \sigma^*\Omega_{A\times_SA}\morphism a_1^*\Omega_A\oplus a_2^*\Omega_A$ is an inverse of the above isomorphism $\sigma^*(p_1^\oast+p_2^\oast)$ in equation~\eqreff{eq:a1a2Omega}. 
	\end{claim}
	It suffices to check this on sections $\omega$ of $\sigma^*\Omega_{A\times_SA}$ which happen to have the form $\omega=\sigma^*(p_1^\oast)(\vartheta)$ or $\omega=\sigma^*(p_2^\oast) (\vartheta)$ for some section $\vartheta$ of $a_1^*\Omega_A$ or $a_2^*\Omega_A$ respectively, since we know from \eqreff{eq:a1a2Omega} that $\sigma^*\Omega_{A\times_SA}\cong a_1^*\Omega_A\oplus a_2^*\Omega_A$ is generated by sections $\omega$ of this form. If $\omega=\sigma^*(p_1^\oast)(\vartheta)$, then
	\begin{align*}
		a_1^*(\sigma_1^\oast)(\omega)=\big(a_1^*(\sigma_1^\oast)\circ  \sigma^*(p_1^\oast)\big)(\vartheta)&=\big(a_1^*(\sigma_1^\oast)\circ  a_1^*\sigma_1^*(p_1^\oast)\big)(\vartheta)\\
		&=a_1^*\big(\sigma_1^\oast\circ  \sigma_1^*(p_1^\oast)\big)(\vartheta)\\
		&=a_1^*\big((p_1\sigma_1)^\oast\big)(\vartheta)\\
		&=a_1^*(\id_A^\oast)(\vartheta)\\
		&=\vartheta
	\end{align*}
	(in the third line we use the fact that $(fg)^\oast=g^\oast\circ g^*(f^\oast)$), and similarly
	\begin{align*}
		a_2^*(\sigma_2^\oast)(\omega)=\big(a_2^*(\sigma_2^\oast)\circ  \sigma^*(p_1^\oast)\big)(\vartheta)&=\big(a_2^*(\sigma_2^\oast)\circ  a_2^*\sigma_2^*(p_1^\oast)\big)(\vartheta)\\
		&=a_1^*\big(\sigma_2^\oast\circ  \sigma_2^*(p_1^\oast)\big)(\vartheta)\\
		&=a_1^*\big((p_1\sigma_2)^\oast\big)(\vartheta)\\
		&=a_1^*((a_1\alpha)^\oast)(\vartheta)\\
		&=0\;,
	\end{align*}
	since $(a_1\alpha)^\oast=\alpha^\oast\circ \alpha^*(a_1^\oast)$ factors over $\Omega_S=0$. Similar calculations apply to the case $\omega=\sigma^*(p_2^\oast)(\vartheta)$, thus proving Claim~\reff{claim:technicalAFInverse}.
	
	By some more technical calculations it follows that 
	\begin{align*}
...
\end{cor}
\begin{proof}
	Since $[n]$ is proper by the two-out-of-three property from \cite[Proposition~2.4.1]{alggeo2}, it suffices by Theorem~\reff{thm:ZariskiMain}\itememph{a} to check that it is quasi-finite and flat. Of course, quasi-finiteness can be checked fibre-wise, and so can $\deg{[n]}=n^{2g}$ once flatness has been established. Moreover, the critère de platitude par fibres (Lemma~\reff{lem:platitudeDeFibres}) shows that flatness too can be checked on fibres. Then all we need to do is applying Theorem~\reff{thm:abelianVarieties}\itememph{b} to each fibre.
...
Let $D$ be a divisor on a regular curve $C$ (as always, this also means that $C$ is connected and proper) of genus $g=\dim_k\Gamma(C,\Omega_C)$ over an algebraically closed field $k$. Recall the notation $\ell(D)=\dim_k\Gamma(C,\Oo_C(D))$. If $\deg D=g$ and $K$ is a divisor such that $\Oo_C(K)\cong \Omega_C$, then Riemann--Roch (in the form of \cite[Corollary~3.1.1]{alggeo2}) shows
\begin{align*}
...
\begin{itemize}
	\item \emph{Points.} On point-set level, $X/G$ consists of the $G$-orbits, and $\pi\colon X\morphism X/G$ maps a point to its orbit.
