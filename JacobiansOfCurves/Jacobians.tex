...
\end{proof}
\begin{rem}
	Note that in the lecture Franke used $\Kk_n\cong \Jj^n\Ff$ instead of $I^n\Ff$, where $\Jj=f^{-1}\Ii$ is the inverse image (in the sense of Definition~\reff{def:f-1}). But $\Jj^n\Ff\cong I^n\Ff$ -- which is not that surprising, since the $I^n$-action on $\Ff$ is given via the algebraic component $\Oo_Y\morphism f_*\Oo_X$ of $f$, so $I^n\Ff=\Jj^n\Ff$ is pretty obvious from the construction of $f^{-1}$ described in the proof of Lemma~\reff{lem:f-1} below. I prefer the notation $I^n\Ff$ -- in particular, this is how Grothendieck denotes it in \cite[Ch.\:III (4.1.7)]{egaIII}, so I believe it's my right to do so as well. Nevertheless, Lemma~\reff{lem:f-1} is \emph{perhaps worthwhile to know} (if you get what I mean), so we will include it now.
\end{rem}
