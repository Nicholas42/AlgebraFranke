...
\begin{proof}
	...
	Finally \itememph{b} $\Rightarrow$ \itememph{a} is trivial, and also we've already seen that $B_\pp^{p+1}$ is free over $A_\pp$ in the proof of \itememph{a} $\Rightarrow$ \itememph{c}. We're done.
\end{proof}
	...
	Finally, part \itememph{e}. If $y\in U_p$ corresponds to $\pp\in\Spec A$, then $C_\pp^{p+1}$ as well as $B_\pp^{p+1}$ are free over $A_\pp$ by Lemma~\reff{lem:BasicallyThm4de}\itememph{c}. Therefore the short exact sequence
	\begin{align*}
...
\begin{proof}
	...
	\end{claim}
	To prove Claim~\reff{claim:DCxfiniteIntersection}, we first remark that $H^0(C_s,\Ll|_{C_s})$ is a one-dimensional $\KK(s)$-vector space. Indeed, to show this, it suffices to prove the same for $H^0(C_{\ov{s}},\Ll|_{C_{\ov{s}}})$ by Remark~\reff{rem:GeometricFibres}\itememph{a}. Now $C_{\ov{s}}$ is proper and integral over $\ov{\KK(s)}$, hence $H^0(C_{\ov{s}},\Ll|_{C_{\ov{s}}})$ is a domain and a finite-dimensional vector space over the algebraically closed field $\ov{\KK(s)}$, which only leaves
	\begin{align*}
		H^0\left(C_{\ov{s}},\Oo_{C_{\ov{s}}}\right)\cong \ov{\KK(s)}\;,
	\end{align*}
	as claimed.
...
\end{enumerate}
We will eventually realize this plan in Chapter~\reff{sec:Jac}. Step~1 is done in Proposition~\reff{prop:symmetricPowers}. Step~2 is treated in Theorem~\reff{thm:final}\itememph{f}. Finally Step~3 and 4 follow from the combined efforts of Section~\reff{sec:Construction}.
