...
\begin{fact}
	...
	\end{alphanumerate}
\end{fact}
\begin{rem}\lbl{rem:SheafInverseImage}
	So far, we have seen three kinds of inverse image functors. Let $f\colon X\morphism Y$ be a continuous map of topological spaces, where for $f^*$ we additionally assume that $X$ and $Y$ are ringed with structure sheaves $\Oo_X,\Oo_Y$. Then we have
	\begin{align*}
		\begin{array}{r c l c}
	...
	Now let $\Mm$ be a sheaf of rings on the ringed space $(Y,\Oo_Y)$. In general, $f^\sharp \Mm$ will only be a $f^\sharp\Oo_Y$-module, so we need to tensor it by $\Oo_X$ to get the pullback $f^*\Mm$. That is, $f^*\Mm=f^\sharp\Mm\otimes_{f^\sharp\Oo_Y}\Oo_X$. By Remark~\reff{rem:tensorProductSheaves}, its stalks are given by
	\begin{align*}
	...
	Since $f^*\Oo_Y=\Oo_X$ (this is clear on stalks) and $f^*$ preserves direct sums (in fact, as a left-adjoined functor it preserves arbitrary colimits, cf.\ Remark~\reff{rem:AdjunctionPreservesStuff}), so inverse images of vector bundles (resp.\ line bundles) are vector bundles (resp.\ line bundles) again by base-locality of $f^*$.
	
	Moreover, when $f$ is a morphism between affine schemes $X=\Spec B$ and $Y=\Spec A$ (thus) coming from a ring morphism $A\morphism B$, then $f^*\snake{M}=(B\otimes_AM)^\sim$ for all $A$-modules $M$ (which is easily seen stalk-wise). It follows that $f^*$ restricts to a functor $f^*\colon \cat{QCoh}(Y)\morphism\cat{QCoh}(X)$.
\end{rem}
...
\end{proof}
By choosing a representative for all possible isomorphism classes of line bundles on $X$ which $\phi^*\Oo_P(1)$ may belong to (and by Remark~\reff{rem:SheafInverseImage} it is indeed a line bundle on $X$), Proposition~\reff{prop:PROJQuasiAdjunction} gives
\begin{cor}
...
\end{proof}
\begin{rem*}
	The isomorphism $\PROJ_T(\tau^*\Rr)\isomorphism\PROJ_S(\Rr)\otimes_ST$ exists in full generality, even when $\Rr$ is not generated by $\Rr_0$ and $\Rr_1$.
\end{rem*}
Let $\Rr$ be an $\IN_0$-graded quasi-coherent $\Oo_S$-algebra, $\Ll$ a line bundle on $S$ and
\begin{align*}
	\Rr^{(\Ll)}=\bigoplus_{k=0}^\infty\Rr_k\otimes \Ll^{\otimes k}
\end{align*}
...
\begin{align*}
	\zeta^*\Tt_k\simeq \zeta^*\Rr_k\otimes_{\Oo_Z}\zeta^*\Ss_k\epimorphism\Ll^{\otimes k}\simeq p_X^*\Oo_X(k)\otimes_{\Oo_Z}p_Y^*\Oo_Y(k)
\end{align*}
...
\end{prop}
\begin{example*}
	We have the closed embedding $\IP_R^m\times\IP_R^n\morphism\IP_R^{m+n+nm}$ defined by 
	\begin{align*}
		R[X_0,\ldots,X_m]_k\otimes_RR[Y_0,\ldots,Y_n]_k\lmorphism R\left[Z_{i,j}\st 1\leq i\leq m,\ 1\leq j\leq n\right]_k
	\end{align*}
	sending $Z_{i,j}$ to $X_i\otimes Y_j$.
\end{example*}
