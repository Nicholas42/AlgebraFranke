...
\begin{defi}
	...
	\end{diagram*}
\end{defi}
Note that $\epsilon$ in the above diagram is automatically an isomorphism. Indeed, we can apply the snake lemma in the abelian category $\Oo_X\cat{-Mod}$ to obtain the vanishing of both $\ker(\epsilon)$ and $\coker(\epsilon)$.
...
\begin{cor}
	...
	\begin{alphanumerate}
		...
		\begin{align}\lbl{eq:Hn+1Iso}
			\begin{split}
				\tau\colon H^{n+1}\big((X_0^\ell,\ldots,X_n^\ell)\big)&\isomorphism R\\
				[f]&\longmapsto \text{the coefficient of }(X_0X_1\cdots X_n)^{\ell-1}\text{ in }f(0,1,\ldots,n)
			\end{split}
		\end{align}
		...
		\begin{align}\lbl{eq:KoszulNonDegPairing}
			S_k\times H^{n+1}\big((X_0^\ell,\ldots,X_n^\ell),S\big)_{-n-1-k}\xrightarrow{\text{multiplication}}H^{n+1}\big((X_0^\ell,\ldots,X_n^\ell),S\big)_{-n-1}\overset{\text{\eqreff{eq:Hn+1Iso}}}{\simeq} R\;.
		\end{align}
	\end{alphanumerate}
\end{cor}
\begin{proof}
...
Let $R$ be an $\IN$-graded ring. Let $R_+\subseteq R$ be the ideal of elements of $R$ of positive homogeneous degree. Recall that there is a bijection
\begin{align*}
...
We define a sheaf of graded rings $\Oo(-)$ on $P=\Proj(R)$ as follows. Let $\Oo(k)_{[\pp]}=(R_\pp)_k$ for homogeneous prime ideals $\pp\subseteq R$ be obtained from $R$ by inverting the \emph{homogeneous} elements of $R_\pp$ only. Note that the subsets of the form $P\setminus V(f)$ with $f\in R$ homogenous form a topology base of $P$. We put
\begin{align*}
	\Oo(k)(U)=\left\{(g_\pp)\in\prod_{\pp\in U}(R_\pp)_k\st\begin{array}{c}
		...
		\text{such that }g_\pp=\left(\text{image of }\gamma\text{ under }R_f\morphism R_\pp\right)\text{ for all}\pp\in V
	\end{array}\right\}\;.
\end{align*}
Then there are operations
\begin{align*}
	\Oo(k)\times \Oo(k)&\morphism \Oo(k)\;,\quad (f_\pp)_{\pp\in U}+(g_\pp)_{\pp\in U}=(f_\pp+g_\pp)_{\pp\in U}\\
	\Oo(k)\times \Oo(\ell)&\morphism \Oo(k+\ell)\;,\quad (f_\pp)_{\pp\in U}\cdot (g_\pp)_{\pp\in U}=(f_\pp g_\pp)_{\pp\in U}	
\end{align*}
and a map
\begin{align}\lbl{eq:ProjPretenderStalk}
	\begin{split}
		...
		\left(\begin{array}{c}
			\text{image of}\\
			(q_\qq)\in\Oo(k)(U)\text{ in }\Oo(k)_\pp
...
\begin{prop}
	\begin{alphanumerate}
		\item $\Proj(R)$ is a scheme. Open subsets of the form $\Proj(R)\setminus V(f)$ where $f\in R_k$ with $k>0$ are affine and form a topology base.
		\item Every finite subset of $\Proj(R)$ is contained in an affine open subset.
		\item The sheaf of modules $\Oo(k)$ on $\Proj(R)$ is quasi-coherent, and there is a homeomorphism
		\begin{align}\lbl{eq:ProjLocal}
			...
			\end{split}
		\end{align}
		under which $(R_\pp)_0\simeq ((R_f)_0)_\qq$. We thus get an identification $\Oo(0)|_{\Proj(R)\setminus V(f)}\simeq \Oo_{\Spec (R_f)_0}$.
		
		Moreover, under \eqreff{eq:ProjLocal} there is a similar isomorphism $(R_\pp)_k\simeq ((R_f)_k)_\qq$ (the latter denotes the localization of the $(R_f)_0$-module $(R_f)_k$ with respect to $\qq$, that is, with respect to the homogeneous elements of $R_f\setminus \qq$). This gives an identification
		...
		\end{align*}
		is an isomorphism.
		\item We have a morphism of preschemes 
		\begin{align}\lbl{eq:Proj(R)toSpecR0}
			...
			\end{split}
		\end{align}
		and $\pi^*\colon \Oo_{\Spec R_0}\morphism \pi_*\Oo_{\Proj(R)}$ is defined by sending an element $rf^{-k}\in (R_0)_f\simeq \Oo_{\Spec R_0}(\Spec R_0\setminus V(f))$ to precisely that element in $\Oo(0)(\Proj(R)\setminus V(f))$. The morphism $\pi$ is of finite type iff $R$ is an $R_0$-algebra of finite type.
		\item If $R$ is Noetherian (which is the case iff $R_0$ is Noetherian and $R$ of finite type over $R_0$) then $\Proj(R)$ is Noetherian and the $\Oo(k)$ are coherent.
	\end{alphanumerate}
\end{prop}
