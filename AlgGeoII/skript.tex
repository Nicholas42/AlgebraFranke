...
\end{proof}
Recall the construction of the quasi-coherent $\Oo_{\Proj R}$-module $\Mm=\snake{M}$ from Remark~\reff{rem:GradedSnakeM} when $M$ is a graded module over the graded ring $R$. When $R$ is of finite type over $R_0$, an argument similar (but even more technical) to the proof of Proposition~\reff{prop:FirstProjProperties}\itememph{e} shows that $(M_f)_0$ is a finitely generated $(R_f)_0$-module for every homogeneous $f\in R_+$, i.e., $\Mm$ is locally finitely generated. When $R_0$ is, in addition, Noetherian, Proposition~\reff{prop:FirstProjProperties}\itememph{f} assures that $\Mm$ is coherent.
...
\end{proof}
\begin{rem*}
	The isomorphism $\PROJ_T(\tau^*\Rr)\isomorphism\PROJ_S(\Rr)\times_ST$ exists in full generality, even when $\Rr$ is not generated by $\Rr_0$ and $\Rr_1$.
\end{rem*}
...
\begin{proof}
	...
	\begin{align*}
		\IP_Y^n\times_Y\snake{Y}\simeq \PROJ_{\snake{Y}}\left(\upsilon^*\Oo_Y[X_0,\ldots,X_n]\right)\simeq \PROJ_{\snake{Y}}\Oo_{\snake{Y}}[X_0,\ldots,X_n]\simeq \IP_{\snake{Y}}^n
	\end{align*}
...
\begin{prop}\lbl{prop:ClosedImmersionPreservesProjective}
	Let $X\morphism[i]Y$ be a closed immersion and $Y\morphism[f]S$ a projective (resp.\ strongly projective) morphism. Then $f\circ i$ is (strongly) projective again.
\end{prop}
\begin{proof}
	For strong projectiveness, this is trivial (the class of closed immersions is closed under composition). Let $f$ be projective and $Y=\PROJ_S(\Rr)$ where $\Rr$ is as in Definition~\reff{def:projectiveMorphism}\itememph{a}. Let $\Jj_X$ be the sheaf of ideals defining the closed subprescheme $X\subseteq Y$ and let $\Ii_n\subseteq \Rr_n$ be the preimage of $f_*\Jj_X(n)\subseteq f_*\Oo_Y(n)$ under the epimorphism\footnote{Where does this come from? Well, we may consider this locally. Let $U\subseteq S$ be affine and $\Rr|_U=\snake{R}$, then $f_*\Oo_Y(n)(U)=\Oo_Y(k)(f^{-1}(U))\simeq \Oo(n)(\Proj R)\simeq R_n$ by Theorem~\reff{thm:CohoOfIP^n}\itememph{a}, so we get surjections on stalks.} $\Rr_n\epimorphism f_*\Oo_Y(n)$. Let $\snake{\Rr}=\Rr/\Ii$ and $\snake{X}=\PROJ_S(\snake{\Rr})$. 
	...
	We claim that $\snake{X}=X$ as closed subpreschemes of $Y$, proving the claim as $\snake{X}\morphism S$ is projective. Obviously, $X\subseteq \snake{X}$ as all sections of $\Ii_n$ define sections of $\Oo_Y(n)$ vanishing on $X$ by definition. That equality holds is a local question hence the following Lemma~\reff{lem:ProjClosedEmbedding} may be applied. 
\end{proof}
\begin{lem}\lbl{lem:ProjClosedEmbedding}
	Let $Y=\Proj R$ and $X\subseteq Y$ a closed subprescheme defined by the sheaf of ideals $\Jj_X\subseteq \Oo_Y$. Let $I\subseteq R$ an ideal defined as in the proof of Proposition~\reff{prop:ClosedImmersionPreservesProjective}. If $r\in R_k$ and $g\in\Jj_X(k)(U)$ for the open subset $U=Y\setminus V(r)$, then there is some $\ell\in\IN$ and $\gamma\in I_{k\ell+k}$ such that $g=\gamma r^{-\ell}$.
\end{lem}
\begin{proof}
	Apply Lemma~\reff{lem:powersOfLl}\itememph{b} to the quasi-coherent $\Oo_Y$-module $\Jj_X$ and the ample line bundle $\Oo(k)$. Note that we can do all this because $R_1$ is finitely generated by Definition~\reff{def:projectiveMorphism}\itememph{a}, hence $\Proj R$ is quasi-compact by the proof of Proposition~\reff{prop:FirstProjProperties}\itememph{e}. Moreover, $\Proj R$ is a scheme and thus quasi-separated.
\end{proof}
\begin{cor}
	\begin{alphanumerate}
...
\end{rem*}
\begin{prop}
	When $X\morphism[f]Y\morphism[g]Z$ are morphisms of preschemes such that $gf$ is proper and $g$ separated, then $f$ is proper.
\end{prop}
