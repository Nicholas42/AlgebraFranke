...
\end{proof}
If $X$ is an $S$-prescheme (cf.\ \cite[Definition~1.5.7]{alggeo1}), we have a homomorphism of abelian groups
\begin{align*}
...
\begin{rem}
	\begin{alphanumerate}
		...
		\end{align*}
		for every quasi-coherent $\Oo_X$-module $\Mm$. Isomorphism classes of line bundles on $X$ get an abelian group structure with $-\otimes_{\Oo_X}-$ defining multiplication, $\Oo_X$ the neutral element and $\Ll^{-1}$ the inverse to $\Ll$. This group is called the \defemph{Picard group} $\Pic(X)$.
...
Let $k$ be algebraically closed. All schemes to appear are supposed to be $\Spec k$-schemes and $\Omega_X^1\coloneqq\Omega_{X/k}$. Also, we use the term \emph{curve} for one-dimensional schemes.
\begin{thm}\lbl{thm:CurvesSerreDuality}
	...
	\begin{alphanumerate}
		...
		\end{diagram}
		commutes.
		\item For every vector bundle $\Vv$ on $C$, there is a non-degenerate pairing
		\begin{align*}
...
\begin{enumerate}
	...
	\item \ldots\ which is group homomorphism $\Pic(C)\morphism\Div(C)$. 
\end{enumerate}
...
\begin{fact}\lbl{fact:OC(D)tensor}
	If $D_1,D_2\in\Div(C)$ are divisors, then $\Oo_C(D_1)\oplus\Oo_C(D_2)\simeq\Oo_C(D_1+D_2)$.
\end{fact}
\begin{proof}
...
\begin{fact}\lbl{fact:Ll(pmx)}
	Let $D$ denote the divisor defined by the closed point $x$ of $C$. For $\Ll$ a line bundle on $C$, put $\Ll(\pm x)=\Ll\otimes_{\Oo_C}\Oo_C(\pm D)$. Then
	\begin{align*}
		\chi(C,\Ll(\pm x))=\chi(\Ll)\pm 1\;,
	\end{align*}
	hence \eqreff{eq:RiemannRoch4} holds for $\Ll(\pm x)$ iff it holds for $\Ll$.
\end{fact}
\begin{proof}
	We only need to show this for $\Ll(-x)$ as $\Oo_C(D)\otimes_{\Oo_C}\Oo_C(-D)\simeq \Oo_C$ by Fact~\reff{fact:OC(D)tensor}, so the roles of $\Ll$ and $\Ll(-x)$ may  be switched to show the other assertion.
	
	The residue field $\KK(x)$ of the local ring $\Oo_{C,x}$ is a $k$-algebra of finite type, hence $\KK(x)\simeq k$ by the Nullstellensatz and the fact that $k$ is algebraically closed. We thus get a closed embedding $\Spec k\simeq\{x\}\morphism[i]C$. Let $\pi$ be a uniformizer of the DVR $\Oo_{C,x}$. Since we are in a Noetherian setting\footnote{The technical argument we need here is \cite[Lemma~1.5.1]{alg2}.}, $\pi$ also generates the maximal ideal $\mm_x\in\Oo_C(U)$ defining $x$ for some affine open neighbourhood $U\ni x$ (that is, we need $v_x(\pi)=1$ and $v_c(\pi)=0$ when $c\in U\setminus\{x\}$ is closed). Shrinking $U$ if necessary we may moreover assume that $\Ll$ trivializes. Let $\lambda\in\Ll^\times(U)$ be a generator. We get a short exact sequence
	\begin{align*}
		0\morphism\Ll(-x)\morphism[\pi\cdot]\Ll\morphism[p]i_*\Oo_{\Spec k}\morphism 0\;,
	\end{align*}
	in which $p$ is given as follows. Let $\ell\in\Ll(V)$, where $V\subseteq C$ is open and $x\in V$ (otherwise $p$ is just the zero morphism). Let $r\in\Oo_{C,x}$ be the unique element such that $\ell=\lambda r$ in $\Ll_x$. Then $p(\ell)=\alpha\bmod \pi\in\Oo_{C,x}/\pi\Oo_{C,x}\simeq k$. Exactness can be easily checked on stalks, using the description of the stalks of $\Oo_C(-D)$ given in the proof of Fact~\reff{fact:OC(D)tensor}. 
	
	By additivity of the Euler--Poincaré characteristic (Claim~\reff{claim:chiAdditive}), 
	\begin{align*}
			\chi(C,\Ll(-x))=\chi(C,\Ll)-\chi(C,i_*\Oo_{\Spec k})\;.
	\end{align*}
...
\end{proof}
Our next preliminary step is to prove Theorem~\reff{thm:CurvesSerreDuality} with $C=\IP_k^1$. For Theorem~\reff{thm:CurvesSerreDuality}\itememph{b}, it suffices to provide an identification of $\Omega_C$ with $\Oo(-2)$ and to show that $c_1(\Oo(1))$ has a non-zero image in the one-dimensional $k$-vector space $H^1(C,\Oo(-2))$ (here we use Theorem~\reff{thm:CohoOfIP^n}\itememph{d}).\footnote{Where does $c_1(\Oo(1))$ come from? Well, we never stated this explicitly, but $\Pic(\IP_k^n)\simeq\IZ$ with $\Oo(1)$ a generator. How does one prove this? Put $R=k[X_0,\ldots,X_n]$ and let $\Ll$ be any line bundle on $\IP_k^n$. On the affine open subset $U_i=\IP_k^n\setminus V(X_i)$, $\Ll$ is given by $\snake{L}_i$ for some $(R_{X_i})_0$-module $L_i$. Giving a line bundle, $L_i$ is invertible and it can be shown that any invertible module over a UFD is trivial, cf.\ \cite[Tag~0BCH]{stacks-project}. Hence $L_i\simeq(R_{X_i})_0$. A careful (but not hard) inspectation of the gluing data shows that $\Ll$ must be isomorphic to some $\Oo(d)$.}
...
\begin{align*}
	...
	\end{cases}\;.
\end{align*}
All derivatives are considered as formal derivatives of rational functions in $X$ and $Y$. Since $f$ is a homogeneous rational function of degree $0$, the two terms for $df$ coincide on $U\setminus V(XY)$ (we leave it to the reader to push around $X$ and $Y$ until this comes out). On $\IP_k^1\setminus V(Y)\simeq \Spec k\left[\frac{X}{Y}\right]$ we have $f(X,Y)=\phi\left(\frac{X}{Y}\right)$ for some polynomial $\phi$, and $d(f)=\frac{1}{Y^2}\cdot \phi'\left(\frac{X}{Y}\right)$. Hence $d$ is a universal derivation there, and similar arguments apply to $\IP_k^1\setminus V(X)$. Thus $\Omega_{\IP_k^1}\simeq \Oo(-2)$, as claimed.

It remains to determine the image of $c_1(\Oo(1))$ under this identification followed by the isomorphism $H^1(\IP_k^1,\Oo(-2))\isomorphism k$ from Theorem~\reff{thm:CohoOfIP^n}\itememph{d}. Let $U_0=\IP_k^1\setminus V(X)$, $U_1=\IP_k^1\setminus V(Y)$, then $\Oo(1)$ is trivialized by $X$ on $U_0$ and by $Y$ on $U_1$. Let $\Uu\colon \IP_k^1=U_0\cup U_1$ denote this affine open cover. Reading carefully through the construction before Proposition~\reff{prop:torsorsH1Bij} again, we find that the element of $\check{H}_\alt^1(\Uu,\Oo_C^\times)$ which gives the $\Oo_C^\times$-torsor $\Oo(1)^\times$ is given by $\frac{X}{Y}\in\Oo_C^\times(U_{0,1})$. The image of $f=\frac{X}{Y}$ under $f\mapsto\frac{d(f)}{f}$ equals $\frac{1}{XY}\neq 0$, hence $c_1(\Oo(1))$ is mapped to a non-zero element.
