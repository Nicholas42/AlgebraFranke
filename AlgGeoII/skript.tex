...
\begin{cor}\lbl{cor:CohomologyOfPushforward}
	...
	\begin{alphanumerate}
		\item If $W\monomorphism[j]X$ is the embedding of the affine open subset $W$ and $\Mm\in\Ob(\cat{QCoh}(X))$, then $H^p(X,j_*\Mm)=0$ for $p>0$.
...
\begin{align}\lbl{eq:cohoPresheaf}
	\Phi^p\colon U\mapsto H^p\left(f^{-1}(U),\Mm\right)\tag{$*$}
\end{align}
...
\begin{align*}
	H^p\left(f^{-1}(U),\Mm\right)=\check{H}^p(\Uu,\Mm)\morphism\check{H}^p(\Vv,\Mm)=H^p\left(f^{-1}(\Vv),\Mm\right)\;,
\end{align*}
and this is the desired presheaf structure.

\begin{defi}
	Sheafifying $\Phi^p$ gives an $\Oo_Y$-module called the \defemph{$p\ordinalth$ direct image} of $\Mm$ under $f$ and denoted $R^pf_*\Mm$.
\end{defi}
 We have a morphism
 ...
 \begin{prop}
 	\begin{alphanumerate}
 		\item \lbl{prop:HigherDirectImage}$R^0f_*\Mm\simeq f_*\Mm$ canonically.
 ...
 \begin{align}\lbl{eq:RestrictionToFibres}
 	H^p(X,\Mm)\morphism R^pf_*\Mm=E_2^{0,p}\tag{\#}
 \end{align}
 ...
 \begin{align}\lbl{eq:pull-back}
 	E_2^{p,0}=H^p(Y,f_*\Mm)\morphism H^p(X,\Mm)\tag{$*$}
 \end{align}
...
\end{align*}
defining the above pull-back morphism \eqreff{eq:pull-back}. There is also an inverse image functor $f^*\colon \Oo_Y\cat{-Mod}\morphism \Oo_X\cat{-Mod}$ which preserves quasi-coherence and is left-adjoint to $f_*$. Thus, for $\Nn$ a quasi-coherent $\Oo_Y$-module, there is a canonical morphism $\Nn\morphism f_*f^*\Mm$ which gives another type of pull-back morphism
\begin{align*}
...
\begin{prop}
	Let $X$ be quasi-separated and $Y$ an arbitrary prescheme. For a morphism $X\morphism[f]Y$, the following conditions are equivalent.
	\begin{alphanumerate}
...
\end{prop}
\begin{proof}
	The implication \itememph{a} $\Rightarrow$ \itememph{b} is trivial and \itememph{b} $\Rightarrow$ \itememph{c} follows easily from Theorem~\reff{thm:CohoOnSchemes}\itememph{b} and Proposition~\reff{prop:HigherDirectImage}\itememph{b}.
	
	Now for \itememph{c} $\Rightarrow$ \itememph{a}. If $U\subseteq Y$ is affine and $\Mm\in\Ob(\cat{QCoh}(f^{-1}(U)))$, then $\Mm$ extends to $X$ (this will be checked in the next lecture) and by Proposition~\reff{prop:HigherDirectImage}\itememph{d}
	\begin{align*}
