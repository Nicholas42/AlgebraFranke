...
\section{The relation between \texorpdfstring{$H^1$}{H} and torseurs}
\begin{defi}
	Let $G$ be any topological space, $\Gg$ a sheaf of groups on $X$. A \defemph{$\Gg$-torseur} or \defemph{$\Gg$-principal homogeneous space} is a sheaf of sets $\Xx$ on $X$ with a morphism
	\begin{align*}
	...
	The torseur is called \defemph{trivial} if $\Xx(X)\neq \emptyset$ (and \emph{yes}, the $\neq$ is correct here).
\end{defi}
\begin{example}
	$\Gg$ with its group multiplication is a $\Gg$-torseur and trivial as $1_{\Gg(X)}\in\Gg(X)$.
\end{example}
\begin{rem}
	\begin{alphanumerate}
		...
		\item If $\Xx$ and $\Yy$ are $\Gg$-torseurs then any morphism $\Xx\morphism[f]\Yy$ of sheaves of sets compatible with the $\Gg$-actions induces isomorphisms on stalks.
	\end{alphanumerate}
\end{rem}
\begin{example}
	\begin{alphanumerate}
		\item Let $X$ be a ringed space. If $\Ll$ is a \emph{line bundle} on $X$ (i.e., a locally free $\Oo_X$-module of rank $1$) then
		\begin{align*}
			U\mapsto \Ll^\times(U)&=\left\{\lambda\in\Ll(U)\st \Oo_U\morphism[\cdot \lambda]\Ll|_U\text{ is an isomorphism}\right\}\\
			&=\left\{\lambda\in\Ll(U)\st 
			\begin{array}{c}
				\text{the image of }\lambda \text{ in }\Ll_x\text{ is a generator of}\\
				\text{that free }\Oo_{X,x}\text{-module for all }x\in U
			\end{array}
			\right\}
		\end{align*}
		is an $\Oo_X^\times$-torseur.
	\end{alphanumerate}
\end{example}
