...
\end{rem}
Now let $R$ be a ring, $S=R[X_0,\ldots,X_n]$ be equipped with the usual grading and put $\IP_R^n=\Proj R$. Then $\Uu\colon \IP_R^n=\bigcup_{i=0}^nU_i$ with $U_i=\IP_R^n\setminus V(X_i)$ is an affine open cover of $\IP_R^n$ and by \eqreff{eq:ProjLocal} we have
\begin{align*}
	U_{i_0,\ldots,i_\ell}=\IP_R^n\setminus V\left(X_{i_0}\cdots X_{i_\ell}\right)\simeq \Spec S[(X_{i_0}\cdots X_{i_\ell})^{-1}]_0\;.
\end{align*}
\begin{thm}\lbl{thm:CohoOfIP^n}
	Let $R$ be a ring, $S=R[X_0,\ldots,X_n]$ and $\Uu$ be the affine open cover of $X=\IP_R^n$ from above. 
	\begin{alphanumerate}
		\item  For $k\geq 0$ there is an isomorphism
		\begin{align}\lbl{eq:Oo(k)(IP^n)}
			\begin{split}
				S_k&\isomorphism \Oo(k)(X)\\
				s&\longmapsto \big(\text{image of }s\text{ in }(S_\pp)_k\big)_{\pp\in X}\;.
			\end{split}
		\end{align}
		For $k<0$ we have $\Oo(k)(X)=0$.
		\item For $0<p<n$ we have $H^p(X,\Oo(k))=0$ for all integers $k$.
		\item $H^n(X,\Oo(k))=0$ for $k>-n-1$ and $n>0$.
		\item There is an isomorphism $H^n(X,\Oo(-n-1))\isomorphism R$ defined (up to sign) by
		\begin{align}\lbl{eq:H^nIP^nO(-n-1)}
			\begin{split}
				\check{C}_\alt^n\big(\Uu,\Oo(-n-1)\big)\simeq S[(X_0\cdots X_n)^{-1}]_{-n-1}&\morphism R\\
				f=\sum_{\alpha\in\IZ^n}f_\alpha X_0^{\alpha_0}\cdots X_n^{\alpha_n}&\longmapsto f_{(-1,\ldots,-1)}\;.
			\end{split}
		\end{align}
		\item For all $k\in\IZ$, the groups $H^0(X,\Oo(k))$ and $H^n(X,\Oo(k))$ are finitely generated free $R$-modules and there is a non-degenerate pairing
		\begin{align}\lbl{eq:nonDegCohoPairing}
			\begin{split}
				H^0\big(X,\Oo(k)\big)\times H^n\big(X,\Oo(-n-1-k)\big)&\morphism H^n(X,\Oo(-n-1))\overset{\text{\itememph{d}}}{\simeq}R\\
				(f,[g])&\longmapsto [fg]\;,
			\end{split}
		\end{align}
...
\end{thm}
\begin{rem*}
	The pairing in \eqreff{eq:nonDegCohoPairing} is canonical but the isomorphism $H^n(X,\Oo(-n-1))\simeq R$ is not.
\end{rem*}
\begin{proof}[Proof of Theorem~\reff{thm:CohoOfIP^n}]
	Surprisingly, the proof will be almost trivial once we discover the connection between the \vC ech complex and the Koszul complex. By \eqreff{eq:O(k)isQC}, on $U_{i_0,\ldots,i_\ell}= \IP_R^n\setminus V(X_{i_0}\cdots X_{i_\ell})$ we have
	\begin{align*}
		\Oo(k)(U_{i_0,\ldots,i_\ell})\simeq S[(X_{i_0}\cdots X_{i_\ell})^{-1}]_k
	\end{align*}
	...
	\begin{align*}
		\Oo(U_{i_0,\ldots,i_\ell})\simeq S[(X_{i_0}\cdots X_{i_\ell})^{-1}]\;.
	\end{align*}
	Note that we have isomorphisms
	\begin{multline}\lbl{eq:CechKoszulIso1}
			S[(X_{i_0}\cdots X_{i_\ell})^{-1}]=\bigcup_{j=0}^\infty R[X_0,\ldots,X_n]\cdot (X_{i_0},\ldots,X_{i_\ell})^{-j}\\
			\lisomorphism \colimit \left(S\xrightarrow{\cdot(X_{i_0}\cdots X_{i_\ell})}S\xrightarrow{\cdot(X_{i_0}\cdots X_{i_\ell})}S\xrightarrow{\cdot(X_{i_0}\cdots X_{i_\ell})}\ldots\right)\tag{$*$}
	\end{multline}
	sending the image of $s\in S$ (as an element of the $j\ordinalth$ member of the sequence) in the colimit to $s\cdot (X_{i_0}\cdots X_{i_\ell})^{-j}$.
	
	For every $j\in\IN$ we get a morphism
	\begin{align*}
		\check{C}_\alt^\ell(\Uu,\Oo)\simeq \prod_{0\leq i_0<\cdots<i_\ell\leq n}S[(X_{i_0}\cdots X_{i_\ell})^{-1}]\lmorphism \bigwedge^{\ell+1}S^{n+1}\simeq K^{\ell+1}\big((X_0^j,\ldots,X_n^j),S\big)
	\end{align*}
	sending $e_{i_0}\wedge \cdots \wedge e_{i_\ell}$ to $(0,\ldots,(X_{i_0}\cdots X_{i_\ell})^{-j},\ldots,0)$ (with the only non-zero entry at position $(i_0,\ldots,i_\ell)$, obviously). Using $\bigwedge^{\ell+1}S^{n+1}\simeq \bigoplus _{0\leq i_0<\cdots <i_\ell\leq n}S$ and \eqreff{eq:CechKoszulIso1}, these morphisms assemble to an isomorphism
	\begin{align*}
		\check{C}_\alt^\ell(\Uu,\Oo)\lisomorphism \colimit[j\in \IN] K^{\ell+1}\big((X_0^j,\ldots,X_n^j),S\big)\;,
	\end{align*}
	where the transition morphisms $K^{\ell+1}\big((X_0^j,\ldots,X_n^j),S\big)\morphism K^{\ell+1}\big((X_0^{j+1},\ldots,X_n^{j+1}),S\big)$ (with respect to which the colimit is taken) sends an element $f\in K^{\ell+1}\big((X_0^j,\ldots,X_n^j),S\big)$ to $\phi\in K^{\ell+1}\big((X_0^{j+1},\ldots,X_n^{j+1}),S\big)$ defined by $\phi(t_1,\ldots,t_{\ell+1})=X_{t_1}\cdots X_{t_{\ell+1}}f(t_1,\ldots,t_{\ell+1})$. Convince yourself that this is compatible with the Koszul differential, so in fact we get a transition map $K^\bullet\big((X_0^j,\ldots,X_n^j),S\big)\morphism K^\bullet\big((X_0^{j+1},\ldots,X_n^{j+1}),S\big)$, and using this an isomorphism
	\begin{align}\lbl{eq:CechKoszulIso2}
		\left(S\morphism\check{C}_\alt^0(\Uu,\Oo)\morphism\check{C}_\alt^1(\Uu,\Oo)\morphism\ldots\right)\lisomorphism \colimit[j\in\IN]K^\bullet\big((X_0^j,\ldots,X_n^j),S\big)[-1]
	\end{align}
	of cochain complexes. The degree-$(-1)$ term $S$ comes from the fact that the $K^\bullet\big((X_0^j,\ldots,X_n^j),S\big)$ for all $j\in\IN$ have an equal term in degree $0$.
	
	Note that if $n=0$, then $\IP_R^0=\Proj R[X_0]\simeq \Spec R$ and everything follows from well-known properties of affine schemes. Now assume $n>0$. By Example~\reff{ex:(X0Xn)regular}\itememph{a}, the sequence $(X_0,\ldots,X_n)$ is a regular sequence in $S$. By Fact~\reff{fact:RegularSeqKoszulComplex}\itememph{d}, so are $(X_0^j,\ldots,X_n^j)$ for $j\in\IN$. We can now reduce the various assertions about the cohomology of $X=\IP_R^n$ to their Koszul counterparts.
	
	Part~\itememph{a}. By Fact~\reff{fact:RegularSeqKoszulComplex}\itememph{b}, $H^p\big((X_0^j,\ldots,X_n^j),S\big)=0$ in degree $p\neq n+1$. In particular, by exactness of filtered colimits equation\eqreff{eq:CechKoszulIso2} gives
	\begin{align*}
		H^0\left(S\morphism\check{C}_\alt^\bullet(\Uu,\Oo)\right)\lisomorphism\colimit[j\in\IN]H^1\big((X_0^j,\ldots,X_n^j),S\big)=0\;,
	\end{align*}
	...
	\begin{align*}
		H^p\left(S\morphism\check{C}_\alt^\bullet(\Uu,\Oo)\right)\lisomorphism\colimit[j\in\IN]H^{p+1}\big((X_0^j,\ldots,X_n^j),S\big)=0\;,
	\end{align*}
