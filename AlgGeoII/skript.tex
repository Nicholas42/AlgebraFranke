...
\begin{proof}
	...
	If $n=-1$, we have $\IP_k^{-1}=\Proj k=\emptyset$ and everything is trivial (whereas the $n=0$ case doesn't seem as obvious to me as Professor Franke made it look like). Now let $n\geq 0$ and the assertion be valid for $n-1$. Let $0\morphism \Mm'\morphism\Mm\morphism\Mm''\morphism 0$ be a short exact sequence of coherent $\Oo_X$-modules. We claim that
	\begin{align*}
...
\begin{rem}
	\begin{alphanumerate}
		...
		\begin{align*}
		\begin{array}{r c l c}
		f^\flat\colon & \cat{PreShv}(Y)\morphism[ ]\cat{PreShv}(X)&\text{left-adjoined to } & \cat{PreShv}(X)\morphism[f_*]\cat{PreShv}(Y)\\
		f^\sharp\colon & \cat{Shv}(Y)\morphism[ ]\cat{Shv}(X) & \text{left-adjoined to } & \cat{Shv}(X)\morphism[f_*]\cat{Shv}(Y)\\
		f^*\colon & \Oo_Y\cat{-Mod}\morphism[ ]\Oo_X\cat{-Mod} & \text{left-adjoined to } & \Oo_X\cat{-Mod}\morphism[f_*]\Oo_Y\cat{-Mod}
		\end{array}
		\end{align*}
		...
		\item Since $f^*\Oo_Y=\Oo_X$ (this is clear on stalks) and $f^*$ preserves direct sums (in fact, as a left-adjoined functor it preserves arbitrary colimits, cf.\ Remark~\reff{rem:AdjunctionPreservesStuff}), so inverse images of vector bundles (resp.\ line bundles) are vector bundles (resp.\ line bundles) again by base-locality of $f^*$.
...
Let $X$ be a quasi-compact one-dimensional regular (and thus locally Noetherian since \cite[Definition~2.4.5]{alggeo1} assumes this implicitly; hence even Noetherian by quasi-compactness) integral scheme, $X_1$ the set of codimension $1$ (i.e., closed) points, $\Div(X)$ the free abelian group generated by $X_1$ which we already encountered \hyperref[par:Divisors]{here}. For $D\in\Div(X)$, the line bundle $\Oo_X(D)$ is defined by
\begin{align*}
	\Oo_X(D)(U)=\left\{f\in K\st v_x(f)\geq D(x)\ \forall x\in U\cap X_1\right\}
\end{align*}
when $U\neq \emptyset$ (and $\Oo_X(\emptyset)=0$ of course). Here, $K=\Oo_{X,\eta}$ denotes the local ring at the generic point $\eta$ of $X$ (which is a field) and $v_x\colon \Oo_{X,x}\morphism\IZ$ for $x\in X_1$ denotes the valuation of the DVR $\Oo_{X,x}$. 
...
Let $k$ be algebraically closed. All schemes to appear are supposed to be $\Spec k$-schemes and $\Omega_X^1\coloneqq\Omega_{X/k}$.
\begin{thm}\lbl{thm:CurvesSerreDuality}
	...
	\begin{alphanumerate}
		...
		\begin{align*}
			\deg \Oo_X(D)\eqqcolon\deg D=\sum_{x\in C_1}D(x)\;.
		\end{align*}
		\item $H^1(C,\Omega_C^1)$ is a one-dimensional $k$-vector space and there is a unique isomorphism $\tau\colon H^1(C,\Omega_C^1)\isomorphism k$ such that the diagram
		\begin{diagram*}
			\node[ob] (a) at (0,1.5) {$\Pic(C)$};
			\node[ob] (b) at (3,1.5) {$H^1(C,\Omega_C^1)$};
			\node[ob] (c) at (3,0) {$k$};
			\node[ob] (d) at (0,0) {$\IZ$};
			\scriptsize
		...
		\begin{align*}
			H^0(C,\Vv)\times H^1(C,\Omega_C^1\otimes_{\Oo_C}\Vv^*)\morphism[m]H^1(C,\Omega_C^1)\isomorphism k
		\end{align*}
		of finite-dimensional vector spaces. Here, $\Vv^*$ denotes the \defemph{dual bundle} $\Vv^*=\Hhom_{\Oo_C}(\Vv,\Oo_C)$ and the pairing $m$ is given by $(v,\ell\otimes\omega)\mapsto \ell(v)\cdot \omega$.
	\end{alphanumerate}
\end{thm}
\begin{rem*}
	\begin{alphanumerate}
		\item Theorem~\reff{thm:CurvesSerreDuality}\itememph{b} and \itememph{c} are called \emph{Serre duality}. They still hold when $C$ is replaced by a \emph{smooth} scheme $X$ over an arbitrary field $k$ and $\Omega_C^1$ by $\omega_X=\bigwedge^{\dim X}\Omega_X^1$.
