...
\end{proof}
\begin{rem*}
	As the proof shows, \itememph{b} $\Rightarrow$ \itememph{c} $\Rightarrow$ \itememph{d} already holds when $I$ is fixed.
\end{rem*}
...
\end{proof}
\begin{defi}
	The largest natural number $d$ such that the equivalent properties from Fact~\reff{fact:flatDim} fail (or $+\infty$ if no such $d$ exists, or $-\infty$, if $M=0$) is called the \defemph{flat dimension} $\fdim_R(M)$ of $M$.
\end{defi}
...
\end{fact}
\begin{proof}
	When $\pdim_R(M)=\ell$, then $M$ has a projective resolution of length $\ell$ by Fact~\reff{fact:projDim}. Using this resolution to compute $\Tor_\bullet^R(M,-)$ we see that $\Tor_p^R(M,-)=0$ for $p>d$, hence $\fdim_R(M)\leq \ell$. This shows \itememph{a}.
...
\end{cor}
\begin{proof}
	By Proposition~\reff{prop:TorMN=TorNM} and Fact~\reff{fact:flatDim}\itememph{d}, $\fdim_R(k)$ is smallest integer $d$ such that $0=\Tor_1^R(k,M)\simeq \Tor_1^R(M,k)$ for all finitely generated $R$-modules $M$. Then $\fdim_R(M)\leq \fdim_R(k)$ follows immediately from Corollary~\reff{cor:pdim=fdim}. If $M$ is arbitrary and $p>\fdim_R(k)$, we thus have $p>\fdim_R(R/I)$ as well for all ideals $I\subseteq R$. Therefore $\Tor_p^R(M,R/I)\simeq \Tor_1^R(R/I,M)=0$. Fact~\reff{fact:flatDim}\itememph{e} then shows $\fdim_R(M)<p$.
\end{proof}
\begin{cor}
...
\end{prop}
\begin{proof}
	By Corollary~\reff{cor:projDimLocal}, $M$ is projective iff $M_\mm$ is projective for all maximal ideals $\mm$ of $R$ iff $M_\pp$ is projective for all $\pp\in\Spec R$. The same holds for flatness by Fact~\reff{fact:flatDimLocal}\itememph{b} and \itememph{c}. By Proposisition~\reff{prop:FreeProjFlatTor}, we see that \itememph{a}, \itememph{b},\itememph{e}, and \itememph{f} are equivalent. Equivalence of \itememph{c} and \itememph{d} is pretty much the definition of vector bundles (using the fact that the $\Spec R_f\simeq \Spec R\setminus V(f)$ form a topology base of $\Spec R$ and \cite[Proposition~1.4.1]{alggeo1}). The implication \itememph{c} $\Rightarrow$ \itememph{f} is trivial and \itememph{f} $\Rightarrow$ \itememph{c} is a Nakayama argument that can be found in \cite[Corollary~1.5.1]{alg2}.
\end{proof}
