...
\begin{proof}
	...
	\begin{align*}
		f_j\left(g_j-\sum_{i=1}^k\rho_{i,j}m_i\right)=0\;.
	\end{align*}
...
As in \cite[Proposition~1.3.1]{alg2} one shows $\dim_k(\mm/\mm^2)\geq d$. Recall that $R$ is called \defemph{regular} when equality holds. In this case, $\mm$ can be generated by $d$ elements $\mu_1,\ldots,\mu_d$ and we have seen in \cite[Corollary~3.4.9]{alg2} that the $\mu_i$ generate the \emph{associated graded ring}
\begin{align*}
	\gr(R,\mm)=\bigoplus_{i=0}^n\mm^i/\mm^{i+1}
\end{align*}
as a polynomial algebra over $k$.
