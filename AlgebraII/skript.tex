...
\begin{proof}[Proof of Theorem \reff{thm:codimIneqRegular}]
    ...
    \begin{align*}
    	\codim(C,Y\times Z)=\codim(C',Y'\times Z')\leqslant d\;.
    \end{align*}
...
\end{rem}
\begin{example}
    A $d\in\Der_R(R[X_1,\ldots,X_n], M)$ is uniquely determined by $(m_i)^n_{i=1}\in M^n$ by
    \begin{align*}
        d&\longmapsto (d(X_i))_{i=1}^n\\
        \left(P\mapsto \sum_{i=1}^n\frac{\partial P}{\partial X_i}\cdot m_i \right) &\longmapsfrom (m_i)_{i=1}^n.
    \end{align*}
    Note that the left hand side is indeed a derivation:
    \begin{align*}
        d(PQ) = \sum_{i=1}^n \frac{\partial(PQ)}{\partial X_i} \cdot m_i = \sum_{i=1}^n \left( P \cdot\frac{\partial Q}{\partial X_i} + \frac{\partial P}{\partial X_i} \cdot Q \right) m_i = P\cdot d(Q) + d(P)\cdot Q
    \end{align*}
    It is easy to see that the two maps are inverse to each other.
\end{example}
\begin{rem*}
    If $M\morphism[\mu] N$ is a homomorphism of $A$-modules then 
    \begin{align*}
        \Der(A,M) &\longto \Der(A,N)\\
        d &\longmapsto \mu \circ d
    \end{align*}
    is a morphism of $A$-modules. This also applies to the same morphism on the $R$-linear derivations. 
    
    The previous example can be formulated as saying that
    \begin{align*}
        d\colon A = R[X_1,\ldots, X_n] &\longto A^n\\
        P &\longmapsto \left(\frac{\partial P}{\partial X_i}\right)_{i=1}^n
    \end{align*}
    is the universal $R$-linear derivation of $A$: Any $\delta \in \Der_R(A,M)$ can be uniquely expressed as $\delta = \mu \circ d$ where $A^n \morphism[\mu] M$ is a uniquely determined $A$-linear homomorphism.
\end{rem*}
\begin{defi}[Kähler differentials] \lbl{def:KahlerDiff}
    Let $A$ be an $R$-algebra. A \defemph{module of Kähler differentials} for $A/R$ is an $A$-module $\Omega_{A/R}$ together with $d_{A/R} \in \Der_R(A,\Omega_{A/R})$ satisfying the following universal property: For any $A$-module $M$ and any $\delta \in \Der_R(A,M)$ there is a unique $A$-homomorphism $\Omega_{A/R}\morphism[\epsilon] M$ such that $\delta = \epsilon \circ d_{A/R}$.
\end{defi}
\begin{rem*}
    \begin{alphanumerate}
        \item 
            The universal property characterizes $\Omega_{A/R}$ up to unique isomorphism (if it exists): If $\snake d_{A/R} \in\Der_R(A,\snake \Omega_{A/R})$ has the same universal property there is a unique iso $\Omega_{A/R}\isomorphism[\iota] \snake \Omega_{A/R}$ such that $\snake d _{A/R} = \iota \circ d_{A/R}$. In fact, the universal property of $d_{A/R}$ shows the existence and uniqueness of a homomorphism $\iota$ with this property. Reversing the roles of $d_{A/R}$ and $\snake d_{A/R}$ we also obtain $\snake\Omega_{A/R} \morphism[j]\Omega_{A/R}$ such that $d_{A/R} = j\circ \snake d_{A/R}$. Then $a = j\circ\iota$ satisfies $d_{A/R} = a\circ d_{A/R}$ which implies $a= \id_{Q_{A/R}}$ by the universal property of $\Omega_{A/R}$. Exchanging $d_{A/R}$ and $\snake d_{A/R}$ gives $\iota \circ j = \id_{\snake\Omega_{A/R}}$. Thus $\iota$ is an isomorphism.
        \item
        ...
        \item 
            If $A$ is a quotient of $R$ ($R\morphism A$ is surjective) then $\Der_R(A,M) = 0$ for all $M$, hence $\Omega_{A/R} = 0$.
    \end{alphanumerate}
\end{rem*}
\begin{defi}[Free Module] \lbl{def:freeModule}
    The free $A$-module $F$ with generating set $M$ is the module of functions $f: M \morphism A$ with finite support. We define $\delta_x \in F$ by $\delta_x(y) = \delta_{x,y}$.
\end{defi}

\begin{rem*}
    \begin{alphanumerate}
        \item  
            One often thinks of $F$ as the module of formal (finite) $A$-linear combinations of $M$ with $f(x)= \sum_{m\in M} f(m) x$ instead of $f= \sum_{x\in M} = \delta_x$.
        \item 
            We have a bijection, for any $A$-module $N$,
            \begin{align*}
                \Hom_{\cat{Set}}(M,N) &\isomorphism \Hom_A(F,N)\\
                \upsilon &\longmapsto \left(f\mapsto \sum_{m\in M} f(m) \upsilon(m)\right)\\
                \left(m\mapsto \phi(\delta_m) \right) &\longmapsfrom \phi.
            \end{align*}
            In other words, mapping $X$ to the free $A$-module generated by $X$ as a functor from $\cat{Set}$ to the category of $A$-modules is left-adjoint to the forgetful functor from the category of $A$-modules to $\cat{Set}$.
    \end{alphanumerate}
\end{rem*}
...
\begin{proof}
    ...
    Let $F_A$ be the free $A$-module generated by the set $A$ and let $K\subseteq F_A$ the submodule generated by the following three types of elements:
    \begin{alphanumerate}
        \item 
            $\left\{\delta_a+\delta_b-\delta_{a+b}\st a,b\in \right\}$
        \item 
    ...
    Let $(A\morphism[d] M)\in \Der_R(A,M)$. By the universal property of $F_A$ there is a unique $c\in \Hom_A(F_A,M)$ such that $d(a) = c(\delta_a)$. We claim that $c$ vanishes on $K$.
    \begin{itemize}
      \item 
    ...
    \end{itemize}
    Consequently, there is a unique $\delta\in \Hom_A(F_A/K , M)$ such that $\delta \circ\pi = c$. Then $d(a) = c(\delta_a) = \delta(\pi(\delta_a)) = \delta(d_{A/R}(a))$. Conversely let $\snake \delta\in \Hom_A(F_A/K,M)$ such that $\snake \delta_{A/R} = d$. Then $(\snake \delta\circ\pi)(\delta_a) = \snake\delta(\pi(\delta_a)) = \snake\delta(d_{A/R}(a)) = d(a)$. and we obtain $\snake\delta \circ \pi = c = \delta\circ \pi$ as $c$ is (in view of the uniqueness property of the universal property of the free module) uniquely determined by $d(a) = c(\delta_a)$. As $\pi$ is surjective, this implies $\snake\delta = \delta$.
\end{proof}
...
\begin{fact}
    ...
    \begin{alphanumerate}
        \item 
            Let $I\subseteq A$ be any ideal, $M$ any $A$-module, $A\morphism[\pi]A/I$ the projection, then 
            \begin{align*}
                0\to \Der_R(A/I,M) &\to \Der_R(A,M) &\to \Hom_{A/I}(I^2/I, M)\\
                d &\mapsto d\circ \pi\\
                & \delta &\mapsto (i\mod I^2\mapsto \delta(i))
            \end{align*}
            is exact.
    \end{alphanumerate}
\end{fact}
