...
\begin{proof}[Proof of Proposition~\reff{prop:Akizuki-Hopkins}]
    ...
    The proof will be completed in the next lecture. Which is now.   
    ...
    We want to show that, if $I\subseteq J$ are ideals of $R$ and $I/J$ is annihilated by one of the $\pp_i$, then $\ell(I/J)$ is finite. Otherwise this quotient would be a vector space of infinite dimension over $R/\pp_i$, hence has an infinite descending chain of subvector spaces whose preimages under $I\morphism I/J$ form an infinite descending chain of ideals in $R$, contradicting \itememph{c}. From this follows that $\nil(R)^i/\nil(R)^{i+1}$ has finite length, by applying this to the filtration quotients of the filtration
    \begin{align*}
        \left(\left(\left(\bigcap_{k=1}^j\pp_k\right)\nil(R)^i\right) /\nil(R)^{i+1}\right)_{j=0}^r\;.
    \end{align*}
    
    This finishes our proof by showing $\ell(R) <\infty$.
\end{proof}
...
\begin{thm}\lbl{thm:hilbertPolynomial}
    Let $R_\bullet$ be an $\IN$-graded ring satisfying the following equivalent conditions:
    \begin{alphanumerate}
    \item 
        $R_\bullet$ is Noetherian and $R_0$ is Artinian and $R_+$ is generated by $R_1$ as an ideal.
    \item 
        $R_0$ is Artinian, $R$ is generated by $R_0$ and $R_1$ and $R_1$ us a finitely generated $R_0$-module.
    \end{alphanumerate}
    Let $M$ be a finitely generated graded $R$-module. Then there are a polynomial $P_M\in \IQ[T]$ and $k_0\in\IZ$ such that $\ell(M_k) = P_M(k)$ for all $k\leq k_0$. This polynomial is uniquely determined and if $R_1$ may be generated by $d$ elements as an $R_0$-module then its degree is less than $d$.
\end{thm}

\begin{rem*}
    We use the convention that the polynomial 0 has degree $-\infty$. When $d=0$, the condition $\deg(P_M) <d$ is thus equivalent to $P_M = 0$.
\end{rem*}
...
\begin{defi}[Hilbert polynomial of projective algebraic variety] \lbl{def:hilbertPolynomialProjVar}
    If $M=R=k[X_0,\ldots, X_n]/\pp$ is the graded coordinate ring of a projective algebraic variety $Z=V(\pp)$ the polynomial $P_M$ is called the Hilbert polynomial of $Z$.
\end{defi}
\begin{proof}[Proof of Theorem~\reff{thm:hilbertPolynomial}]
    It is easy to see that \itememph{a} and \itememph{b} are equivalent and that $P_M$ is unique. For existence we use induction on $d$.
    
    If $d=0$, $R_\bullet=R_0$. As $M$ is finitely generated, $M_k = 0$ when $k\geq k_0$ and $P_M =0$. When $0\morphism M'\morphism M\morphism M'' \morphism 0$ is a short exact sequence of $R$-modules and the assertion is valid for $M'$ and $M''$, then it is valid for $M$ and $P_M = P_{M'} +P_{M''}$, since we have $\ell(M_k) = \ell(M'_k)+\ell(M''_k) $ for $k\gge 0$, and the sum has degree smaller than $d$ if both summands are of degree smaller than $d$. If the assertion is valid for $M$ then it is valid for $M[\ell]$ (with $\ell\in\IZ$) and $P_{M[\ell]}(-) = P_M(-+\ell)$. 
    
    Let $d>0$ and the assertion be valid if $R_1$ may be generated by fewer than $d$ elements, as an $R_0$-module. By Proposition~\reff{prop:filtrationByPrimeIdeals} and by the previous steps, we may assume $M=R/\pp$ where $\pp$ is a homogeneous prime ideal. Let $X=X_1,\ldots, X_d$ generate $R_1$ as an $R_0$-module. If $X\in\pp$ then $M$ is a module over $R/X\cdot R$ to which the induction assumption may be applied as $(R/X\cdot R)_1\simeq R_1/X\cdot R_0$ is generated over $R_0$ by the images of $X_2,\ldots, X_d$. Otherwise we have a short exact sequence 
    \begin{align*}
        0 \morphism M \morphism[X\cdot] M \morphism M/X\cdot M \morphism 0\\
        0 \morphism M_{k-1}\morphism[X\cdot] M_k \morphism M_k/X\cdot M_{k-1} \morphism 0
    \end{align*}
    and the induction assumption may be applied to $M/X\cdot M$ (with grading $(M/X\cdot M)_k = M_k/X\cdot M_{k-1}$) which is a module over $R/X\cdot R$ when $k$ is large enough, where $P_{M/X\cdot M}$ has degree smaller than $d-1$. The assertion thus follows from the elementary fact:
\end{proof}
\begin{fact}
    \begin{alphanumerate}
    \item 
        If $f\colon \IZ\morphism \IQ$ is a function such that there is a polynomial $P$ of degree smaller than $e$ such that $f(k) -f(k-1) = P(k)$ when $k$ is sufficiently large, where $e\geq 0$, then there exists a polynomial $Q\in\IQ[T]$ such that $f(k) = Q(k)$ for sufficiently large $k$. 
    \item 
        If $P\neq 0$ then $\deg Q = \deg P +1$. If $\deg P = \delta$, then we have $(\delta+1) \cdot q_{\delta+1} = p_\delta$ for the leading coefficients. 
    \end{alphanumerate}
\end{fact}
\begin{proof}
    To prove \item{a}, we use induction on $e$. If $e=0$ then $P=0$ and $f(k+1) = f(k)$ for sufficiently large $k$ and $Q$ can be chosen as constant. If $e>0$ and $P$ has degree $e-1$ and $p_{e-1}$ denotes its leading coefficient, then the induction assumption may be applied to $g(k) = f(k) -\frac{p_{e-1}}{e}k^e$:
    \begin{align*}
        g(k) -g(k-1) = f(k) -f(k-1) - \frac{p_{e-1}}{e}(k^e -(k-1)^e) = P(k) - \frac{p_{e-1}}{e}e\cdot k^{e-1} + L
    \end{align*}
    where $L$ is a $\IQ$-linear combination of $k^0,\ldots, k^{e-2}$, and the induction assumption may be applied to $g$ as the leading coefficients cancel. By the induction assumption, $g(k) = R(k)$ when $k\gge 0$ where $R\in \IQ[T]$ has degree smaller or equal to $e-1$. Thus $f(k) = R(k) + \frac{p_{e-1}}{e}k^e$ which is a polynomial of degree $e$ and leading coefficient $\frac{p_{e-1}}{e}$ as claimed by \itememph{b}.
\end{proof}
\begin{fact}
...
\end{fact}
\begin{proof}
    If $r\in \bigcap_{i=1}^k\pp_i$ then $r(M_i/M_{i-1}) = 0$ hence $r\cdot M_i\subseteq M_{i-1}$ and $r^k\cdot M =0$, hence $r^k\in \Ann(M)$. On the other side, if $r\not\in \pp_i$, and $m\in M_i\setminus M_{i-1}$ then $r^\ell\cdot m \neq 0$ for arbitrary $\ell$ as the image of $m$ in $M_i/M_{i-1} \simeq R/\pp_i$ is non-zero and $r\not\in \pp_i$ is not a zero divisor in $R/\pp_i$. Thus, $r\not\in \sqrt{\Ann(M)}$.
\end{proof}
\begin{prop}
    \begin{alphanumerate}
    \item 
        If $Z=V(\pp)$ (where $\pp\subseteq k[X_0,\ldots, X_n]$ is a homogeneous prime ideal) is a projective algebraic variety, then the degree then the degree of its Hilbert polynomial equals $\dim(Z)$.
    \item 
        If $R=k[X_0,\ldots, X_n]$ and $M$ is a finitely generated graded $R$-module with a filtration $0=M_0\subsetneq M_1\subsetneq\ldots\subsetneq M_k$ with $M_i/M_{i-1}\simeq R/\pp_i[t_i]$ as in Proposition~\reff{prop:filtrationByPrimeIdeals}, then the degree of the Hilbert polynomial $P_m$ equals
        \begin{align*}
            \max\left\{\dim(V(\pp_i))\st 1\leq 1\leq k\right\}
        \end{align*}
        where the empty maximum and the dimension of $\emptyset$ are take to be $-\infty$.
    \end{alphanumerate}
\end{prop}
\begin{rem*}
    The Hilbert polynomial is easily seen to be additive: $P_M \sum_{i=1}^k P_{M_i/M_{i-1}}$. Moreover, the leading coefficient is positive such that $\deg\left(\sum_{i=1}^k P_{M_i/M_{i-1}}\right) = \max\left\{\deg P_{M_i/M_{i-1}}\st 1\leq i\leq k\right\}$.
\end{rem*}
\begin{proof}
    It follows \itememph{a} from \itememph{b} by applying it to $M=R/\pp$. Without losing generality, we may assume that $M=R/\pp$. When $n=0$, $\pp=R_+$ (in which case $P_M=0$, $\dim(V(\pp)) = -\infty$) or $\pp=0$ ($P_M=1$, $\dim(V(\pp)) = 0$). Thus, the assertion is true in this case. Induction argument on $n$.
\end{proof}
