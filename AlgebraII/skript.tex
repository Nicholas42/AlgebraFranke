...
\begin{proof}[Proof of Proposition~\reff{prop:HilbertDegreeEqualsDimension}]
    The previous remark shows that it suffices to show \itememph{a}, restrict to the case $M=R/\pp$. We use indicztion on $n$. When $n=0$, $\pp=R_+$ (in which case $P_M=0$ and $\dim(V(\pp)) = -\infty$) or $\pp=0$ (in which case $P_M=1$ and $\dim(V(\pp)) = 0$). Thus, the assertion is true in this case.
\end{proof}
