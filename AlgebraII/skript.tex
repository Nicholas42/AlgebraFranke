...
\begin{fact}
    The set $\supp(M)$ is Zariski-closed. Let $\supp(M) = \bigcup_{i=1}^\ell V(\qq_i)$ be its decomposition into irreducible components (cf. Proposition~\reff{prop:irreducibleDecomposition}\itememph{b}) and $Q(M) = \left\{\qq_1,\ldots,\qq_\ell\right\}$. Then 
    \begin{align}
    ...
    \end{align}
    For any filtration like \eqreff{eq:filtrationByPrimeIdeals}, $Q(M)$ is equal to the set of $\subseteq$-minimal elements of $\left\{\pp_1,\ldots,\pp_k\right\}$.
\end{fact}
\begin{proof}
    ...
    \begin{align*}
    	\sqrt{\Ann_R(M)}  =\bigcap_{i=1}^k \pp_k = \bigcap_{\qq\in Q} \qq\;,
    \end{align*}
    where $Q$ is the set of $\subseteq$-minimal elements of $\left\{\pp_1,\ldots,\pp_k\right\}$. Hence $\supp M=\bigcup_{\qq\in Q} V(\qq)$ where no member of this union contains another, hence it is the decomposition into irreducible components and $Q= Q(M)$ and \eqreff{eq:sqrtAnnCap} follows. 
...
\end{rem*}
\begin{fact}
    If $x\in R\setminus\bigcap_{\qq\in Q(M)}\qq$, then $\dim(M/x M) <\dim(M)$, unless $\dim(M) =\infty$.
\end{fact}
\begin{proof}
    We have $\Ann_R(M/x M) \supseteq \Ann_R(M)$, hence $\supp M/x M \subseteq \supp M$ and therefore $\dim(M/x M) \leq \dim(M)$. Moreover, if $\Ann(M/x M) \subseteq \qq_0 \subsetneq \ldots \subsetneq \qq_\ell$ is any chain of prime ideals, then $\qq_0 \in \supp(M/x M)\subseteq \supp M$, hence $\qq_0\in V(\qq)$ for some $\qq\in Q(M)$ but $\qq\subsetneq \qq_0$ as $x\in \qq_0$ but $x\not\in \qq$ by our assumption, hence $\dim(M)\geq \ell+1$.
\end{proof}
...
\begin{fact}\lbl{fact:samuelPolynomial}
    If $I\subseteq R$ is any ideal of definition there is a unique polynomial $Q= Q(M,I)$ such that $\ell_R(M/I^kM) = Q(k)$ for any large enough $k\in \IN$.
\end{fact}
...
\begin{defi}[Samuel polynomial]\lbl{def:samuelPolynomial}
    This polynomial $Q=Q(M,I)$ is called the \defemph{Samuel polynomial} of $M$ with respect to $I$ and its degree is denoted by $d(M,I)$. Moreover, let 
    \begin{align*}
...
\end{proof}
And finally, we are able to prove Theorem~\reff{thm:krullIdeal} (and thus also the principal ideal theorem, Theorem~\reff{thm:krullPrincipalIdeal}) at last!
...
\begin{prop}[Artin-Rees] \lbl{prop:artinRees}
    If $R$ is an Noetherian ring, $I\subseteq R$ an ideal, $N\subseteq M$ a submodule of a finitely generated $R$-module $M$, then there exists a $d\in\IN$ such that $N\cap I^{k+d} \subseteq M \subseteq I^kN$. Here $d$ should only depend on $R$, $I$, $M$ and $N$.
\end{prop}
\begin{proof}
    Let 
    \begin{align*}
        A = \left\{\sum_{i=0}^\infty a_kT^k\in R[T] \st a_k I^k\right\}
    \end{align*}
    be the \emph{Rees-algebra} (or \emph{blowup-algebra}). One may note, that due to $A$ being a subset of the polynomial ring, in each element, all but finitely many $a_k$ have to be $0$. This is an $R$-algebra of finite type, hence Noetherian. If $I$ is generated by $(i_\ell)_{\ell=1}^L$ then $A$ is generated over $R$ by $\left\{ i_\ell T \st 1\leq \ell \leq L\right\}$. The module 
    \begin{align*}
        \snake M = \left\{\sum_{i=0}^\infty \mu_iT^i\in M[T] \st \mu_j \in I^jM\right\}
    \end{align*}
    is finitely generated (by $(m_k T^0)_{k=1}^K$ when the $m_k$ generate $M$ as an $R$-module) as an $A$-module. As $A$ is Noetherian, $\widehat N = (N[T])\cap \snake M \subseteq \snake M$ is also finitely generated. Let $(n_\lambda T^{c_\lambda})_{\lambda = 1}^\Lambda$ be generators of $\widehat N$ over $A$. Then the assertion holds for $d = \max\left\{c_\lambda\st 1\leq \lambda \leq \Lambda\right\}$. 
    
    The set of homogeneous elements of degree $k+d$ of $\widehat N$ equals $((I^{k+d}M)\cap N)\cdot T^{k+d}$ and its elements must be of the form
    \begin{align*}
        \sum_{\lambda = 1}^\Lambda (a_\lambda T^{k+d-c_\lambda})(n_\lambda T^{c_\lambda})
    \end{align*}
    where $a_\lambda \in I^{k+d-c_\lambda} \subseteq I^k$. The result follows as $a_\lambda n_\lambda \in I^k N$.
\end{proof}

\begin{cor}[This is not Corollary~\reff{cor:dimRMaxIdeal}]\lbl{cor:shortExactd}
    Let $0\morphism M'\morphism M \morphism M''\morphism 0$ be short exact sequence of finitely generated modules over a Noetherian local ring $R$ in which some ideal of definition has been fixed. Then $d(M) = \max(d(M'), d(M''))$ and if the Samuel polynomials of $M'$ and $M$ have the same degrees and leading coefficients (e.g. because they are isomorphic), then $d(M'') < d(M)$.
\end{cor}
\begin{proof}
    We have 
    \begin{align*}
        0 &\morphism M'/(I^k\cap M') \morphism M/I^kM \morphism M''/I^kM'' \morphism 0\\
        0 &\morphism I^kM +M'/I^kM \nearrow
    \end{align*}
    by an easy diagram chase, hence $\ell(M/I^kM) = \ell(M''/I^kM'') + \ell(M'/(I^kM\cap M'))$ and an application of \hyperref[prop:artinRees]{Artin-Rees-Lemma} gives the 
    
    hence $Q_{M'}(k-c) \leq \ell(M'/I^kM\cap M') \leq Q_{M'}(k)$ hence
    \begin{align*}
        Q_{M'}(k-c) + Q_{M''}(k) \leq Q_M(k) \leq Q_{M'}(k) + Q_{M''}(k)
    \end{align*}
    and if $d=\max(d(M'), d(M''))$ it follows that $Q_M(k) \in \Oo(k^d)$ and also $(k-c)^d \gge Q_M$ proving $\deg(Q_M) = d$. If $Q_M$ and $Q_{M'}$ have the same degree and leading coefficient $\gamma$, then $Q_{M'}(t-c), Q_{M'}(t)$ as wall as everything in between and also $Q_M(t)$ is in $\gamma k^d +\Oo(k^{d-1})$ hence $Q_{M''}(k) \in \Oo(k^{d-1})$ as $k\to \infty$ hence the degree of $Q_{M'}$ is smaller or equal to $d-1$.
\end{proof}
\begin{cor}[This also not Corollary~\reff{cor:dimGeneratorsDefinition}]
    Appyling this to \eqreff{eq:filtrationByPrimeIdeals} we obtain 
    \begin{align*}
        d(M) = \max\left\{d(R/\qq) \st \qq\in Q_M\right\}\;.
    \end{align*}
\end{cor}
\begin{proof}
    ...
    \begin{align*}
        \max\left\{d(R/\pp_i\st 1 \leq i \leq k\right\}
    \end{align*}
    (as in \eqreff{eq:filtrationByPrimeIdeals}) but the $\qq\in Q_M$ are the minimal elements of $\left\{\pp_i \st 1\leq i\leq k\right\}$ and if $\pp_i\supseteq \qq$ we have $R/\qq \epimorphism R/\pp_i$, hence $\ell(R/I^k+\qq) \geq \ell(R/I^k +\pp_i)$ hence $d(R/\pp_i) \leq \deg(R/\qq)$.
\end{proof}
\begin{cor}[Krull, not Krull's height theorem as in Corollary~\reff{cor:KrullHeight}, but still Krull (who, as we all know, was a \emph{n00b} compared to Grothendieck)]
    If $R$ is a local Noetherian ring with maximal ideal $\mm$, then 
    \begin{align*}
        \bigcap_{k=0}^\infty \mm^k = \{0\}\;.
    \end{align*}
\end{cor}
\begin{proof}
    Let $M=R$, $N= \bigcap_{k=0}^\infty \mm^k$ then $\mm N \supseteq (\mm^{d+1} M) \cap N =  \mm^{d+1}\cap N = N$ by Proposition~\reff{prop:artinRees} and $N=0$ by \NAK.
\end{proof}
\begin{proof}[Proof of Theorem~\reff{thm:idealOfDefinition}]
    First we show $\dim(M) \leq d(M)$ by induction on $d(M)$. When $d(M) = 0$ the lengths of $M/I^kM$ are bounded by some constant $C$ independent of $k$, hence $\ell(M) \leq C$. Hence $\ell(R/\pp_i) \leq C$ in \eqreff{eq:filtrationByPrimeIdeals} hence all $\pp_i=mm$ by Proposition~\reff{prop:Akizuki-Hopkins}. Therefore $\dim(M) = \max\left\{\dim(R/\pp_i)\st 1\leq i\leq k\right\} = 0$. Let $d(M)>0$ and let the assertion be proved for all $N$ with $d(N) < d(M)$. It is sufficient to prove it for all modules $R/\pp$ with the same $d$ because by Corollary~\reff{cor:shortExactd} it is inherited by $M$ from the $R/\pp_i$ occuring in \eqreff{eq:filtrationByPrimeIdeals}. Let $\pp_i \subseteq \qq_0 \subsetneq \qq_1 \subsetneq \ldots \subsetneq \qq_\delta$ a chain of prime ideals contained in $\supp(M)$, then we may select $x\in \qq_1\setminus \qq_0 \subseteq \qq_1\setminus \pp_i$. Then $x$ is no zero-divisor in $M$ and $\qq_1\subsetneq \ldots \subsetneq \qq_\delta$ are still contained in $\supp(M/x M)$ hence $\dim(M/xM) \geq \delta -1$ but $0\morphism M\morphism[x] \morphism M/xM$ and Corollary~\reff{cor:shortExactd} show $d(M/xM) <d(M)$ and $\delta-1 < d(M)$ or $\delta \leq d(M)$.
    
    \begin{cor}[YES, a Corollary in a proof, deal with it, I also have to]
        The finite-dimensionality of $R$ now been established.
    \end{cor}
    
    We now show $d\leq \delta$ (this is not the delta from the last step, if you wondered). If $\delta(M) = 0$, $\ell(M) < \infty$ and $Q_M(k) \leq \ell(M)$ for sufficiently large $k$, hence $Q_M$ is constant and $d=0$. Let $\delta(M) =k$ and the assertion be valid for $N$ with $\delta(N) < k$. We have $x_i\in I$, $1\leq i \leq k$ such that $\ell(M/\sum_{i=1} x_iM)< \infty$ hence $\ell(N/\sum_{i=1}^{k-1} x_iN) < \infty$ where $N=M/x_k M$. It follows that $d(N) \leq k-1$ But
    \begin{align*}
        0 \morphism x_kM/x_k\cap I^\ell M \morphism M/I^\ell/M \morphism N/I^\ell N\morphism 0
    \end{align*}
    and $\ell(x_kM/x_kM\cap I^\ell M) \leq \ell(M/I^{\ell-1} M)$ as $M/I^{\ell-1} M \morphism x_kM / (x_kM \cap I^\ell M)$, $m \mod I^{\ell-1} M \morphism (x_km)\mod(x_1M\cap I^\ell M)$ is surjective. It follows that $Q_N(\ell) \geq Q_M(\ell) - Q_M(\ell-1)$ and if $Q_M (\ell) = \gamma \ell^d + \Oo(\ell^{d-1})$ then $Q_M(\ell) - Q_M(\ell-1) = \gamma d\ell^{d-1} + \Oo(\ell^{d-2})$ hence $d(N)\geq d(M) -1$ and $d(M) \leq d(N) +1 \leq \delta(N) +1 \leq \delta(M)$.
    
    \begin{cor}[*facepalm*]
        The Principal Ideal Theorem of Krull and its consequences follow.
    \end{cor}
    
    To end the proof, we have to show $\delta \leq \dim(M)$. Induction on $\dim(M)$. If $\dim(M) = 0$ all $\pp_i$ in \eqreff{eq:filtrationByPrimeIdeals} are $\pp_i= \mm$ hence $\ell(M) < \infty$ and $\delta(M) =0$. Let $\dim(M) < 0$ and the assertion be valid for modules of lower dimension. Let $x_1 \in I  \setminus \bigcup_{\qq\in Q_M}\qq$ (which is possible by prime avoidance as $I\subseteq \qq$ would imply $\mm = \sqrt{I}\subseteq \sqrt{\qq} = \qq$ which is impossible as $\dim M \geq 1$\textbraceright.  Let $N= M/x_1 M$ then $\sqrt{\Ann_R(N)} = \sqrt{xR + \bigcap_{\qq\in Q_M}\qq}$ hence $\supp(N) \subseteq \supp(M)$, hence any $\rr\in Q_N$ contains in some $\qq\in Q_M$ but $\rr \neq \qq$ as $x_1\in \rr$, $x_1\not \in \qq$. Thus, every irreducible component of $\supp N$ is strictly contained in some irreducible component of $\supp M$. Thus $\delta(N) \leq \dim (N) < \dim(M)$ and if $d=\dim(M)$ there are $x_2,\ldots, x_{d-1}\in I$ such that $\ell(M/\sum_{i=1}^d x_i M) = \ell(N/\sum_{i=2}^d x_i N) < \infty$, hence $\delta(M) \leq \dim(M)$.
