...
\begin{defi}[Regular function]\lbl{def:regularFunction}
    Let $Z\subseteq\IP^n(k)$ be a quasi-projective algebraic variety in $\IP^n(k)$. A function $Z\morphism[f] k$ is called \defemph{regular} at $x\in Z$ if $x$ has an open neighbourhood $U$ in $\IP^n(k)$ such that there are $p,q\in R = k[X_0,\ldots,X_n]$ which are homogeneous of the same degree, such that $q(y_0,\ldots,y_n) \neq 0$ for $[y_0,\ldots,y_n]\in U$ and
    \begin{align*}
...
\end{example}
\begin{prop}
    Let $Z\subseteq \IP^n(k)$ be a quasi-projective algebraic variety, then $Z\cap \IA^n(k) = Z \setminus V(X_0)$ is quasi-affine or empty, and a function $f\colon Z\cap \IA^n(k)\morphism k$ is regular in the sense of Definition~\reff{def:regularFunction} if and only if it is regular in the sense of the quasi-affine counterpart of that definition. 
\end{prop}
\begin{proof}
...
\begin{proof}[Proof of Theorem~\reff{thm:gradedRing}]
    ...
    Let $\eta\in \Oo^\ell(X\setminus V(f))$ (the case $\ell=0$ corresponds to the study of $\Oo(X\setminus V(f))$), where $f\neq 0$ is homogeneous of degree $\delta >0$. By Definition~\reff{def:regularFunction}, every $x\in X$ has an open neighbourhood $U_x$ such that there are polynomials $p_x,q_x\in R=k[X_0,\ldots, X_n]$ where $q_x$ is homogeneous of degree $b_x$, $p_x$ of degree $a_x = b_x+\ell$ and $V(q_x)\cap U_x = \emptyset$ and such that 
    \begin{align*}
        \eta(y_0,\ldots, y_n) = \frac{b_x(y_0,\ldots, y_n)}{q_x(y_0,\ldots, y_n)}
    \end{align*}
    when $[y_0,\ldots, y_n]\in U_x\subseteq X\setminus V(f)$. Replacing $p_x$ by $p_x\cdot r$, $q_x$ by $q_x\cdot r$, and $U_x$ by $U_x\setminus V(r) = X\setminus V(r)$ with $r$ homogeneous and $\IP^n(k)\setminus U_x \subseteq V(r)$ and $x\not \in V(r)$, we may assume that $U_x\cap X = X\setminus V(q_x)\subseteq X\setminus V(f)$. The topological space $\IP^n(k)$ (and thus also $X$) being Noetherian, $X\setminus V(f)$ is quasi-compact, hence is covered by finitely many $X\setminus V(q_{x_i})$, $i=1,\ldots,N$. Let $p_i= p_{x_i}$ and $q_i = q_{x_i}$. Then 
    \begin{align*}
    ...
    \end{align*}
    In other words, there are $h_i\in R_{s\cdot \delta-b_i}$ such that $\phi^k\equiv \sum_{i=1}^N h_iq_i \mod\pp$. Let 
    \begin{align*}
    ...
    \end{align*}
    and thus $\eta(x) = h(x)$ when $x\in X\setminus V(q_j)$. As the sets cover $X\setminus V(f)$, surjectivity follows.
...
\begin{rem*}
    \begin{alphanumerate}
        ...
        \item   
            ...
            One would expect that for $\ell$ sufficiently large, $\lambda$ ``vanishes to a very high order'' at $0$, hence extends to an element of $\Oo(C(X))$ and this turns out to be correct, as is shown on the 10$\ordinalth$ exercise sheet. It follows that $S_N(X) \isomorphism \Oo^N(X)$ when $N$ is sufficiently large.
        \item 
            In general, $\pp$ defines a sheaf of ideals $\Jj$ on $\IP^n=\IP^n(k)$ and $\Oo^\ell_X$ can be identified with $\Oo^\ell_{\IP}/\Jj\Oo^\ell_{\IP^n}$ and one obtains a part of a long exact cohomology sequence
            \begin{diagram*}
            	\node[ob] (0o) at (0,1.5) {$0$};
            	\node[ob]  (H0J) [right=0.75cm of 0o] {$H^0\left(\IP^n,\Jj\Oo_{\IP^n}^\ell\right)$};
            	\node[ob]  (H0O) [right=0.75cm of H0J] {$H^0\left(\IP^n,\Oo_{\IP^n}^\ell\right)$};
            	\node[ob]  (H0OX) [right=0.75cm of H0O] {$H^0\left(\IP^n,\Oo_X^\ell\right)$};
            	\node[ob]  (H1J) [right=0.75cm of H0OX]  {$H^1\left(\IP^n,\Jj\Oo_{\IP^n}^\ell\right)$};
            	\node[ob]  (0u) at (0,0) {$0$};
