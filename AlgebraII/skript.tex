...
\begin{rem}
    ...
    \end{align*}  
    are irreducible closed subsets of codimension 1 and (thus) dimension 2. But $Y\cap Z = \{0\}$ has codimension 3 in $X$ and the inequality from Theorem~\reff{thm:codimIneqRegular} fails, due to the failure of regularity of $X$ at $0$.
\end{rem}
...
\begin{proof}[Proof of Proposition~\reff{prop:gradedNoetherian}]
    ...
    We obtain \itememph{e} from \itememph{b} since $R_+$ is a homogenous ideal in $R$ and for any ideal $I$ of $R_0$ the sum $I+R_+$ is a homogenous ideal of $R$ and when $I+R_+$ is generated by $f_1,\ldots, f_d\in R_0$ and $f_{d+1},\ldots,f_e$ (homogenous of positive degree) then $I$ is generated by $f_1,\ldots, f_d$.
...
\begin{rem*}
    ...
    \item 
     ...
     \item   
         In particular, $\ell_R(M/IM)=\ell_{R/I}(M/IM) < \infty$ by Proposition~\reff{prop:Akizuki-Hopkins}\itememph{d} as $R/I$ is Artinian and $M$ finitely generated over it. Hence $\delta(M,I) < \infty$ as we may e.g. take $x_1,\ldots, x_k$ to be generators of $I$.
    \item 
...
\end{cor}
\begin{proof}
    We have $\ell_R(R/(x_1R+\ldots +x_nR) ) = \ell_R(R/\mm) = 1< \infty$, hence $\delta(R,\mm) \leq n$ and $\dim(R) \leq n$ by Theorem~\reff{thm:idealOfDefinition}.
\end{proof}
...
\begin{proof}
    ...
    \end{align*}
    by an easy diagram chase. Hence $\ell_R(M/I^kM) = \ell_R(M''/I^kM'') + \ell_R\big(M'/(I^kM\cap M')\big)$ by Remark~\reff{rem:ArtinSchreier}\itememph{f} and an application of the Artin-Rees lemma (Proposition~\reff{prop:artinRees}) gives the existence of some constant $c\in \IN_0$ such that $I^kM'\subseteq I^kM\cap M'\subseteq I^{k-c}M'$. Then there are surjective maps $M'/I^kM'\epimorphism M'/(I^kM\cap M')\epimorphism M'/I^{k-c}M'$, hence their lengths over $R$ must be decreasing in that order and we obtain $Q_{M',I}(k-c) \leq \ell_R\big(M'/(I^kM\cap M')\big) \leq Q_{M',I}(k)$ for $k$ sufficiently large by Fact~\reff{fact:samuelPolynomial}. Thus, for $k$ sufficiently large,
    \begin{align*}
...
\end{proof}
\begin{cor}[This also not Corollary~\reff{cor:dimGeneratorsDefinition}]\lbl{cor:dEqualsMaxDim}
    Appyling this to \eqreff{eq:filtrationByPrimeIdeals} we obtain 
    \begin{align*}
...
\begin{proof}
    ...
    \end{align*}
    (as in \eqreff{eq:filtrationByPrimeIdeals}) but the $\qq\in \QQ_M$ are the $\subseteq$-minimal elements of $\left\{\pp_1,\ldots,\pp_k\right\}$ and if $\pp_i\supseteq \qq$ we have $R/\qq \epimorphism R/\pp_i$, hence $\ell_R\big(R/(I^k+\qq)\big) \geq \ell_R\big(R/(I^k +\pp_i)\big)$, hence $d(R/\pp_i,I) \leq d(R/\qq,I)$.
\end{proof}
...
\end{cor}
\begin{proof}
    By Artin-Rees (Proposition~\reff{prop:artinRees}), there is a $c\in\IN_0$ such that $\mm N \supseteq IN\supseteq I^{c+1} M \cap N = N$. Then $N=0$ by \NAK.
\end{proof}
\begin{rem*}
...
\begin{proof}[Proof of Theorem~\reff{thm:idealOfDefinition}]
    ...
    of length $C+1$ of $R$-submodules of $M/I^kM$, contradiction.
     ...
     Hence $\ell_R(R/\pp_i) \leq C$ in \eqreff{eq:filtrationByPrimeIdeals}, hence all $R/\pp_i$ are Artinian and thus zero-dimensional by Proposition~\reff{prop:Akizuki-Hopkins}. Therefore $\dim(M) = \max\left\{\dim(R/\pp_i)\st 1\leq i\leq k\right\} = 0$ using \eqreff{eq:dimSuppM}. 
    ...
    \emph{Step 3.} To end the proof, we have to show $\delta(M,I) \leq \dim(M)$. We do induction on $\dim(M)$. If $\dim(M) = 0$, all $\pp_i$ in \eqreff{eq:filtrationByPrimeIdeals} are maximal, hence $\ell_R(M) < \infty$ and $\delta(M,I) =0$. 
...
\begin{rem}
    \begin{enumerate}
    \item 
        \itememph{a} can be deduced from the rest. Let $v(x) = 1$, then $v(0) = v(0\cdot x) = v(0) + v(x) = v(0) +1$, implying $v(0)\not \in \IN$. 
    \item 
        It easily follows $v(\epsilon) = 0$ for $\epsilon \in R^\times$. Hence $v(x) = v(-x)$ and $v(x) = v(x+y-y) \geq \max(v(x+y), v(y))$ showing equality in \itememph{c}, when $v(x) \neq v(y)$.
    \item 
        Ordered groups other than $\IZ$ (e.g $\IZ\times \IZ$ ordered lexicographically) may be considered as target of the valuation, giving a more general notion. But these will fail to be Noetherian, unless they are DVR.
    \end{enumerate}
\end{rem}
\begin{defi}[Normal ring]\lbl{def:normal}
    A domain $R$ is called \defemph{normal} if it s integrally closed in its field of quotients $K$. Recall that $x\in K$ is in the integral closure of $R$ in $K$ iff there is a finitely generated $R$-submodule $M\neq\{0\}$ of $K$ such that $x\cdot M\subseteq M$  iff there is an $R$-subalgebra $C\subseteq K$ such that $x\in C$ and $C$ is finitely generated as an $R$-module.
\end{defi}

\begin{defi}[Fractional ideal] \lbl{def:fractionalIdeal} %At least, it was just a remark, not a fact....
    A \defemph{fractional ideal} of a domain $R$ is a $R$-submodule $I\subseteq K$ with $a\cdot I \subseteq R$ for some $a\in R\setminus \{0\}$, where zero is excluded. We put $I^{-1} = \left\{j\in K\st jI\subseteq R\right\}$. We call $I$ \emph{invertible} iff $I\cdot I^{-1}  = R$. When $R$ is Noetherian, it is clear that $I^{-1}$ is a fractional ideal and the invrtible fractional ideals form a group.
\end{defi}
\begin{prop}\lbl{prop:invertPrincipal}
    A fractional ideal $I$ is invertible iff it is finitely generated and $I\cdot R_\pp$ is a principal ideal in $R_\pp$ for any $\pp\in \Spec R$.
\end{prop}
\begin{proof}
    If $I$ is invertible, there are $(a_i)_{i=1}^n$ and $(b_i)_{i=1}^n$ such that $a_i\in I$, $b_i\in I^{-1}$ and $\sum_{i=1}^n a_i b_i = 1$ and it follows that the $a_i$ generate $I$ (and, that the $b_i$ generate $I^{-1}$, so $I^{-1}$ is also finitely generated). If $\pp\in \Spec R$ there is $i$, $1\leq i\leq n$ such that $a_ib_i\not\in  \pp$ hence $a_ib_i\in R_\pp^\times$ and $a_i$ generates $I\cdot R_\pp$. Hence \itememph{b} follows.
    
    Let \itememph{b} hold. We clearly have $I^{-1}\cdot R_\pp\subseteq (IR_\pp)^{-1}$ where equality holds when $I$ is finitely generated. $I\cdot R_\pp$ becomes principal then $(IR_\pp) (IR_\pp)^{-1} = R_\pp$ and $I\cdot I^{-1}\morphism R$ becomes an isomorphism after localization at any $\pp\in \Spec R$. but any such morphism of modules is an isomorphism.
\end{proof}
\begin{lem}\lbl{lem:invertLocal}
    Let $R$ be a normal local domain and $\mm$ is finitely generated, then $\mm$ is invertible or $\mm^{-1} = R$.
\end{lem}
\begin{proof}
...
\begin{proof}
    Assume \itememph{a}. If $I\subseteq R$ is not $0$ and $x\in I$ is such that $v(x)$ is minimal, then it is easy to see that $I = xR = \left\{r\in R\st v(r) \geq v(x)\right\}$. So every ideal in $R$ is principal. 
    
    The implication from \itememph{b} to \itememph{c} is trivial.
    
    Now assume \itememph{c}, we want to show \itememph{a}. Let $v(x) = \sup\left\{k\in \IN \st x\in \mm^k\right\}$. By the Kurll intersection theorem, $v(x) = \infty$ iff $x=0$. Let $\mm = \pi R$, then $\mm^k = \pi^k R$ (hence $\pi$ is not nilpotent, as $\mm$ is nilpotent and $\dim R = 0$ otherwise) and $x\in \pi^{v(x)} R$ for all $x\in R\setminus\{0\}$, and it follows that $v(xy) = v(x) + v(y)$, that $R$ is a domain and all the further properties of a DVR.
    
    The points \itememph{d} and \itememph{e} are obviously equivalent by what was recalled before.
    
    Now assume \itememph{a}, we want to show \itememph{e}. Let $x\in K$ be integral over $R$: $x^n = \sum_{i=0}^{n-1} r_i x^i$ with $r_i \in R$. Then $nv(x) \geq \min \left \{r_ix_i\st 0\leq i\leq n\right\} \geq \min\left\{ 0, (n-1)v(x)\right\}$. (as $r_i\in R$, hence $v(r_i)\geq 0$) and $v(x)\geq 0$ (as it is finite otherwise).
    
    \itememph{e} $\to$ \itememph{c}. We will show the equivalent to Proposition~\reff{prop:invertPrincipal}. Assume that $\mm$ is not invertible. By Lemma~\reff{lem:invertLocal}, $\mm^{-1} = R$. let $x\in \mm\setminus \mm^2$. by Proposition~\reff{prop:filtrationByPrimeIdeals}, there is $y\in R$ such that $\upsilon = y\mod xR\in R/xR$ satisifies $\Ann_R(\upsilon) = \pp$ for some $\pp\in \Spec R$. Then $\pp = \left\{r\in R\st ry\subseteq xR\right\}$ is not 0 as $x\in\pp$. As $\dim(R) = 1$, $\pp = \mm$. Let $a=y/x$ then $a\not \in R$ as $y\in xR$ otherwise. But $y\in \mm^{-1}$ because for $\mu\in\mm = \pp$ we have $\mu y\in xR$ hence $\mu a = (\mu y)/x\in R$. A contradiction.
\end{proof}
...
\begin{thm}\lbl{thm:dedekindDomain}
    ...
    \item 
        $R$ is Noetherian and any $R_\pp$ is field or a DVR.
    \item 
    ...
    \end{alphanumerate}
    In fact, the decomposition in \itememph{e} is for ideals not equal to $0$ unique up to reordering and the group of fractional ideals is free with the non-zero prime ideals as generators.
\end{thm}
\begin{defi}[Dedekind rings]
    Such $R$ are called \defemph{Dedekind domains} and $\operatorname{Cl} = (\text{fractional ideals}/(K^\times/R^\times)$ is the \defemph{class group} of $R$.
\end{defi}
...
\itememph{a} $\rightarrow$ \itememph{b} is trivial.

\itememph{b} $\rightarrow$ \itememph{a}: Let $I$ be any fractional ideal. Then there is some $r\in R\setminus\{0\}$ such
...
\itememph{b} $\rightarrow$ \itememph{c}: As invertibility for fractional ideals is equivalent to \enquote{finitely generated and locally principal}
(Proposition \ref{prop:invertPrincipal}),
\itememph{b} implies that any ideal is finitely generated, hence
$R$ is Noetherian. If $\pp \in \Spec R$ and $\pp\neq \{0\}$, then the same proposition implies
that $\pp R_\pp$ is principal ($\pp$ being invertible) and $R_{\pp}$ is a DVR by Theorem \ref{thm:regularDVR}.

\itememph{c} $\rightarrow$ \itememph{b}: This also follows from Proposition \reff{prop:invertPrincipal}.

\itememph{c} $\rightarrow$ \itememph{d}: It is clear that $R$ is Noetherian, and $\dim R\leq 1$ follows easily from
$\dim R_\pp\leq 1$ for all $\pp\in \Spec R$ (use \itememph{c} and Theorem \ref{thm:regularDVR}). We still have to show $R$ is normal.

We have seen that DVRs are normal. For any domain $R$, $R=\bigcap_{\mm \in \mSpec R} R_m$ (the intersection being
taken in the field of fractions $K$): If $k\in K$
is in the intersection but not in $R$, then $I=\{r\in R \mid rk\in R\}$ is a principal ideal ($k\notin R$, hence $1\in I$) not contained
in any proper ideal $\mm$ (as $k\notin R_\mm$ otherwise), a contradiction.

If $k\in K$ is integral over $R$ and $R$ satisfies \itememph{c}, it is integral over the $R_\mm$ hence contained in $R_\mm$ (as $R_\mm$ are DVRs),
hence contained in $\bigcap_{\mm} R_\mm = R$.

\itememph{d} $\rightarrow$ \itememph{\tilde d} Normality is easily seen to be preserved under localization, hence the $R_\mm$ are normal, hence
regular by Theorem \ref{thm:regularDVR}.

\itememph{\tilde d} $\rightarrow$ \itememph{c}: Theorem \ref{thm:regularDVR}.

\itememph{c} $\rightarrow$ \itememph{e}: Let $I\neq 0$ be any ideal of $R$. For $\mm\in \mSpec R$, we have $I\cdot R_\mm = a R_\mm$
for some $a\in R_\mm$. Put $v_\mm(I) = v_\mm(a)$ where $v_\mm : \mathrm{Quot}(R_\mm) \rightarrow \mathbb Z\cup \{\infty\}$ is the
function on $K$ defined by the valuation $v_\mm$ of $R_\mm$.
Then $I=\prod_{\mm\in \mSpec R} \mm^{v_\mm(I)}$ as
\begin{align*}
\left(\prod_{m\in \mm\Spec R} \mm^{e_\mm}\right)_{\nn} = \prod_{\mm} (\mm R_\nn)^{e_\mm} = (nR_\nn)^{e_\nn},
\end{align*}
since $\mm R_\nn = R_\nn$ when $\mm\neq \nn$ because
$\mm$ and $\nn$ are maximal ideals, so $\mm\not\subseteq \nn$.
It follows that $v_\nn\left(\prod_\mm \mm^{e_\mm}\right) = e_\nn$. As $IR_\mm$ is uniquely determined by $v_\mm(I)$, it follows
that $I_\nn = \left(\prod_\mm \mm^{v_\mm(I)}\right)_\nn$ for any $\nn\in \mSpec R$,
implying $I=\prod_\mm \mm^{v_\mm(I)}$ by arguments similar to the ones for the proof of (\itememph{c}$\rightarrow$\itememph{d}) above.
Uniqueness of the decomposition
also follows from these considerations.

\itememph{e} $\rightarrow$ \itememph{c} (When $R$ is Noetherian).
It is easy to see that \itememph{e} is inherited by localizations,
hence it suffices to show that any local Noetherian domain $R$ with \itememph{e} is a field or a DVR. Let $\mm\subset R$ be the maximal
ideal, $k=R/\mm$ and assume that $x,y\in \mm/\mm^2$
are linearly independent over $k$ (in particular $\neq 0$). Let $V,W\subseteq \mm/\mm^2$ be $k$-subvector spaces spanned by $x$ (resp. $y$)
and let $I,J\subseteq \mm$ be the preimages of $V$ and $W$. Let moreover
$I=\prod_{i=1}^n \pp_i$ be a decomposition of $I$ into prime ideals as in \itememph{e}. We have $\pp_i\subseteq \mm$,
hence $I\subseteq \mm^n$ implying $n\leq 1$ (since $\mm^2\subsetneqq I$).
As $n=0$ would imply $I=R$, we get $n=1$, hence $I=\pp$ and $\pp = \sqrt \pp = \sqrt I = \mm$
(since $I\supseteq \mm^2$). This shows $I=\pp = \mm$ and the same holds for $J$, a contradiction.
We conclude $\dim(\mm/\mm^2)=0$, so $(\mm=0$ and $R$ is a field) or ($\dim(\mm/\mm^2)=1$ and $R$ is a DVR (Theorem
\ref{thm:regularDVR})).

Prof. Franke claims he \emph{has} a proof that \itememph{e} implies that $R$ is Noetherian, but does not give to us.
This implication can nevertheless be seen as follows: Suppose first that $I,J$ are non-zero ideals of $R$ such that $I\subseteq J$.
Fix decompositions of $I$ and $J$ as in \itememph{e}.
If $\pp$ is any maximal ideal and $e\in \IN$ (resp. $f\in \IN$) the multiplicity of $\pp$ in the decomposition of $I$ (resp. $J$)
then $\pp^e R_{\pp}=IR_\pp \subseteq JR_\pp=\pp^f R_{\pp}$.
Pick an a non-zero $\alpha \in\pp$ and decompose $\alpha R_\pp = \pp^g R_\pp$ (since \itememph{e} is inherited by $R_\pp$).
Then $\alpha^e R_\pp = \left(\pp^{e} R_\pp\right)^g \subseteq \left(\pp^{f} R_\pp\right)^g = \alpha^f R_\pp$,
so $\alpha^f$ divides $\alpha^e$. Since $R_\pp$ is a domain, we conclude $e\leq f$.

We see: Once we fixed a decomposition of $I$ as in \itememph{e}, there are only finitely many possibilities
for the decompositions of ideals $J\supseteq I$. In particular, an increasing chain of ideals
$I=I_0\subseteq I_1\subseteq I_2\subseteq\cdots$ in $R$ will satisfy that only finitely many different ideals occur in it!
This shows that any increasing chain of ideals in $R$ must become stationary, so $R$ is Noetherian.
\end{proof}
...
By Theorem \reff{thm:regularDVR}, $\Oo_{C,c}$ is a DVR. Let $v_c : \Oo_{C,c}^\times \rightarrow\mathbb Z$ be its valuation.
...
\end{example*}
\begin{proof}[Proof sketch Theorem \ref{thm:maxNoether}, finishing the final lecture in a hurry and five minutes overtime]

\itememph{a} $\rightarrow$ \itememph{b}:
...
To prove \itememph{a}, one notes the the result can be shown to hold when $d$ is large (exercise sheet 10, exercise 6).
...
$G=AP+BX_0$ and $\varphi X_0=BX_0$ on $C$, such that $\varphi=B$ there.
\end{proof}
