...
\end{defi}
\begin{example}
    If $R$ is a graded ring, the \defemph{augmentation ideal} $R_+$ is $R_+= \bigoplus_{i=1}^\infty R_n$. In the case $R=k[X_0, \ldots, X_n]$ (graded as in Example \reff{ex:gradedPolynomial}) we have $R_+= \left\{f\in R\st f(0) = 0\right\}$.
\end{example}
For now on let $k$ be an algebraically closed field.
\begin{defi}[Projective vanishing set] \lbl{def:projectiveVanishingSet}
    If $I\subseteq R= k[X_0,\ldots, X_n]$ is a homogenous ideal we put $V(I) = V_{\operatorname{Proj}} (I) = \left\{[x_0,\ldots, x_n]\st f(x_0,\ldots, x_n) = 0 \text{ for all } f\in I\right\}$ as the \defemph{projective vanishing set} of $I$.
\end{defi}

\begin{rem}
    \begin{alphanumerate}
        \item 
            In this section let $V(I) = V_{\operatorname{Proj}}(I)$ and $V_{\operatorname{aff}}(U) \subseteq k^{n+1}$ denote affine vanishing set as studied before.
        \item 
...
\begin{prop}
    ...
    \begin{alphanumerate}
        ...
        \item 
            Any set $\MM\neq 0$ of homogenous ideals in $R$ has an $I\in \MM$ such that $I\not\subseteq J$ for $J\in \MM$ and $J\neq I$.
        \item 
            Any ascending chain $I_0\subseteq I_1\subseteq \ldots$ of homogenous ideals in $R$ becomes stationary for some $n\in\IN$, i.e. $I_m = I_n$ for all $m\geq n$.
        \item 
...
\end{rem}
\begin{proof}
    The implication \itememph{f} $\Rightarrow$ \itememph{a} follows from Hilbert's Basissatz. That \itememph{a} implies \itememph{b} to \itememph{d} is trivial since these are special cases of the definitions of Noetherian. We conclude \itememph{d} from \itememph{c} by the axiom of choice. For the implication \itememph{d} $\Rightarrow$ \itememph{b} the proof for the ungraded case still applies, as the ideal generated by a set of homogenous elements of $R$ is homogenous and for every inclusion $J\subsetneq I$ of ideals in $R$ with homogenous $I$ there is some homogenous $f\in I\setminus J$.
...
\begin{rem*}
    \begin{alphanumerate}
        ...
        \item 
            The condition $\sqrt{I}\subsetneq R_+$ for homogenous ideals $R$ can also be expressed as $\dim_k(R/I) = \infty$.
    \end{alphanumerate}
\end{rem*}
...
\begin{proof}
    ...
    \begin{align*}
        \sqrt{J} = \left\{f\in R\st f(x) = 0 \text{ for all } x\in V_{\operatorname{aff}}(J)\right\}
    \end{align*}
    and $V(J)\neq \emptyset$ for $J\subsetneq R$. Since we assume $\sqrt{I}\subsetneq R_+ = \sqrt{R_+}$ this implies that there is $x\in V_{\operatorname{aff}}(I)V_{\operatorname{aff}}(R_+) = V_{\operatorname{aff}}(I)\setminus\{0\}$. Let $x=(x_0,\ldots, x_n)$, then $[x_0,\ldots,x_n]\in V(I)$. 
    
    Moreover, let $f\in R$ be such that $f(x) = 0$ when $x\neq 0$ and $[x_0,\ldots, x_n]\in V(I)$. Then $f(x)=0$ when $x\in V_{\operatorname{aff}}(I)\setminus\{0\}$. For such $x$ and $\lambda\neq 0$, $f(\lambda x) = 0$. Since the Zariski closure of $k^\times \cdot x$ in $k^{n+1}$ is $k\cdot x$. As $V(I)\neq \emptyset$ it follows that $0\in V_{\operatorname{aff}}(f)$, hence $V_{\operatorname{aff}}(f)\supsetneq V_{\operatorname{aff}}(I)$ and $f\in \sqrt{I}$ by the affine Nullstellensatz.
\end{proof}
\begin{rem*}
    The only homogenous ideals $I\subseteq R$ such that $\sqrt{I}=I$ and $V(I)\neq\emptyset$ are $I=R_+$ and $I=R$. Also, any homogenous ideal of $R$ equals $R$ of is contained in $R_+$.
\end{rem*}
