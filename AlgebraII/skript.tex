...
\begin{rem*}
    \begin{alphanumerate}
        ...
        \item 
            ...
            \end{align*}
            In other words, the functor $\cat{Set}\morphism A\cat{-Mod}$ mapping $X$ to the free $A$-module generated by $X$ is \emph{left-adjoint} to the forgetful functor from the category of $A$-modules to $\cat{Set}$.
    \end{alphanumerate}
\end{rem*}
...
\begin{proof}[Proof of Proposition~\reff{prop:finiteTypeAlgebrasRegular} if $A$ is a domain]
	...
	\end{align*}
	Now $\Omega_{A/k}\otimes_AK$ equals $\Omega_{K/k}$, which by Proposition~\reff{prop:kahlerFieldExtension1} has dimension $\deg\tr(K/k)$ over $K$. But $\deg\tr(K/k)=\dim(A)$ by \cite[Theorem~10]{alg1} and we're done.
\end{proof}
...
\begin{example}\lbl{ex:O(l)LineBundle}
    ...
    \end{align*}
    defines an isomorphism $\Oo|_U \morphism[\cdot X_0^\ell] \Oo(\ell)|_U$ of sheaves of (abelian groups or) $\Oo$-modules. Thus, $\Oo(\ell)$ is indeed a line bundle on $Y$. 
\end{example}
\begin{prop}
...
\begin{proof}[Proof of B\'ezout's Theorem]
    ...
    \end{align*}
    Let $0=M_0\subsetneq \ldots \subsetneq M_n = M$ be a filtration as in Proposition~\reff{prop:filtrationByPrimeIdeals}, with $M_i/M_{i-1} \simeq (R/\qq_i)[t_i]$, $\qq_i\in\Spec R$ being homogeneous prime ideals and $t_i$ some integers. By Fact~\reff{fact:AnnPrimeCap} we have    
    \begin{align*}
    ...
    It remains to prove Claim~\reff{claim:occurrencesOfq}. We have $M_\qq \simeq (R_\qq/H R_\qq)$ by the exactness of localization. By more properties of localization, the $R_\qq$-modules $0=(M_0)_\qq\subseteq \ldots\subseteq (M_k)_\qq=M_\qq$ form a filtration of $M_\qq$ with filtration quotients 
    \begin{align*}
...
\begin{cor}\lbl{cor:dimOfGr}
    The dimension of any Noetherian local ring $(R,\mm)$ is equal to dimension of its associatd graded ring $\gr(R,\mm)$.
\end{cor}
\begin{proof}
...
The implication \itememph{a} $\Rightarrow$ \itememph{b} is trivial. For \itememph{b} $\Rightarrow$ \itememph{a}, let $I$ be any fractional ideal. Then there is some $r\in R\setminus\{0\}$ such
...
Now for \itememph{b} $\Rightarrow$ \itememph{c}. As invertibility for fractional ideals is equivalent to being finitely generated and locally principal
...
The converse implication \itememph{c} $\Rightarrow$ \itememph{b} also follows from Proposition~\reff{prop:invertPrincipal} and the fact that DVRs are always PIDs.
...
\end{thm}
\begin{example*}
Let $Q\subseteq \IP^2(k)$ be a conic and $P_1,\dotsc,P_6$ different points and $g_i$ the staight line connecting
