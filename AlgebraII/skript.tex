...
\end{proof}
\begin{defi}
    The graded ring defined by a projective variety $X= V(\pp) \subseteq \IP^n(k)$ is defined as $S_*(X) = k[X_0,\ldots, X_n]/\pp$. We would like to compare this to the structure sheaf, or of the line bundles $\Oo^\ell$ on $X$. 
\end{defi}
\begin{thm}\lbl{thm:gradedRing}
    Let $X$ be a projective algebraic variety. 
    \begin{alphanumerate}    
      \item 
        For any $f\in S_*(X)$ which is homogeneous of positive degree,
        \begin{align*}
