...
\end{proof}
\begin{varthm}{fact}This should actually be in Fact~\reff{fact:DerExactSequences}, but I refuse to change a fact that was stated two lectures ago.
    \begin{itemize}
      \item[\itememph{c}]
        When $X\subseteq R$ is multiplicative and $M$ an $A_X$-module, then we have
        \begin{align}\lbl{eq:DerLocalizationIso1}
            \Der_{R_X}(A_X,M) \isomorphism \Der_R(A,M).
        \end{align}
      \item[\itememph{d}]
        When $S\subseteq A$ is multiplicative and $M$ an $A_S$-module, then we have
...
\end{varthm} 
\begin{proof}
    Both maps are defined by composition of derivations with the ring homomorphism $A\morphism A_X$ or $A\morphism A_S$, so this is well-defined.
    
    To proof \itememph{c}, take any element $d\in \Der_R(A,M)$. This is $R$-linear and hence defines a unique homomorphism $A_X\morphism[d] M$ of $R_X$ modules, by our assumption on $M$ and the universal property of the localization. To confirm Leibniz rule, look at
    \begin{align*}
    ...
    \end{align*}
    implies $d(\alpha) = 0$ since $\sigma\in (A_S)^\times$. This shows injectivity of the map. For surjectivity, let $\delta\in \Der_R(A,M)$ and put $d\left(\frac{a}{s}\right) = \frac{s\cdot\delta(a) - a\cdot\delta(s)}{s^2}$. We have
    \begin{align*}
...
\begin{varthm}{cor}
    \begin{alphanumerate}
        \item[\itememph{c}]
            As $A\morphism A_X \xrightarrow{d_{A_X/R_X}} \Omega_{A_X/R_X}$ is an $R$-linear derivative of $A$, there is a unique $\Omega_{A/R}\morphism[\alpha] \Omega_{A_X/R_X}$ of $A$-modules such that $d_{A_X/R_X} (a) = \alpha(d_{A/R}(a))$ for any $a\in A$. By the universal property of the localization $\Omega_{A/R}\morphism[\lambda](\Omega_{A/R})_X$ there is a unique $\iota$ such that $\alpha = \iota \lambda$. Thereby we obtain
            \begin{align*}
                \Omega_{A_X/R_X} \lisomorphism[\iota] (\Omega_{A/R})_X.
            \end{align*}
        \item[\itememph{d}]
            Since $A\morphism A_S\xrightarrow{d_{A_S/R}} \Omega_{A_S/R}$ is an element of $\Der_R(A,\Omega_{A_S/R})$ there is a unique $\Omega_{A/R} \morphism[\alpha]\Omega_{A_S/R}$ of $A$-modules such that $d_{A_S/R}(a) = \alpha(d_{A/R}(a))$. By the universal property of $\Omega_{A/R}\morphism[\lambda](\Omega_{A/R})_S$ there is a unique $\iota$ such that $\alpha=\iota \lambda$. This gives us
            \begin{align*}
                \Omega_{A_S/R} \lisomorphism(\Omega_{A/R})_S.
            \end{align*}
    \end{alphanumerate}
\end{varthm}
...
\begin{defi}
    ...
    \end{align*}
    which has the universal property for $R$-bilinear maps $M\times N \morphism[\tau] T$: If $\tau$ is any such map, there is a unique $t\in \Hom_R(M\otimes_R N , T)$ such that $\tau(a,b) = t(a\otimes b)$.
\end{defi}
...
\begin{rem}
    \begin{alphanumerate}
        ...
        \item 
            $M\otimes_R N$ is generated as an $R$-module by $\left\{m\otimes n \st m\in M, n\in N\right\}$. In fact, let $T$ be the quotient by elements of this form, $\tau = 0$, $t_1=0$, $t_2$ the projection $M\otimes_R N \morphism T$. Thus, a morphism $M\otimes_R N \morphism[f] X$ of $R$-modules is uniquely determined by giving $f(m\otimes n)$ for $m\in M$, $n\in N$. Existence is guaranteed by the universal property, provided that the expression given for $f(m\otimes n)$ is bilinear in $m$ and $n$.
    \end{alphanumerate}
\end{rem}
\begin{prop}
    $M\otimes_R N$ exists.
\end{prop}
\begin{proof}
    $M\otimes_R N$ can be constructed by $F/K$ and $m\otimes n$ by the image of $\delta_{(m,n)}$ in $F/K$. Here $F$ is the free $R$-module generated by $M\times N$ and $K$ the submodule generated by elements of the form 
    \begin{align*}
        &\delta_{(m+m',n)}-\delta_{(m,n)}-\delta_{(m',n)}\\
        &\delta_{(m,n+n')}-\delta_{(m,n)}-\delta_{(m,n')}\\
        &\delta_{(rm,n)}-r\delta_{(m,n)}\\
        &\delta_{(m,rn)}-r\delta_{(m,n)}.
    \end{align*}
    
\end{proof}
\subsection{Use of the tensor product to base-change a module}
...
We have a homomorphism $M\morphism A\otimes_R M$ sending $m$ to $1\otimes m$, a morphism of $R$-modules by the $R$-bilinearity of $\otimes$. Let $T$ be any $A$-module and $\tau\in \Hom_R(M,T)$, then there is a unique homomorphism $t\colon A\otimes_R M \morphism T$ of $R$-modules such that $t(a\otimes m) = a\tau(m)$. And thus $\tau$ makes

\begin{diagram}
    \node (M) at (0,1.25) {$M$};
    \node (tensor) at (2.5,1.25) {$A\otimes_R M $};
    \node (T) at (1.25,0) {$T$};
    \scriptsize
    \draw[->] (M) -- (tensor) node[pos=0.5, above] {$1\otimes - $};
    \draw[->] (M) -- (T) node[pos=0.5, below left] {$\tau$};
    \draw[->] (tensor) -- (T) node[pos=0.5, below right] {$t$};
\end{diagram}

commutative, and is $A$-linear:
\begin{align*}
