...
\begin{center}
	\begin{minipage}{0.42\textwidth}
		...
		\begin{tikzpicture}
			...
			\draw[domain=0:1.59,variable=\x] plot[quick hobby] (\x, {\x *sqrt(\x)});
			\draw[domain=0:1.59,variable=\x] plot[quick hobby] (\x, {-\x *sqrt(\x)});
			\fill (0,0) circle (3pt);
		\end{tikzpicture}
	...
	\begin{minipage}{0.42\textwidth}
		...
		\draw[->, line width=1] (-2.25,0) -- (2.25, 0) node[below] {$X$};
		\draw[->, line width=1] (0,-2.25) -- (0,2.25) node[right] {$Y$};
		\draw[domain=-1.25:1.25,variable=\x] plot[quick hobby] (\x, {\x *sqrt(\x*\x +1)});
		\draw[domain=-1.25:1.25,variable=\x] plot[quick hobby] (\x, {-\x *sqrt(\x*\x +1)});
		\fill (0,0) circle (3pt);
		\end{tikzpicture}
...
\begin{proof}[Proof of Theorem \reff{thm:codimIneqRegular}]
    We will show the following: If $f_1,\ldots, f_d\in\Oo_{X,x}$ generate $\mm_{X,x}$, then there exists an affine open neighbourhood $U$ of $x$ in $X$ and preimages of the $f_i$ in $\Oo(U)$ (which we will also call $f_i$) such that 
    \begin{align*}
        \Delta \cap (U\times U) &= \left\{(x,y)\in U\times U \st f_i(x) = f_i(y) \text{ for } 1\leq i \leq d\right\}\\
        &= V((f_1(x)-f_1(y)),\ldots, (f_d(x)-f_d(y)))
    \end{align*}
    (possibly after shrinking $U$), then we obtain $\codim(C, U\times U) \leq \codim(Y,X) +\codim(Z,X)+d$ by Corollary \reff{cor:capV(f)codimension} and Proposition \reff{prop:codimensionProduct} where we use the identification of $Y\cap Z$ with $\Delta \cap (Y\times Z)$ as in the proof of Theorem \reff{thm:codimIneq}. Hence, 
    \begin{align*}
        \codim(C,X)& = \dim(X) -\dim(C)\\
        &= \dim (X) - (\dim(X)+\dim(X) -\codim(C,X\times X))\\
        &= \codim(C\cap(U\times U),U\times U) -\dim X\\
        &\leq \codim(Y,X) +\codim(Z,X) +d-\dim(X) \\
        &=\codim(Y,X) +\codim(Z,X),
    \end{align*}
    provided that $d=\dim(X)$, which is possible for an appropriate choice of the $f_i$ when $X$ is regular at $x$.
    
    Let $R= k[X_1,\ldots, X_m]$ and $I\subseteq R$ (respectively $\mm\subseteq R$) be the prime (respectively maximal) ideal defining $k^\ell \times \{0\}^{m-\ell}$ (respectively $\{0\}^m$) then $\mm^2 \cap I \subseteq \mm\cdot I$. This can be shown as follows. We have 
    \begin{align*}
        \mm^2 = \left\{\sum_{\alpha\in M } p_\alpha X^\alpha \st p_\alpha\in k, \text{ only finitely many } p_\alpha\neq 0\right\},
    \end{align*}
    where $M = \left\{(\alpha_1,\ldots, \alpha_m) \st |\alpha| \sum_{i=1}^m \alpha_i \geq 2\right\}$ and 
    \begin{align*}
        I = \left\{\sum_{\alpha\in N} p_\alpha X^\alpha \st p_\alpha\in k, \text{ only finitely many } p_\alpha\neq 0\right\},
    \end{align*}
    with $N = \left\{(\alpha_1,\ldots, \alpha_m) \st \exists i: \ell< i \leq m: \alpha_i \leq 1\right\}$. When $f\in \mm^2\cap I$ it follows that $f= \sum_{\alpha\in \IN^m} p_\alpha X^\alpha$ where $p_\alpha = 0$ unless $\alpha \in M\cap N$, i.e. there is an $i$ with $\ell < i\leq m$ and $p_{\alpha_i}>0$. Let $\beta = (\delta_{j,i})_{j=1}^m $ then $\gamma = \alpha-\beta\in \IN^m$ and $|\gamma| \geq |\alpha|-|\beta| = |\alpha| -1 > 0$, such that $X^\gamma\in\mm$ and $X^\beta\in I$.
    
    Let $\xi\in k^m$, $L\subseteq k^m$ an affine subspace containing $\xi$. Let $I$ (respectively $\mm$) be the prime (respectively maximal) ideal defining $L$ (respectively $\{\xi\}$). Then $I\cap \mm^2 \subseteq I\cdot \mm$. This can be reduced to the previous step by an affine automorphism of $k^m$.
    
    Let $x\in k^n$, $\mm_x \subseteq S = k[X_1,\ldots, X_n]$ the maximal ideal defined by $x$, $f_1,\ldots, f_n\in S$ such that their images generate $\mm_x/(\mm_x^2)$. Then there is $h\in R = k[X_1,\ldots, X_n,Y_1,\ldots Y_n]$ sucht that $h(x,x) \neq 0$ and $h\cdot I_\Delta \subseteq \langle g_1,\ldots, g_n\rangle_S$ where $g_i(x,y) = f_i(x)-f_i(y)$ and $I_\Delta\subseteq S$ is the prime ideal of functions vanishing on $\Delta = \left\{(x,x)\st x\in k^n\right\}$. To see this, let $\xi = (x,x)$ and $J = I_\Delta \cdot S_{\mm_\xi}$. Let $\nn = \mm_xi\cdot S_{\mm_\xi}$ denote the maximal ideal of $S_{\mm_xi}$. From the previous step it follows that 
    \begin{align}\lbl{eq:thm15}
        \nn^2\cap J \subseteq J\cdot \mm .
    \end{align}
    Let $\gamma \subseteq J$ be the ideal generated by the images of the $g_i$. If $f\in J$ then there are $(c_i)_{i=1}^n$ such that $f-\sum_{i=1}^n c_i g_i\in \nn^2$. We have $\phi = f-\sum_{i=1}^n c_ig_i\in \nn^2$ iff $(\nabla g)(\xi) = 0$. Since $f|_\Delta = 0$, $\frac{\partial f}{\partial x_i} + \frac{\partial f}{\partial y_i} = 0$ at $\xi\in \Delta$. Thus, it is sufficient to have 
    \begin{align*}
        \frac{\partial f}{\partial x_i}(\xi) = \sum_{j=1}^n c_j \frac{\partial g_j}{\partial x_i}(\xi) = \sum_{j=1}^n c_j \frac{\partial f_j}{\partial x_i}(x)
    \end{align*}
    which is possible since the $f_i$ generate $\mm_x/\mm_x^2$. It follows that $J \subseteq \gamma + (J\cap \nn^2) = \gamma + J\cdot \nn$. By Corollary \reff{cor:NAK}, we have $J= \gamma$. Now let $(\phi_i)_{i=1}^N$ generate $I_\Delta$. Since $J = I\cdot S_{\mm_\xi}$ equals $\gamma = \langle g_1, \ldots, g_n\rangle_{S_{\mm_\xi}}$, there are $h\not\in \mm_xi$ and $\rho_{i,j}$ such that $h\cdot \phi_i = \sum_{j=1}^n \rho_{i,j}g_j$, showing our claim.
    
    Let $f_1,\ldots, f_d$ be elements of $S= k[X_1,\ldots, X_n]$ such that their images in $\Oo_{X,x}$ form a base of $\mm_{X,x}/\mm_{X,x}^2$ ($\mm_{X,x} \coloneqq \rad \Oo_{X,x}$). Let $f_{d+1},\ldots, f_n$ be chosen in such a way that the images of $f_1,\ldots, f_n$ form a base of $\mm_x/\mm_x^2$. If $h$ is as in the previous step, $U= X\setminus V(h)$ has the required property.
\end{proof}
...
\end{rem}
\begin{proof}
    Since $(I\cap J) \sqsubseteq I\cdot R_S$ and $(I\cap J) \sqsubseteq J\cdot R_S$, $(I\cap J) \cdot R_S\subseteq I\cdot R_S$ and $(I\cap J)\cdot R_S \subseteq I\cdot R_S$, proving $(I\cap J) \cdot R_S \subseteq I\cdot R_S \cap J\cdot R_S$. Conversely, let $\rho \in (I \cdot R_S)\cap (J\cdot R_S)$. Since $\rho\in I\cdot R_S$, $\rho = \frac{i}{s}$ where $i\in I$ and $s\in S$. Since $\rho\in J\cdot R_S$, $\rho = \frac{j}{t}$ where $j\in J$ and $t\in S$. Since $\frac{i}{s}=\frac{j}{t}$ in $R_S$, there is $\sigma \in S$ such that $\sigma \cdot i\cdot t = \sigma \cdot j \cdot s$. Then $\rho = \frac{\sigma \cdot i\cdot t}{\sigma \cdot s\cdot t} = \frac{\sigma \cdot j\cdot s}{\sigma \cdot s\cdot t}\in (I\cap J)\cdot R_S$.
\end{proof}



\section{The module of Kähler differentials}
\begin{defi}[Derivations]\lbl{def:derivation}
    Let $A$ be a ring, $M$ an $A$-module, $d: A\to M$  a homomorphism of the additive group. We say that $d$ is a \defemph{derivation} of $A$ with values in $M$ if the \emph{Leibniz rule} $d(a\cdot b) = b\cdot d(a) + a\cdot d(b)$ holds.
    ...
    \end{alphanumerate}
    Let $\Der(A,M)$ denote the set of derivations with values in $M$. This is an $A$-module by $(a\cdot d)(b) = a\cdot d(b)$.
...
\begin{rem*}
    ...
    \begin{alphanumerate}
        \item 
            $d(1) =0$ as $d(1) = d(1\cdot 1) = d(1) + d(1)$.
        \item
            Assuming $d(r\cdot a) = r\cdot d(a)$ we obtain $d(r) = d(r\cdot 1) = 0$.
        \item 
            Assuming $d(r) = 0$ we obtain $d(a\cdot r) = a\cdot d(r) + r\cdot d(a) = r\cdot d(a)$.
    \end{alphanumerate}
\end{rem*}
