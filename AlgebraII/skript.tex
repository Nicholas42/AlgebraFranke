...
\end{defi}
\begin{rem*}
    When $\Spec R$ is disconnected, there may be $R$-modules $M$ which are locally free but not of a rank $d$ for any fixed $d$. In this situation, there is a locally constant function $d$ on $\Spec R$ such that $M$ is locally free of rank $d(\pp)$ at every $\pp\in \Spec R$.
\end{rem*}
\begin{proof}
	Indeed, suppose that $f,g\in R\setminus\pp$ such that $M_f\simeq R_f^d$ and $M_g\simeq R_g^e$ for some natural or cardinal numbers $d,e$.  Denote $R_{f,g}$ and $M_{f,g}$ the localizations of $R$ respectively $M$ by the multiplicative set generated by $f$ and $g$, then
	\begin{align*}
	R_{f,g}^d=(R_f\otimes _RR_g)^d=R_f^d\otimes_R R_g\simeq M_f\otimes_RR_g=M\otimes_RR_{f,g} =M_{f,g}
	\end{align*}
	and similarly $R_{f,g}^e\simeq M_{f,g}$, proving that indeed $d=e$. In particular, in the above situation the rank function $d(\pp)$ is well-defined and constant on the open set $\Spec R\setminus V(f)$ for any $f$ such that $M_f$ is free over $R_f$, hence $d$ is indeed locally constant.
\end{proof}
...
\begin{prop}\lbl{prop:locallyFreeDim}
    ...
    \begin{alphanumerate}
        \item 
            For all $x\in X$, $\dim(M/\mm_x M)=n$ where $\mm_x$ is the maximal ideal of $\Oo_{X,x}$.
        \item 
...
\begin{lem}
    Let $R$ be a ring, $M$ a finitely generated $R$-module, $\pp\in \Spec R$. If $\mu_1,\ldots,\mu_k\in M$ are such that their images in $M_\pp/\pp\cdot M_\pp$ generate this $\KK(\pp)$-vector space, then there is $f\in R\setminus \pp$ such that their images generate $M_f$ as an $R_f$-module.
\end{lem}
\begin{proof}
...
\begin{proof}[Proof of Proposition \reff{prop:locallyFreeDim}]
    It is clear that \itememph{b} implies \itememph{a}. Let $R=\Oo(X)$, if \itememph{b} holds, then for any $\mm_x$ as in \itememph{a} there are $\mu_1,\ldots,\mu_n\in M_f$, for some $f\in R\setminus \mm_x$ which freely generate $M_f$ as an $R_f$-module. Then their images in $M/\mm_x M$ generate this as a $\KK(\mm_x)$-vector space and from 
    \begin{align*}
    	\KK(\mm_x)^n=(R/\mm_xR)^n=(R/\mm_xR)_f^n=(R_f/\mm_xR_f)^n\simeq M_f/\mm_xM_f=(M/\mm_xM)_f=M/\mm_xM
    \end{align*}
    we conclude that the images of $\mu_1,\ldots,\mu_n$ then indeed must form a basis. Here we used some well-known facts about the behaviour of quotients under localizations (cf. \cite[Proposition~2.3.2\itememph{e}]{alg1}) and $R/\mm_xR=(R/\mm_xR)_f$ and $(M/\mm_xM)_f=M/\mm_xM$ since $f$ is already invertible in $\KK(\mm_x)$.
    
    Let \itememph{a} be satisfied, $\pp\in \Spec R$, $\mm = \mm_x$ any maximal ideal containing $\pp$. Applying \itememph{a} at $x$ we obtain, $\mu_1,\ldots,\mu_n\in M$ such that their images in $M/\mm_x M$ form a base of that $\KK(\mm_x)$-vector space. By applying the previous lemma, there is $f\in R\setminus\mm_x\subseteq R\setminus \pp$ such that the $\mu_i$ generate $M_f$ as an $R_f$ module. We claim that they do so freely. Suppose that 
    \begin{align*}
   ...
   Without losing generality let $r_1 \neq 0$. As $X$ is irreducible, there is $y\in X\setminus\big(V(f)\cup V(r_1)\big)$. Then the $\mu_i$ generate $M/\mm_y M$ because they generate $M_f$ as an $R_f$-module and $f(y)\neq 0$. But we have
    ...
    \end{align*}
    in $M/\mm_y M$ with the first coefficient $r_1(y) \neq 0$. Hence the images of $\mu_2,\ldots,\mu_n$ already generate $M/\mm_y M$, hence $\dim(M/\mm_y M) <n$, which is a contradiction to \itememph{a}.
\end{proof}
\begin{rem*}
    \begin{alphanumerate}
      \item 
        Since each prime ideal $\pp$ in \reff{def:locallyFree} is contained in a maximal ideal, it is sufficient to require $M$ being locally free at every maximal ideal.
      \item 
        A generalization of Proposition \reff{prop:locallyFreeDim} would be
        \begin{quote}
            If $M$ is a finitely generated $R$-module and $\dim_{\KK(\pp)}(M_\pp/\pp M_\pp) = \dim_{\KK(\pp)} M \otimes_R \KK(\pp)$ equals $\ell$ for all $\pp\in \Spec R$ then $M$ is locally free of rank $\ell$.
        \end{quote}
        However, if $R$ is such that $\#\Spec R=1$ but fails to be a field (e.g. $R=\IZ/\pp^2\IZ$ for $p$ prime) then there are modules which are finitely generated but not free (e.g. $R/\mm$ where $\mm$ is the only maximal ideal), hence not locally free (since $R\setminus \mm\subseteq R^\times$). But the function $\pp\morphism \dim_{\KK(\pp)}(M_\pp/\pp M_\pp)$ has no choice but to be constant as $\#\Spec R = 1$. However the proof of Proposition \reff{prop:locallyFreeDim} works if $R$ has no nilpotent elements.
      \item 
        The closest analogue to Proposition \reff{prop:locallyFreeDim}:
        \begin{quote}
            If $M$ is a finitely generated $R$-module and $\dim_{\KK(\mm)} (M/\mm M) = \dim_{\KK(\mm)} (M\otimes_R \KK(\mm)) = \ell$ for any maximal ideal $\mm$, then $M$ is a locally free $R$-module of rank $\ell$.
        \end{quote}
        but this fails unless $R$ is, in addition to $\nil(R) = \{0\}$, a \emph{Jacobson ring}: The maximal ideals of $R$ form a dense subset of every closed subset of $\Spec R$. For instance, algebras of finite type over $\IZ$ or any field or any PID with infinitely many prime ideals may be taken (when $\nil(R) = \{0\}$), but not local rings which are not fields (as $R/\mm$ is counterexample).

    \end{alphanumerate}
\end{rem*}

\begin{prop}
    Let $X$ be an affine algebraic variety of dimension $n$ over the algebraically closed field $k$. Then the following conditions are equivalent:
    \begin{alphanumerate}
        \item 
            $X$ is regular.
        \item 
...
\begin{rem*}
    \begin{enumerate}
      \item 
        ...
        \begin{itemize}
          \item    
            $(\Omega_{R/k})_f \simeq \Omega_{R_f/k}$
          \item 
            $X\setminus V(f)$ is an affine open neighbourhood of $x$ and $\Oo(X\setminus V(f)) \lisomorphism R_f$.
        \end{itemize}
      \item 
        It turns out (in the next section) that $\Omega_{R/k}$ is locally free of rank $\dim(X)$ if it is locally free at all. 
      \item 
...
\end{lem}
\begin{proof}
    We have $A\simeq B/I$ with $B=R[X_1,\ldots,X_n]$. We have seen that $\Omega_{B/R} \simeq B^n$ is finitely generated. But
    \begin{align*}
        I/I^2 \morphism \Omega_{B/R}/(I\Omega_{B/R}) \morphism \Omega_{A/R} \morphism 0
    \end{align*}
    expresses $\Omega_{A/R}$ as a quotient of $\Omega_{B/R}/(I\Omega_{B/R})$ which is finitely generated over $B/I\simeq A$. Hence $\Omega_{A/R}$ is finitely generated over $A$.
\end{proof}

\begin{prop}
    Let $x\in X$. Let $X$ be realized as a closed irreducible subset $V(I)\subseteq k^m$, where $I$ is a prime ideal in $S= k[X_1,\ldots, X_m]$. Then $R\simeq S/I$ and we have a short exact sequence
    \begin{align*}
        I/I^2 \morphism \Omega_{S/k}/(I\Omega_{S/k}) \morphism \Omega_{R/k} \morphism 0,
    \end{align*}
    where $\Omega_{S/k}/(I\Omega_{S/k}) \simeq R^n$. Taking tensor products over $R$ with $k(x)=k$ gives a sequence
    \begin{align*}
        (I/I^2)\otimes_R k \morphism R^n\otimes_R k \morphism \Omega_{R/k} \otimes_R k \morphism 0,
    \end{align*}
    which is pointwise isomorphic to 
    \begin{align*}
        I/(\mm I +I^2) \morphism[\phi] k^n \morphism \Omega_{R/k}/(\mm_x\Omega_{R/k} )\morphism 0,
    \end{align*}
    where $\phi\colon f\mapsto (\nabla f)(x)$. By a fact about the functor $-\otimes_R M$ (namely that it is \emph{right exact}, meaning that it commutes with cokernels) the sequence stays exact. Thus $\Omega_{R/k}$ being locally free is equivalent to $\dim_k(\Omega_{R/k}\otimes k(x)) = n$ for all $x\in X$. This is equivalent to $\dim_k(\text{image } (I/I^2)\otimes_R k(x) \morphism k^n) = \dim_k(\left\{(\nabla f)(x) \st f\in I\right\}) = m-n$ for all $x\in X$. So it is equivalent to $X$ being regular.
\end{prop}
\begin{rem*}
    The Lemma \reff{lem:exactSequenceTensor} was used.
\end{rem*}
...
\begin{prop}
    If $L/K$ is a separable (algebraic) field extension, $\Omega_{L/K} = 0$.
\end{prop}

\begin{prop}
    If $L/K$ is an extension of fields of characteristic 0 and $B\subseteq L$ a transcendence base then $\left\{ d_{L/K} b\st b\in B\right\}$ is a base of the $L$-vector space $\Omega_{L/K}$ (and $b\in B$ uniquely determined by $d_{L/K} b$). In particular, the extension is algebraic if and only if $\Omega_{L/K}$ vanishes.
\end{prop}
\begin{rem*}
    If $K = k$, $L = K(X)$ (the field of rational functions, where $X$ is an affine algebraic variety of dimension $n$ over $k$) this implies that $\Omega_{K(X)/k} \simeq \Omega_{R/k} \otimes_R K \simeq (\Omega_{R/k})_{R\setminus\{0\}}$ (note that $\Omega$ commutes with localization) has dimension $\deg\tr(K(X)/k) = \dim X$. Hence $\Omega_{R/k}$ must be locally free of rank $\dim(X)$ if it is locally free at all.
\end{rem*}
...
\begin{align*}
    (M\otimes_R A)\otimes_A(A\otimes_R N)&\isomorphism (M\otimes_R N) \otimes_R\\
...
\begin{lem}\lbl{lem:exactSequenceTensor}
    Let $R$ be a commutative ring and 
    \begin{align*}
        X'\morphism X\morphism X''\morphism 0
    \end{align*}
    is an exact sequence of $R$-modules and $M$ is any $R$-module. Then 
    \begin{align*}
        X'\otimes_R M\morphism X\otimes_R M \morphism X''\otimes_R M \morphism 0
    \end{align*}
    stays exact.
\end{lem}
\begin{proof}
    The first sequence is exact iff $X''\simeq \coker(X'\morphism X)$ and we can show that 
    \begin{align*}
        \coker(X'\otimes_R M \morphism X\otimes_R M)  &\lisomorphism \coker(X'\morphism X) \otimes_R M
    \end{align*}
    by composing universal properties.
\end{proof}
\printbibliography
\end{document}          
