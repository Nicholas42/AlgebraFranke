...
\begin{proof}[Proof of Proposition~\reff{prop:Akizuki-Hopkins}]
    ...
    As $\nil(R)^n=\{0\}$, this shows that $\ell(R) <\infty$ (again by Remark~\reff{rem:ArtinSchreier}\itememph{g}). In particular, $R$ must be Noetherian and this finishes our proof.
\end{proof}
...
\begin{prop}
    ...
    \item 
        If $R=k[X_0,\ldots, X_n]$ and $M$ is a finitely generated graded $R$-module with a filtration $0=M_0\subsetneq M_1\subsetneq\ldots\subsetneq M_\ell$ with $M_i/M_{i-1}\simeq (R/\pp_i)[t_i]$ as in Proposition~\reff{prop:filtrationByPrimeIdeals}, then the degree of the Hilbert polynomial $P_M$ equals
        \begin{align*}
            \max\left\{\dim\big(V(\pp_i)\big)\st 1\leq i\leq \ell\right\}
        \end{align*}
...
\begin{proof}[Proof of Proposition~\reff{prop:HilbertDegreeEqualsDimension}]
    The previous remark shows that it suffices to show \itememph{a} restricted to the case $M=R/\pp$. We use induction on $n$. When $n=0$, $\pp=R_+$ (in which case $P_M=0$ and $\dim(V(\pp)) = -\infty$) or $\pp=0$ (in which case $P_M=1$ and $\dim(V(\pp)) = 0$). Thus, the assertion is true in this case.
    
    %The proof this lecture looks somewhat different
    
    Induction on $d=\max\left\{\dim(V(\pp_i))\st 1\leq i \leq \ell\right\}$. By the additivity of $P_M$ on short exact sequences and because the leading coefficients are positive, we have $\deg P_M = \max\left\{\deg P_{M_i/M_{i-1}}\st 1\leq i\leq \ell\right\}$ and it is sufficient to deal with $M=R/\pp$. When $d=-\infty$, we have $\pp=R_+$ by the projective Nullstellensatz and $P_M=0$ as $M$ is conzentrated(?) in degree 0 and both sides are $-\infty$. As $R_+$ is generated by $R_1$ as an ideal, it follows that there is $X\in R_1\setminus \pp$. As $\pp$ is prime, the sequence
    \begin{align*}
        0 \morphism M[-1] \morphism[\cdot X] M \morphism M/X\cdot M \morphism 0
    \end{align*}
    is exact. Thus $P_M(t) -P_M(t-1) = P_N$ by the additivity of the Hilbert polynomial, where $N = M/X\cdot M $. Let $0 = N_0\subsetneq \ldots \subsetneq N_m = N$, $N_i/N_{i-1} \simeq R/\qq_i[t_i]$, $\qq_i\in \Spec R$ homogeneous, be a filtration as in Proposition~\reff{prop:filtrationByPrimeIdeals}. By Fact~\reff{fact:AnnPrimeCap}, $\bigcup_{i=1}^m V(\qq_i) = V(\Ann_R(N))$ and $\Ann_R(N) = \pp+ X\cdot R$ as $N\simeq R/(\pp+X\cdot R)$. Thus, $\bigcup_{i=1}^m V(\qq_i) = V(\pp)\cap V(X)$ is a proper subset of $V(\pp)$. It follows that the induction assumption may be applied to $N$ and it follows that $\deg P_N = \max\left\{\dim(V(\qq_i))\st 1\leq i \leq m\right\} < d$. If $d=0$, it follows that $\deg(P_N) = -\infty$ and $P_N=0$ and $P_M$ is constant. On the other side, $P_M = 0$ would imply the vanishing of $M=R/\pp$ in large homogeneous degrees and $\pp= \sqrt{\pp} \supseteq R_+$ and $d=-\infty$. Thus, $P_M$ is a non-zero constant and of degree $0$. Otherwise, the irreducible components of $V(\pp)\cap V(X)$, which have dimension $d-1$ by Theorem~\reff{thm:projectiveIntersection}, must be among the $V(\qq_i)$, hence $\max\left\{\dim(V(\qq_i))\st 1\leq i\leq m\right\} = d-1$ and $\deg P_N = d-1$. By Fact(can't find the right number, it isn't 2), $P_N(t) = P_M(t) - P_M(t-1)$ implies $\deg P_M =d$.
\end{proof}
\begin{lem}[a.k.a. Lemma~1] \lbl{lem:newtonInterpolation}
    Let $P\in \IQ[T]$ be of degree $d\in\IN$ such that $P(k)\in \IZ$ for all sufficiently large $k\in\IZ$. Then $P(k) \in \IZ$ for all $k\in \IZ$ and 
    \begin{align*}
        P(t) = \sum_{k=0}^d \snake p_k \binom tk
    \end{align*}
    where $\snake p_k = (\Delta^kP)(0)$ (with $(\Delta P)(t) = P(t)-P(t-1)$, $\Delta^k P = \Delta \Delta^{k-1} P$). In particular, $\snake p_k\in\IZ$ for all $k$ and $d!\cdot p_d = \snake p_d$ where $P(T) \sum_{i=0}^d p_iT^i$.
\end{lem}
\begin{proof}
    Let $P(T) = \frac{Q(T)}{N}$ where $Q\in \IZ[T]$ and $N$ is a positive integer. Then $Q(k)\mod N$ only depends on $k\mod N$, hence vanishes for all $k\in \IZ$ if it does for sufficiently large $k\in \IZ$. Thus $P(k)\in \IZ$ for all $k\in \IZ$. The formula for $\snake p_k$ is the Newton interpolation formula and follows from 
    \begin{align*}
        \Delta \binom tk = \begin{cases} \binom t{k-1} & k>0\\ 0 &k=0 \end{cases}
    \end{align*}
    together with $\binom 0k = 0$ for $k>0$.
\end{proof}
\begin{defi}
    Let $Z= V(\pp) \subseteq \IP^n(k)$ be a projective algebraic variety of dimension $d$. The \defemph{degree} of $Z$ is $d!\cdot p_d$ where $P(t) = \sum_{i=0}^d p_i T^i$ is the Hilbert polynomial of $Z$.
\end{defi}
\begin{example}
    \begin{itemize}
      \item 
    ...
    \item 
        Let $Z= V(P)$ where $P\in R = k[X_0,\ldots, X_n]$ be an irreducible polynomial of degree $d$. Then $S_\bullet(Z) = R/P\cdot R$ and by the short exact sequence
        \begin{align*}
            0\morphism[-d]\morphism[\cdot P] R\morphism S_\bullet (Z) \morphism 0
        \end{align*}
        we have $P_R(T) - P_R(T-d) = P_{S_\bullet(Z)}(T)$. But $P_R(T) = \binom{T+n}n$ hence $P_Z = \frac{T^n-(T-d)^n}{n!} + \text{ lower powers } = \frac{d\cdot nT^{n-1}}{n!} + \text{ lower powers}$ and $\deg Z = (n-1)! \frac{n\cdot d}{d!} = d$. Hence $\deg Z = \deg P$ when $Z=V(P)$ where $P$ is homogeneous irreducible polynomial, motivating the term \emph{degree}
    \end{itemize}
\end{example}
\begin{rem*}
...
\end{rem*}
\begin{rem*}
    Bezouts Theorem in the general from states $\deg Z\deg P = \sum_{i=1}^\ell i(Z,P, Y_i) \cdot \deg(Y_i)$ if $Z\subseteq \IP^n(k)$ is a projective algebraic variety of positive dimension, $P\in k[X_0,\ldots, X_n]$ a homogeneous polynomial not identically vanishing on $Z$, $Z\cap V(P) = \bigcup_{i=1}^\ell Y_i$ is the decomposition into irreducible components and $i(Z,P, Y_i)$ the \emph{intersection multiplicity} of $Z$ with $V(P)$ in $Y_i$.
\end{rem*}
...
\begin{prop}
    ...
    \item  \lbl{prop:gradedLocalization}
        Let $R$ be a $\IZ$-graded ring and $S\subseteq R$ a multiplicative set consisting of homogeneous elements. Then there is a unique (up to unique isomorphism) graded ring $R_S$ with a morphism $R \morphism[\lambda] R_S$ of graded rings such that $\lambda(S) \subseteq R_S^\times$ and such that the universal property for morphisms $R \morphism[\tau] R$ of graded rings with $\tau(S) \subseteq T^\times$ holds: For any such $\tau$, there is a unique morphism $R_S \morphism[t] T$ such that $\tau = \lambda t$. As an ungraded ring, $R_S$ is the ordinary localization and $(R_S)_d = \left\{\frac{r}{s} \st s\in S\cap R_e, r\in R_{d+e}\right\}$. Under the bijection 
        \begin{align*}
            \Spec(R_s) &\simeq  \left\{\pp\in \Spec R\st  \pp\cap S\right\}\\
            \pp &\morphism \pp \cdot R_s\\
            \qq\sqcap R &\lmorphism \qq
        \end{align*}
    \item 
    ...
    \item
        For every graded $R$-module $M$ there is a unique (up to unique isomorphism) $R_S$-module $M_S$ with a morphism $M\morphism[\mu] M_S$ of graded $R$-modules, which has has the universal property for morphisms $M\morphism[\gamma] N$ of graded $R$-modules $M$ to graded $R_S$-modules: For every such $\gamma$, there is a unique morphism $M_S\morphism[n] N$ such that $\gamma = n\mu$. As an ungraded module, $M_S$ is the ordinary localization, and the grading is 
        \begin{align*}
            (M_S)_d = \left\{\frac{m}{s}\st s\in S\cap R_e, r\in M_{e+d}\right\}\;.
        \end{align*}
        The functor $M\morphism M_S$ commutes with kernels of morphisms, quotients, $\bigoplus$ and $[t]: M[t]_S \simeq M_S[t]$.
    \end{alphanumerate}
\end{prop}
\begin{proof}
    Tedious calculation.
