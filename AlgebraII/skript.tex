...
\begin{proof}
    ...
    Let $\eta\in \Oo^\ell(X\setminus V(f))$ (the case $\ell=0$ corresponds to the study of $\Oo(Z\setminus V(f))$), where $f\neq 0$ is homogeneous of degree $\delta >0$. By Definition~\reff{def:regularFunction}, every $z\in Z$ has an open neighbourhood $U_z$ such that there are polynomials $p_z,q_z\in R=k[X_0,\ldots, X_n]$ where $q_z$ is homogeneous of degree $b_z$, $p_z$ of degree $a_z = b_z+\ell$ and $V(q_z)\cap U_z = \emptyset$ and such that 
    \begin{align*}
...
\begin{defi*}[Graded submodule]\lbl{def:gradedSubmodule}
    \begin{alphanumerate}
        \item
            If $N\subseteq M$ is a graded submodule of a graded $R$-module $M$ (i.e. for $n\in N$, the decomposition $n=\sum_{i=-\infty}^\infty h_i$ into elements of $M$ is a decomposition into elements of $N$.), then $M//N$ is the graded quotient with the grading $(M/N)_d) = M_d/N_d$.
        \item   
            $M[k]_\ell \coloneqq M_{k+l}$, $M[k]$ is called the \emph{shift} of $M$ by $k$.
    \end{alphanumerate}
\end{defi*}
...
\begin{rem}
    \begin{alphanumerate}
    \item 
        By a theorem of Artin-Schreier, the number $\ell$ is determined uniquely by $M$, as are the isomorphism classes of $M_i/M_{i-1}$ up to permutation.
    \item 
        The length of a $k$-vector space is its dimension. 
    \item
        The length of $\IZ/N\IZ$ is the number of prime factors of $N$ with respect to their multiplicity.
    \item 
    ...
    \item 
        If $0\morphism M' \morphism M\morphism M'' \morphism 0$ is a short exact sequence ten $\ell(M) = \ell(M') +\ell(M'')$.
    \end{alphanumerate}
\end{rem}
...
\begin{prop}
    ...
    \item  
        If $R$ and $M$ are graded ($R$ being $\IN$-graded), we may chose this filtration in such a way that the $M_i$ are graded submodules and the $\pp_i$ homogeneous and $M_i/M_{i-1} \simeq (R/\pp_i)[t_i]$, where $t_i\in \IN$ and the isomorphism respects the grading.
    \end{alphanumerate}
\end{prop}
\begin{lem}
    Let $M\neq\{0\}$ be a module over a Noetherian ring,  then it contains a submodule isomorphic to $R/\pp$ for some $\pp\in \Spec R$.
\end{lem}
\begin{proof}
    The set of ideals $\left\{\Ann_R(m) = \left\{r\in R\st r\cdot m = 0\right\} \st m\in M\setminus \{0\}\right\}$ in $R$ has a $\subseteq$-maximal element, as $R$ is Noetherian. Let $m\neq 0$ and $\Ann_r(m) = \pp$ is maximal. Then the submodule $R\cdot m \subseteq M$ is isomorphic to $R/\pp$. We have $1\not\in \pp$ as $m\neq 0$. Let $ab\in \pp$. If $a\not\in\pp$, then $a\cdot m \neq 0$ and $\Ann_R(a\cdot m)\supsetneq \Ann_R(m)$, and equality mus occur by our choice of $m$. Thus $b\in \Ann_R(a\cdot m)$ implies $b\in \Ann_R(m)$, and $\pp$ is prime.
\end{proof}
\begin{proof}[Proof of Proposition~\reff{prop:filtrationByPrimeIdeals}]
    By our assumption, $M$ is Noetherian. Hence the set $\MM$ of submodules of $M$ with such a filtration has an $\subseteq$-maximal element $M'$. If $M'\neq M$ we could find $Q\subseteq M/M'$ which is isomorphic to $R/\pp$. The the preimages of $Q$ in $M$ has a filtration of the desired kind.
\end{proof}
\begin{rem*}
    \begin{alphanumerate}
    \item 
    ...
    \item 
        In particular, $M$ in Proposition~\reff{prop:filtrationByPrimeIdeals} is of finite length iff the $\pp_i$ are maximal. In this case, the length of the filtration equals $\ell(M)$ and the $\pp_i$ are unique up to permutation. Otherwise, the length of the filtration may be non-unique, e.g. $\{0\}\subsetneq \IZ$ or $\{0\}\subsetneq 2\IZ \subsetneq \IZ$ when $M=R=\IZ$.
    \end{alphanumerate}
\end{rem*}
\begin{proof}[Proof of Proposition~\reff{prop:Akizuki-Hopkins}]
    Given the Krull dimension is $0$, the prime ideals are maximal an so $R/\pp_i$ is simple and has finite length by Proposition~\reff{prop:filtrationByPrimeIdeals}.
    
    Let $R$ be of finite length as an $R$-module. We use induction on the number of generators of $M$ to prove \itememph{d}. If $M$ is generated by one element, it is isomorphic to $R/I$ and its length is smaller or equal to $\ell(R)$. If it is generated by $m_1,\ldots, m_k$ and the assertion is known for $k-1$ generators, then, if $M'\subseteq M$ denotes the submodule generated by $m_1,\ldots, m_{k-1}$, $\ell(M') < \infty$ by the induction assumption and $M/M'$ is generated by the image of $m_k$, hence has finite length by the previous consideration. 
    
    The length of the members of such sequence as in \itememph{e} form a strictly decreasing sequence. Hence the length of the sequence is bounded by $\ell(M)$ assuming that $M$ is of finite length, which is given by \itememph{d}.
    ...
    \begin{lem}[Prime avoidance]\lbl{lem:primeAvoidance}
        Let $\pp_1,\ldots, \pp_r$ be ideals of a ring $R$ such that there at most two among them who are not prime. If $I$ is an ideal of $R$ not contained in an $\pp_i$, then it is also not contained in the union of all the $\pp_i$. Professor Franke does not want to proof this here and recommends the appropriate chapter in a book by Eisenbud. You can also find a proof by Franke himself in \cite[Lemma~2.5.1]{alg1}.
    \end{lem}
    
    First we prove that any prime ideal is maximal, i.e. the Krull dimension is $0$. If $x\in R/\pp$ and $x\neq 0$, $x^n\cdot(R/\pp)$ would form a strictly descending chain of ideals in $R/\pp$ if $x$ failed to be a unit, so any $x\in R/\pp\setminus\{0\}$ is a unit. If $R$ had infinitely many prime ideals (in other words, infinitely many maximal ideals), there would be a sequence $(\mm_i)_{i=1}^\infty$ of maximal ideals an the sequence
    \begin{align*}
        \mm_1 \supsetneq \mm_1\cap\mm_2\supsetneq \ldots\supsetneq \bigcap_{i=1}^k \mm_i \supsetneq \ldots
    \end{align*}
    is strictly descending, a contradiction to \itememph{c}. ($f_i\in\mm_i\setminus\mm_{k+1}$, $\prod_{i=1}^k f_i \in \bigcap_{i=1}^k \mm_i$ but not in $\mm_{k+1}$).
    
    Now assume there are only finitely many prime ideals $\mm_1, \ldots,\mm_k$ distinct prime ideals of $R$. Then $\nil(R) = \bigcap_{i=1}^k \mm_i$, the sequence $\nil(R)^i \supsetneq \ldots\supsetneq \nil(R)^{i+1}$ mus terminate at some point by \itememph{c}. Thus, let $n\in\IN$ such that $\nil(R)^n = \nil(R)^i$ for all $i\geq n$. Let $N = \nil(R)^n$. If $N\neq 0$, the annihilator $K$ of $N$ would be an ideal of $R$ strictly contained in $R$. From \itememph{c} and the axiom of choice it follows that any set of ideals of $R$ contains a $\subseteq$-minimal element. Thus, there is an ideal $K\subsetneq I\subseteq R$ minimal among all such ideals. If $x\in I\setminus K$ we have $I= K+x\cdot R$ by the minimality of $I$. We have $K\subseteq x\cdot \nil(R) + K \subseteq I$, hence $x\cdot  \nil(R) + K = K$ or $x\cdot \nil(R) +K = I$ by the minimality of $I$. In the second case $(I/K) = \nil(R)\cdot(I/K)$ and $I=K$ by \NAK, a contradiction to our choice of $I$. In the first case, $x\cdot \nil(R) \subseteq K$, hence $x\cdot \nil(R)^{n+1}\subset K\cdot N = 0$. But $\nil(R)^{n+1}= \nil(R)^n = N$, hence $x\in K$. A contradiction to our choice of $I$. Now $\nil(R)^i/\nil(R)^{i+1}$ has a filtration $\nil(R)^{i+1}(\pp_1\cap \ldots\cap \pp_k)$ with simple quotients, hence finite length.
\end{proof}
