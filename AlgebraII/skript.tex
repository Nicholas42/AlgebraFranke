...
\begin{example}
    ...
    \begin{align*}
        ...
        f&\longmapsto ((x_0,\ldots, x_n) \mapsto x_i^\ell f([x_0,\ldots, x_n])
    \end{align*}
    defines an isomorphism of sheaves of abelian groups or $\Oo$-modules by $\Oo|_U \morphism[\cdot x_0^\ell] \Oo^\ell|_U$. Thus, $\Oo^\ell$ is a line bundle on $Y$. 
\end{example}
\begin{prop}
...
\end{prop}
\begin{proof}
    The first part is obvious by Proposition \reff{prop:projectiveAffineTopology}. The comparison of the structure sheaves will be left as an exercise.
\end{proof}
\begin{cor*}
...
\begin{cor}\lbl{cor:tupleRegularContiuous}
    If $f_1,\ldots, f_n\in \Oo(Z)$ then $Z\morphism[(f_1,\ldots, f_n)]\IA^n$.
\end{cor}
\begin{rem*}
...
\begin{defi}[Field of rational functions]
    If $Z$ is any prevariety, let the \defemph{field of rational functions} $K$ be the set of all pairs $(f,U)$ where $U$ is a non-empty open subset, $f\in\Oo_Z(U)$ modulo the equivalence relation $(f,U)\sim (\phi, W)$ iff there is a non-empty $V\subseteq U\cap W$ such that $f|_V = \phi|_V$.
\end{defi}
\begin{rem*}
    \begin{alphanumerate}
        \item 
            By Corollary \reff{cor:restrictionInjective}, $f|_V = \phi|_V$ is equivalent to $f|_{U\cap W} = \phi|_{U\cap W}$. One may think of $K=\bigcup_{U\neq\emptyset,\text{ open}} \Oo(U)$ but of course that would be imprecise. However, by the sheaf axiom and Corollary \reff{cor:restrictionInjective} it follows that for any $f\in K$ there are a largest open subset $U$ and $\phi \in \Oo_Z(U)$ such that $f=(U,\phi)/_\sim$, and $\phi$ is uniquely determined by $f$. This $U$ is called the \emph{domain of definition} of the rational function $f$.
        \item 
            If $Z$ had a point $\eta_Z$ which is dense in $Z$, we would have $K = \Oo_{Z,\eta_Z}$ as any open $U\neq \emptyset$ must contain $\eta_Z$.
        \item
        ...
        \item 
            If $U\subseteq Z$ is non-empty, the fields $K_U$ and $K_Z$ of rational functions on the respective spaces are canonically isomorphic as any $f\in K_Z$ has a representative $(\phi, W)$ with $W\subseteq U$) (if $(\snake\phi, V)$ is any representative of $f$, $W=V\cap U$ and $\phi=\snake \phi|_W$ will do).
    \end{alphanumerate}
\end{rem*}
...
\begin{defi}[Prevariety]\lbl{def:prevariety}
    A \defemph{prevariety} over an algebraically closed field $k$ is Noetherian, irreducible topological space $X$ together with a subsheaf $\Oo_X$ of the sheaf of $k$-valued functions on $X$ such that any $x\in X$ has an open neighbourhood $U$ for which there is a homeomorphism $U\isomorphism[\phi] V\subseteq k^n$ where $V$ is a quasi-affine variety in $k^n$ and such that a $k$-valued function $\lambda$ on the open subset $W$ satisfies $\lambda \in \Oo_X(W)$ if and only if $\phi^*\lambda = (y\mapsto \lambda(\phi(y))$ defines an element of $\Oo(\phi(W))$. 
...
\begin{rem}
    ...
    \begin{alphanumerate}
        ...
        \item 
            For any pair $S\doublemorphism T$ of continuous maps, $K = \Eq(a,b) =\left\{t\in T\st a(t) = b(t)\right\}$ is closed in $T$.
    \end{alphanumerate}
\end{rem}
\begin{defi}[Stalk]\lbl{def:stalk}
    The \defemph{stalk} at $x\in X$ of a presheaf $\Ff$ on the topological space $X$ is $\Ff_x$ is the set of pairs $(U,\alpha)$ of open neighbourhoods $U$ of $x$ and $\alpha\in \Ff(U)$ modulo the equivalence relation $\sim$. This relation is defined by $(U,\alpha)\sim (V, \beta)$ iff there is an open neighbourhood $W\subseteq U\cap V$ of $x$ such that $\alpha|_W = \beta|_W$. The group (ring) operations $\circ\in \{+,\cdot\}$ are given as follows: If $a= (U,\alpha)/_\sim$ and $b= (V,\beta)/_\sim$ are elements of $\Ff_x$, then 
    \begin{align*}
        a\circ b = (W, (\alpha|_W)\circ(\beta|_W))/_\sim
    \end{align*}
...
\end{defi}
\begin{rem}
    Let $X\subseteq k^n$ be quasi-affine and $\Oo$ be its structure sheaf. When $f=(U,\phi)/_\sim\in \Oo_x$, $f(x)\coloneqq \phi(x)$ is independent of its representative. When $f(x)\neq 0$, $f\in \Oo_x^\times$ as $U\setminus V(\phi)$ is an open neighbourhood of $x$ in $U$ and $\phi|_{U\setminus V(\phi)} \in \Oo(U\setminus V(\phi))^\times$. It follows that $\Oo_x\setminus\Oo_x^\times$ is the kernels of the homomorphism $\Oo_x \morphism k$, $f\mapsto f(x)$ an hence an ideal. It follows that $\Oo_x$ is a local ring. As the construction of stalks is local\footnote{Pun not intended}, $\Oo_{X,x}$ is a local ring with maximal ideal $\mm_{X,x} = \left\{f\in \Oo_{X,x}\st f(x) = 0\right\}$ for any prevariety $X$ and any $x\in X$. 
\end{rem}
\begin{rem}
