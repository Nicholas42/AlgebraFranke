...
\end{lem}
\begin{rem*}
	Note that the obvious-seeming stronger relation $\Ann_R(M/IM)=I+\Ann_R(M)$ may fail, even for well-behaved rings. The following counterexample is taken from \url{https://math.stackexchange.com/questions/79538/annihilator-of-quotient-module-m-im}.
...
\end{rem*}
To prove Lemma~\reff{lem:radicalOfAnnihilator}, we need a slight generalization (often regarded a variant) of the Nakayama lemma.
\begin{lem}[cf. Eisenbud, {\cite[Corollary~4.7]{eisenbudCommAlg}}]\lbl{lem:NAKgeneralization}
	Let $R$ be a ring, $I\subseteq R$ an ideal and $M$ a finitely generated $R$-module such that $IM=M$. Then there exists $x\equiv 1\mod I$ such that $xM=0$.
\end{lem}
\begin{proof}
	We mimick the proof of \NAK. Let $m=(m_1,\ldots,m_n)^t$ be a vector of generators of $M$. As $M=IM$, there are $\alpha_{i,j}\in I$ such that $m_i=\sum_{j=1}^n\alpha_{i,j}m_j$. If $\alpha=(\alpha_{i,j})$ denotes the matrix formed by the $\alpha_{i,j}$, we thus have $(\id_n-\,\alpha)\cdot m=0$. Multiplying by the matrix of cofactors of $\id_n-\,\alpha$, we obtain $\det(\id_n-\,\alpha)\cdot m=0$. Thus, for $x=\det(\id_n-\,\alpha)$ we have $xM=0$ and also $x\equiv 1\mod I$ as the reduction modulo $I$ of $\id_n-\,\alpha$ equals the identity matrix.
\end{proof}
\begin{proof}[Proof of Lemma~\reff{lem:radicalOfAnnihilator}]
	It is clear that $I+\Ann_R(M)\subseteq \Ann_R(M/IM)$, hence this inclusion also holds for their radicals.
	
	Conversely, let $f\in\Ann_R(M/IM)$. If $f$ is nilpotent, we certainly have $f\in\sqrt{I+\Ann_R(M)}$. Otherwise consider the localized module $M_f$ over the localization $R_f$. We have $M_f/IM_f\simeq (M/IM)_f$, hence $f\cdot M_f/IM_f=0$. But since $f$ is a unit in $R_f$, this implies $M_f/IM_f=0$ or $M_f=IM_f$. By Lemma~\reff{lem:NAKgeneralization} we find an $x\in R_f$ such that $xM_f=0$ and $x\equiv 1\mod IM_f$.
	
	Now let $m_1,\ldots,m_n$ be generators of $M$ over $R$. If $x=yf^{-s}$ for some $y\in R$ and $s\geq0$, then, as $xm_i=0$ in $R_f$ we find $k_i\geq 0$ such that $f^{k_i}ym_i=0$ in $R$. Then $f^ky\in\Ann_R(M)$ if $k$ is large enough, that is, if $k\geq\max\{k_1,\ldots,k_n\}$. Now $x\equiv 1\mod IM_f$ implies $x-1=af^{-t}$ for some $a\in I$ and $t\geq0$. This is equivalent to $f^{k-s}y-f^{k}=f^{k-t}a$ for $k\geq s,t$ large enough. Combining both, we get $f^k\in I+\Ann_R(M)$ if $k$ is large enough, as we wanted to show.
	
	This shows $\Ann_R(M/IM)\subseteq\sqrt{I+\Ann_R(M)}$. This of course still holds if we take the radical on the left-hand side.
\end{proof}
\begin{rem}[Recall]
    For a Noetherian local ring $R$, its maximal ideal $\mm$ and $k=R/\mm$, we have
    \begin{align*}
        \dim_k(\mm/\mm^2) = \min\left\{n \st \mm \text{ may be generated by } n \text{ elements}\right\}\;.
    \end{align*}
    We have $\dim(R) \leq n$ and $R$ is called \emph{regular} when equality occurs.
\end{rem}
\begin{example*}
    Take $y^2 = x^2+x^4$ over a field of characteristic not equal to $2$. We have $y = \pm x\sqrt{1+x^2}$ and $\sqrt{1+x^2} = 1 + \sum_{i=1}^\infty \frac{x^{2i}}{2^{2i-i}}$. 
...
\end{example*}
One would intuitively expect, that there are always singularities, where irreducible components meet or where there are any non-zero zero-divisors.

\begin{cor}
    Let $R$ be a Noetherian local ring of dimension $d$ with maximal ideal $\mm$.
    \begin{alphanumerate}
    \item 
        If $x_1,\ldots, x_d$ generate $\mm$ then 
        \begin{align*} 
            k[X_1,\ldots, X_d] &\morphism \gr_\mm(R)\\
            X^\alpha &\morphism x^\alpha \mod \mm^{|\alpha| +1}
        \end{align*}
        where $\alpha$ is arbitrary in $\IN_0^d$, is bijective.
    \item 
        $R$ is domain.
    \end{alphanumerate}
\end{cor}
\begin{proof}
    Obviously the homomorphism from \itememph{a} is well-defined and it is surjective, as the $x_i$ generate $\mm$ as an ideal, hence $\left\{x^\alpha\st \alpha\in \IN_0^d, |\alpha| = k\right\}$ generate $\mm^k$ as an ideal. If it failed to be bijective. There would be a homogeneous $P\in k[X_1,\ldots, X_n]$ contained in the kernel. If $H$ denotes the Hilbert polynomial of $k[X_1,\ldots, X_d]/P$ we have $\deg H < d-1$ and $\ell(\mm^\ell /\mm^{\ell+1}) = \dim_k(\mm^\ell/\mm^{\ell+1}) \leq H(\ell)$ for large $\ell$ and it follows that the Samuel polynomial of $R$ has degree smaller then $d$, a contradiction to Theorem~\reff{thm:idealOfDefinition} and $\dim(R) = d$.
    
    Obviously, $R\neq \{0\}$ as it is local. Assume $f,g\in R\setminus\{0\}$ and $fg = 0$. By the Krull intersection lemma there are $k,\ell \geq 0$ such that $f\not\in \mm^{k+1}$, $g\in \mm^{\ell+1}$. The smallest such $k,\ell$ are natural numbers. Then $\ov f = f\mod \mm^{k+1} \neq 0 $ in $\mm^k/\mm^{k+1}$ and $\ov g= g\mod m^{\ell+1}\neq 0$ in $\mm^\ell/\mm^{\ell+1}$ and $fg\mod \mm^{k+\ell+1} = \ov f\ov g$ in $\gr_{k+\ell}(R,\mm)$, and by \itememph{a} and because $k[X_1,\ldots, X_d]$ is a domain this does not vanish, a contradiction.
\end{proof} 
...
\begin{thm}
    ...
    \item 
        $R$ is a filed or one-dimensional Noetherian normal.
    \item 
...
\begin{defi}[Dedekind rings]
    Such $R$ are called \defemph{Dedekind domains} and $\operatorname{Cl} = (\text{fractional ideals}/(K^\times/R^\times)$ is the class group of $R$.
\end{defi}
