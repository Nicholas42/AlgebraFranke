...
\pagenumbering{roman}
 \thispagestyle{plain}
This text consists of unofficial notes on the lecture Algebra~II, taught at the University of Bonn by Professor Jens Franke in the winter term (Wintersemester) 2017/18. 
...
\end{itemize}
The not-particularly-humble authors of these notes want to use this opportunity to recommend
\begin{itemize}
...
\begin{thm}[Krull's principal ideal theorem]\lbl{thm:krullPrincipalIdeal}
    ...
    %done
    Let $R$ be a noetherian ring, $f\in R$, $\pp\in \Spec R$ minimal among all prime ideals containing $f$. Then $\hoehe(\pp) \leq 1$.
\end{thm}
\begin{rem*}
    \begin{alphanumerate}
    \item
...
\end{thm}    
We'll eventually prove Theorem~\reff{thm:krullPrincipalIdeal} and \reff{thm:krullIdeal} in Section~\reff{sec:PROOOFS} (page~\pageref{cor:KrullHeight}) using Hilbert polynomial arguments. The following corollary can be derived in the same way as Corollary~\reff{cor:V(f)codimension1} from Theorem~\reff{thm:krullPrincipalIdeal}, but again we also give a standalone proof.
...
\end{cor}
The derivation from Corollary~\reff{cor:V(f)codimension1} by induction on $r$ is significantly easier then the similar inductive derivation of Theorem~\reff{thm:krullIdeal} from Theorem~\reff{thm:krullPrincipalIdeal} due to the fact that $k^n$ is catenary.
...
\begin{proof}[Proof of Corollary~\reff{cor:kahlerExactSequences}]
	...
	\end{itemize}
	It remains to show exactness. By the unnamed fact from page~\pageref{fact:HomRightExact}, it is sufficient to show that exactness holds after applying the functor $\Hom_B(-,T)$ for any $B$-module $T$ (where in \itememph{a} we have the special case $B=A/I$). Note that 
	\begin{align*}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\end{center}
	\end{itemize}
	 To show that $\iota$ is an isomorphism in \itememph{c}, it is sufficient to show that we have an isomorphism after applying $\Hom_{A_X}(-,T)$ to both sides for every $A_X$-module $T$. This is so because of the unfortunately unnamed fact from page~\pageref{fact:HomRightExact} applied to the sequence $0\morphism \Omega_{A/R}\otimes_AA_X\morphism[\iota]\Omega_{A_X/R_X}\to0$ (which is exact iff $\iota$ is an isomorphism). But
	 \begin{align*}
...
\begin{proof}
	...
	\end{align*}
	The left-most arrow is obtained by applying $d_{R/k}$ to a representative $\mu\in\mm^2$ of $(\mu\bmod\mm^3)$ and we know that $\mm\morphism\Omega_{R/k}\otimes_R\KK(\mm)$ factors over $\mm^2$ (becoming the morphism we would like to show is injective). Hence the left-most arrow is the $0$-morphism, which proves $\Omega_{R/k}\otimes_R \KK(\mm)\cong\Omega_{(R/\mm^2)/k}\otimes\KK(\mm)$ and we may indeed replace $R$ by $R/\mm^2$, thus assuming $\mm^2=0$.
...
\end{proof}
	...
	Now assume that $\Omega_{A/k}$ is locally free at $\pp$ and $A$ is reduced or $\cha k=0$. We will show that the rank of $\Omega_{A/k}$ at $\pp$ is necessarily equal to $\dim(A_\pp)+\trdeg (\KK(\pp)/k)$. To do this, let $\qq$ be a minimal prime of $A$ which is contained in $\pp$ and such that $\qq A_\pp$ is part of  a maximal ascending chain of prime ideals in $A_\pp$ (i.e., to find $\qq$ we choose such a maximal chain in $A_\pp$ and take the preimage of the smallest prime ideal in that chain). Then $\dim(A_\pp)=\dim(A_\pp/\qq A_\pp)$ and our goal is to reduce everything to $A/\qq$. This is done in two steps.
	
	\emph{Step 1.} We first show that $\qq/\qq^2=0$. To do so, we'll give an argument that works both when $A$ is reduced and when $\cha k=0$ -- but for different reasons, so we start with some case work.
	
	\emph{Case 1.} If $A$ is reduced, so is the localization $A_\qq$. Hence from $\dim(A_\qq)=\hoehe(\qq)=0$ we deduce that $A_\qq$ is a field. Then $A_\qq\cong A_\qq/\qq A_\qq$ can be no other than the quotient field $\KK(\qq)$ of $A/\qq$ and we have $\dim_{\KK(\qq)}\Omega_{\KK(\qq)/k}=\trdeg(\KK(\qq)/k)$ by Proposition~\reff{prop:OmegaL/K}\itememph{b}.
	
	\emph{Case 2.} Now suppose that $\cha k=0$ and let $x\in \qq A_\qq$. Since $\qq A_\qq$ is the only prime ideal of $A_\qq$, every element $x\in \qq A_\qq$ is nilpotent by Proposition~\reff{prop:nilradicalCapPrimeIdeals}. Choose $n$ such that $x^n=0$ but $x^{n-1}\neq 0$. Denoting $d_{A_\qq/k}$ the derivation associated to $\Omega_{A_\qq/k}$, we have $0=d_{A_\qq/k}x^n=nx^{n-1}d_{A_\qq/k}x$ by the Leibniz rule. Since $k$ has characteristic $0$, $n$ is invertible in $k$ and thus also in the $k$-algebra $A_\qq$. Hence $x^{n-1}d_{A_\qq/k}x=0$. Also $\Omega_{A_\qq/k}=(\Omega_{A_\pp/k})_{\qq A_\pp}$ is a free $A_\qq$-module since $\Omega_{A_\pp/k}$ is a free $A_\pp$-module. Hence we must have $d_{A_\qq/k}x\in \qq\Omega_{A_\qq/k}$ and therefore $d_{A_\qq/k}$ descends to a morphism
	\begin{align*}
		\ov{d_{A_\qq/k}}\colon \KK(\qq)\morphism \Omega_{A_\qq/k}/\qq\Omega_{A_\qq/k}\;.
	\end{align*}
	Clearly $	\ov{d_{A_\qq/k}}$ is a $k$-linear derivation, hence induces a morphism $\Omega_{\KK(\qq)/k}\morphism[\delta] \Omega_{A_\qq/k}/\qq\Omega_{A_\qq/k}$ by the universal property of $\Omega_{\KK(\qq)/k}$. It's easy to check that $\delta$ is left-inverse to the canonical morphism $\Omega_{A_\qq/k}/\qq\Omega_{A_\qq/k}\morphism \Omega_{\KK(\qq)/k}$ (indeed, their composition must be $\id_{\Omega_{\KK(\qq)/k}}$ by the universal property of $\Omega_{\KK(\qq)/k}$). Thus $\Omega_{A_\qq/k}/\qq\Omega_{A_\qq/k}\morphism \Omega_{\KK(\qq)/k}$  is injective and it follows that 
	\begin{align*}
		\dim_{\KK(\qq)}\Omega_{A_\qq/k}/\qq\Omega_{A_\qq/k}\leq \dim_{\KK(\qq)}\Omega_{\KK(\qq)/k}\;.
	\end{align*}
	...
	\emph{Step 2.} We can finish the reduction to $A/\qq$ as follows. From the sequence
	\begin{align*}
		\qq/\qq^2\morphism \Omega_{A/k}\otimes_A A/\qq\morphism\Omega_{(A/\qq)/k}\morphism 0
	\end{align*}
	we thus get $\Omega_{A/k}\otimes_A A/\qq\cong\Omega_{(A/\qq)/k}$. Hence $\Omega_{A/k}$ being locally free at $\pp$ implies that $\Omega_{(A/\qq)/k}$ is locally free of the same rank at $\pp/\qq$.
	
	Hence, by Lemma~\reff{lem:dimConditionForFreeness} we see that \eqreff{eq:CondForLocFree} holds for $A/\qq$ and its quotient field $\KK(\qq)$ instead of $A$ and $K$. By Proposition~\reff{prop:OmegaL/K}\itememph{b} and \cite[Theorem~10]{alg1},
	\begin{align*}
		\dim_{\KK(\qq)}\left(\Omega_{(A/\qq)/k}\otimes_A\KK(\qq)\right)&=\dim_{\KK(\qq)}\Omega_{\KK(\qq)/k}=\trdeg (\KK(\qq)/k)
		=\dim(A/\qq)\\
		&=\dim(A_\pp/\qq A_\pp)+\trdeg(\KK(\pp/\qq)/k)\;.
	\end{align*}
	But $A_\pp$ and $A_\pp/\qq A_\pp$ have the same dimension (by construction of $\qq$) and the same residue field $\KK(\pp)=\KK(\pp/\qq)$, hence the last line of the above equation equals $\dim(A_\pp)+\trdeg(\KK(\pp)/k)$. This shows that equality holds in \eqreff{eq:dimOmega}, proving that $A_\pp$ is indeed regular.
\end{proof}
...
\end{defi}
\begin{proof}[You know you are attending a Franke lecture when a definition gets proved]
...
\section{The Samuel polynomial and the principal ideal theorem}\lbl{sec:PROOOFS}

Let $R$ be a noetherian ring, $M$ a finitely generated $R$-module. By Proposition~\reff{prop:filtrationByPrimeIdeals},  $M$ has a filtration
\begin{align}
    0 = M_0\subsetneq M_1\subsetneq\ldots \subsetneq M_k = M\;, \lbl{eq:filtrationByPrimeIdeals}
\end{align}
where $M_i/M_{i+1} \cong R/\pp_i$ with $\pp_i\in\Spec R$ some prime ideals. We'll investigate some properties of such filtrations.
...
\begin{defi}[Support]\lbl{def:support}
    ...
    \item  
        For the duration of  the next 1 or 2 lectures or so, we set 
        \begin{align*}
...
\end{cor}
\begin{proof}
    This proof captures only the case where $k=R/\mm$ is algebraically closed and $\gr(R,\mm)$ is a domain. For an unconditional proof, cf.\ Eisenbud, \cite[Corollary~12.5]{eisenbudCommAlg}.
...
\end{proof}
And finally, we are able to prove Theorem~\reff{thm:krullIdeal} and thus also Krull's principal ideal theorem, Theorem~\reff{thm:krullPrincipalIdeal}, at last!
...
\begin{proof}[Proof of Theorem~\reff{thm:idealOfDefinition}]
    ...
    \begin{cor*}[\emph{Yes}, a corollary inside a proof -- why not?]
    ...
    \begin{cor*}[\dotso and another one]
...
\begin{cor}\lbl{cor:regLocRingsAreDomains}
    Let $R$ be a noetherian local ring with maximal ideal $\mm$. Suppose that $R$ is regular of dimension $d$.
    \begin{alphanumerate}
    \item 
