...
\begin{rem*}
    ...
    \begin{align*}
    	(m_i)_{i=1}^n \mapsto \sum_{i=1}^n g_im_i\;,\quad (m_{i,j})_{i<j}\mapsto \bigg(\sum_{j=1}^n g_j m_{i,j}\bigg)_{i=1}^n\;,\quad\ldots
    \end{align*}
...
\begin{defi}\lbl{def:qcqs}
    ...
    \begin{alphanumerate}
        ...
        \item 
            ...
            \end{itemize}
        \item 
            \defemph{quasi-separated} if the preimage of any affine open subset of $Y$ is quasi-separated.
    \end{alphanumerate}
\end{defi}
\begin{rem*}
    \begin{alphanumerate}
...
\begin{defi}
	...
	\begin{alphanumerate}
		\item \defemph{stable under base-change} if in any fibre product diagram
		\begin{diagram*}
...
\end{fact}
\begin{proof}
	The equivalence of \itememph{a} to \itememph{c} was proved in Definition~\reff{def:qcqs}\itememph{b}. It is easy to see that \itememph{b} implies \itememph{d}. Assuming \itememph{d}, if $U\subseteq Y$ is affine and $f^{-1}(U)$ is quasi-compact, then for $\lambda\in \Oo_Y(U)$, we have $f^{-1}(U\setminus V(\lambda)) = f^{-1}(U) - V(f^*\lambda)$ being quasi-compact by Fact~\reff{fact:complementClosedQC}. By Fact~\reff{fact:affineSchemesTopologyBase}, such $U\setminus V(\lambda)$ form a topology base.
\end{proof}
...
\end{proof}
\begin{fact}
    The class of quasi-compact morphisms is base local and closed under composition and base change. 
\end{fact}
\begin{proof}
    Closedness under composition is obvious from Fact~\reff{fact:qcmorphism}\itememph{a} and base locality is obvious from Fact~\reff{fact:qcmorphism}\itememph{c}. Closedness under base-change may, by base locality reduced to the case where $Y$ is affine and the change is by $\snake Y\morphism Y$ with $\snake Y$ affine. In this case, $X$ is quasi-compact, hence $X=\bigcup_{i=1}^nX_i$, where $X_i\subseteq X$ is affine and open and $\snake X= X\times_Y\snake Y$ is $\bigcup_{i=1}^n \snake X_i$ with $\snake X_i = X_i\times_Y \snake Y$ is affine as was seen at the end of section 1.4. By the following fact $\snake X_i \morphism \snake Y$ is quasi-compact. When $U\subseteq Y$ is a quasi-compact open subset this means that $\snake f^{-1}(U) \cap \snake X_i = (\snake f|_{\snake X_i})^{-1} (U)$ is quasi-compact and $\snake f^{-1}(U)$ being the finite union of these subsets, is quasi-compact as well. 
\end{proof}
\begin{fact}
...
\begin{proof}\lbl{fact:qcmorphismBetweenQcAndAffine}
    By Fact~\reff{fact:complementClosedQC} $f^{-1}(Y\setminus V(\lambda)) = X\setminus V(f^*\lambda)$ is quasi-compact, for $\lambda\in \Oo_Y(Y)$. As sich $Y\setminus V(\lambda)$ form a topology base on $Y$, Fact~\reff{fact:qcmorphism}\itememph{c} holds.
\end{proof}
...
\begin{proof}
    ...
    Let $(U_i)_{i=1}^n$ be affine open subsets covering $X$ and the assertion hold for closed subsets of $X$ which may be covered by fewer than $n$ affine open subsets of $X$. If $Y\subset U_1$ we are in the previous situation, otherwise $\snake Y = Y\setminus U_1$ is a non-empty closed subset of $Y$ which can be covered by $n-1$ affine open subsets and thus containing a closed point.
\end{proof}
\begin{example}
    IN $X=\Spec k\left[X_i \st i\in \IN\right]$, the open subset $X\setminus \{m\}$ with $\mm= \left\langle X_i\st i\in\IN \right\rangle$ fails to be quasi-compact as $X\setminus \{\mm\} = \bigcup_{i=1}^\infty X\setminus V(X_i)$ has no finite subcover. Then $X\setminus \{\mm\}\morphism X$ is a morphism which fails to be quasi-compact.
\end{example}
\begin{lem}\lbl{lem:qsmorphism}
    ...
    \item 
        The diagonal $X \morphism[\Delta] X\times_Y X$ is a quasi-compact morphism.
    \end{alphanumerate}
\end{lem}
\begin{proof}
    Let \itememph{a} be satisfied and $U\subseteq Y$ be quasi-separated, we have to show that $f^{-1}(U)$ is quasi-separated. Without losing generality we may assume $U=Y$ is quasi-separated and have to show that $X$ is quasi-separated. For two arbitrary quasi-compact open subsets $V$ and $W$ of $X$, $V\cap W$ is quasi-compact. If is sufficient to show this when $f(V)$ and $f(w)$ are contained in affine open subsets $\Omega$ and $\Theta$ of $Y$. Every $x\in V$ has some affine open neighbourhood $V_x\subseteq V$ with $f(V_x)\subseteq \Omega_x$ where $\Omega_x \subseteq Y$ is affine. Similarly, every $x\in W$ has a neighbourhood $W_x\subseteq W$ such that $f(W_x)\subseteq \Theta_x$ which is affine. The for every $x\in V$, $\xi \in W$, $V_x\cap W_\xi$ is quasi-compact by the special case, and $V=\bigcup_{i=1}^m V_{x_i}$ and $W = \bigcup_{j=1}^n W_{x_j}$ and 
    \begin{align*}
    ...
    \end{align*}
    with $\vartheta_j\in \Oo_Y(\Theta)$. Then $V\setminus V(f^*\omega_i)$ and $W\setminus V(f^* \vartheta_j)$ are quasi-compact and have image contained in $\Omega$, and $f^{-1}(\Omega)$ is quasi-separated,
    \begin{align*}
    ...
    Now assume \itememph{c}. Let $\Delta$ be quasi-compact. If $U\subseteq Y$ is affine and $V$ and $W$ are affine open subsets of $f^{-1}(U)$, $V\times_Y W = V\times_U W$ as $U\morphism Y$ is monic. As $U$, $V$, and $W$ are affine, so is $V\times_U W$ by Corollary~\reff{cor:affineSchemesFibreProduct}. But $V\cap W$ equals $\Delta^{-1}(V\times_Y W)$ which is quasi-compact as $\Delta$ is a quasi-compact morphism. This shows \itememph{b}.
    
    Consider \itememph{a} to be true. Let $V, W\subseteq X$ be affine open subsets of $X$ such that $f(V)\cup f(W) \subseteq U$ where $U\subseteq Y$ is affine. Note that such $V\times_U W = V\times_Y W$ cover $X\times_Y X$. As quasi-compactness of morphisms is a base-local property, it suffices to show that $\Delta^{-1}(V\times_Y W)\morphism V\times_Y W$ is quasi-compact. This however, follows from Fact~\reff{fact:qcmorphismBetweenQcAndAffine} as $\Delta^{-1}(V\times_Y) = V\cap W$ is quasi-compact by our assumption and $V\times_Y W= V\times_U W$ is affine. This implies \itememph{c}.
\end{proof}
\begin{defi}[Quasi-separated morphisms]\lbl{def:qsmorphism}
    Such morphisms are called quasi-separated.
\end{defi}
\begin{fact}\lbl{fact:qsmorphismProperties}
    Any separated morphism is quasi-separated, since Lemma~\reff{lem:qsmorphism}\itememph{c} holds as any closed immersion is quasi-compact. The class of quasi-separated morphisms is base-local and closed under base change, composition and fibre products.
\end{fact}
\begin{proof}
    The closedness under composition follows from condition \itememph{b} of Lemma~\reff{lem:qsmorphism}. Base locality follows from \itememph{c} of the same lemma, since quasi-compactness of morphisms is base-local, for $U\subseteq Y$ open and $X_U = f^{-1}(U)$ the $X_U\times_U W_U$ are open subsets of $X\times_Y X$ and $\Delta^{-1}(X_U\times_U X_U) = X_U$. Closedness under base change $\snake \morphism Y$ is also a consequence of condition \itememph{c} of Lemma~\reff{lem:qsmorphism} as, for $\snake Y\morphism Y$ and $\snake X = X\times_Y \snake Y$, $\snake X \morphism[\Delta] \snake X\times_{\snake Y}\snake X$ is a base change of $X\morphism[\Delta] X\times_Y X$ with respect to $\snake X\times_{\snake Y} \snake X \morphism X\times_Y X$.
\end{proof}
...
\begin{defi}[Sober space] \lbl{def:soberSpace}
    A topological space is called \defemph{sober} if any irreducible subset has precisely one generic point (which is often denoted by $n_Z$).
\end{defi}
\begin{rem*}
