...
\end{fact}
\begin{proof}
	\itememph{a} is just \itememph{\alpha} from Definition~\reff{def:localMorphisms}\itememph{b}, in case you didn't notice. And part \itememph{b} is immediately follows from the fact that $\Oo_{Z,z}\morphism[g^*]\Oo_{Y,y}\morphism[f^*]\Oo_{X,x}$ equals $\Oo_{Z,z}\morphism[(gf)^*]\Oo_{X,x}$ by Remark~\reff{rem:pullbackthingyAtStalks}.
...
\begin{fact}
    ...
    \item 
        Let a topological space be called \defemph{locally Noetherian} if the following equivalent conditions hold:
        \begin{itemize}
        \item[\itememph{\alpha}]
...
\begin{proof}
    ...
    To obtain \itememph{c} $\Rightarrow$ \itememph{a}, note that for any quasi-coherent $\Oo_Y$-algebra $\Aa$ and $U\subseteq Y$ affine open, its preimage $p^{-1}(U)$ under $\SPEC_Y(\Aa)\morphism[p]Y$ equals $\SPEC_U(\Aa|_U)\simeq \Spec A$ if $\Aa|_U=\snake A$ for $A$ an $\Oo_Y(U)$-algebra (and such an $A$ may be found by Proposition~\reff{prop:coherentDef}\itememph{a} and Definition~\reff{def:quasicoherent}\itememph{a} as $\Aa$ is quasi-coherent). Then the argument from \itememph{b} $\Rightarrow$ \itememph{c} can be reversed.
\end{proof}
...
\begin{rem}
    ...
    \item 
        Any finite or integral morphism is universally closed. Indeed, a finite (resp.\ integral) morphism is closed by Proposition~\reff{prop:finiteClosed} and all its base changes are finite (resp.\ integral) again by Corollary~\reff{cor:AffineStableUnderBaseChange}.
    \end{alphanumerate}
\end{rem}
\begin{example}
...
\section{Functors, functor morphisms, and the Yoneda lemma}
  \begin{defi}
  	...
  	\end{itemize}
  	A functor is called \defemph{faithful} if $F:\Hom_\Aa(X,Y)\to\Hom_\Bb(FX,FY)$ is injective and \defemph{fully faithful} if it's bijective.
  \end{defi}
