...
\begin{prop}
    ...
    \item\lbl{prop:specStalksCorrespondence}
        ...
        \end{align*}
        Under this bijection, the generic points of the irreducible components (i.e. the maximal irreducible subsets) of $X$ containing $x$ correspond to the minimal prime ideals of $\Oo_{X,x}$.
    \item
...
\begin{prop}
    ...
    \end{alphanumerate}
    If the underlying topological space is Noetherian, this is equivalent 
    \begin{alphanumerate}
    \item[\itememph{d}]
...
\end{prop}
\begin{proof}
    Assume \itememph{a} and let $U\subseteq X$ be open and $f,g\in \Oo_X(U)$ such that $fg = 0$. Then $U = V(f)\cup V(g)$ and as $U$ is irreducible and $V(f)$ and $V(g)$ closed we have without loss of generality $U = V(f)$ hence $f\in(\nil(\Oo_X))(U)$ by Fact~\reff{fact:nilRadical}. As $X$ is reduced, $f=0$, which shows \itememph{b}.
    
    It is obvious, that \itememph{b} implies \itememph{c}.
    
    Assuming \itememph{c} we can show \itememph{a}. Assume $\nil(\Oo_X) \neq 0$, so let $f\in \nil(\Oo_X)(U)$ with $f\neq 0$, then we can cover $U$ with affine open subsets and there has to be an affine $\snake U \subseteq U$ such that $f|_{\snake U} \neq 0$ and $f\in \nil(\Oo_X)(\snake U)$. This is a contradiction and shows reducedness. For irreducibility, take $X=A\cup B$ where $A$ and $B$ are closed and not equal to $X$. Then $V=X\setminus A$ and $W= X\setminus B$ are non-empty open subsets, hence contain non-empty affine open subsets $\snake V \subseteq V$ and $\snake W \subseteq W$. Let $f,g\in \Oo_X(\snake V\cup\snake W)$ be such that $f|_{\snake V} = 1$, $f|_{\snake W} = 0$, $g|_{\snake V} = 0$, $g|_{\snake W} = 1$, then $\snake V\cup \snake W$ is affine and $f$ and $g$ are zero divisors in $\Oo_X(\snake V\cup \snake W)$.
    
    If we assume \itememph{b}, then 
    \begin{align*}
        \Oo_{X,x} = \colimit[U\ni x] \Oo_X(U)
    \end{align*}
    and the colimit of domains is again a domain. So we conclude \itememph{d}.
    
    It is obvious that \itememph{d} is stronger then \itememph{e}.
    
    Now let $X$ be Noetherian and \itememph{e} be true. If $U\subseteq X$ is open and $f\in \Oo_X(U)$ nilpotent, its image in $\Oo_{X,x}$ is nilpotent, hence $0$, for every closed point $x\in X$. Let $V\subseteq U$ quasi-compact (e.g. affine) and let $x\in V$ be closed. The image of $f$ in $\Oo_{X,x}$ being zero, there is an open neighbourhood $V_x$ of $x$ such that $f|_{V_x} = 0$. But these $V_x$ cover $V$ by Proposition~\reff{prop:closedSubsetPoint} ($V\setminus\bigcup_{x\in V} V_x$ being closed, where the union is taken over all closed points), hence $f|_V = 0$. Since those $V$ cover $U$, $f= 0$. Let $X=\bigcup_{i=1}^n$ be the decomposition of $X$ into irreducible components. Let $1\leq i<j\leq n$. If $X_i\cap X_j \neq \emptyset$ it would contain a closed point $x$ by Proposition~\reff{prop:closedSubsetPoint}. By Proposition~\reff{prop:specStalksCorrespondence}\itememph{a} $\Oo_{X,x}$ would contain two different minimal prime ideals (corresponding to $X_i$ and $X_j$) hence fail to be a domain. So this proofs \itememph{a}.
\end{proof}
\begin{fact*}
...
\begin{proof}
    \begin{align*}
        (\Spec A) \amalg (\Spec B) \simeq \Spec A\oplus B
    \end{align*}
\end{proof}
\begin{defi}[Integral preschemes]\lbl{def:integralPrescheme}
    Such a prescheme is called \defemph{integral}.
\end{defi}
...
\begin{prop}\lbl{prop:locFiniteMorph}
    ...
    \begin{alphanumerate}
    \item 
        If $U\subseteq X$ and $V\subseteq Y$ are affine $f(U)$ are affine and $f(U)\subseteq V$ then $\Oo_X(U)$ is of finite type over $\Oo_Y(V)$.
    \item 
...
\begin{lem}\lbl{lem:propsOfAffine}
    Let $E$ be a property of affine open subsets of a prescheme $X$ satisfying the following properties:
    \begin{alphanumerate}
    \item 
        $E(V)$ implies $E(V\setminus V(f))$ for all $f\in \Oo_X(V)$.
    \item 
        If $(f_i)_{i=1}^n \in \Oo_X(V)$, $V$ being affine, such that $\bigcap_{i=1}^n V(f_i) =\emptyset$ and such that $E(V\setminus V(f_i))$ for all $i$, then $E(V)$ holds.
    \end{alphanumerate}
    Then the following conditions are equivalent:
    \begin{alphanumerate}
      \item[\itememph{\alpha}]
        $X$ may be covered by affine open subsets $U$ such that $E$ holds for all of them.
    \item[\itememph{\beta}]
        $E$ holds for arbitrary affine open subsets of $X$.
    \end{alphanumerate}
\end{lem}
\begin{proof}
    The implication \itememph{\beta} $\to$ \itememph{\alpha} being trivial we have to show the other direction. We first have to show that $E(U)$ holds when $U\subseteq V$ and $E(V)$ holds ($U$ and $V$ being affine open subsets). Since the $V\setminus V(\lambda)$ ($\lambda\in \Oo_X(V)$) form a topology base on $V$, $U$ may be covered by such subsets which inherit $E$ by \itememph{a}. But $V\setminus V(\lambda)$ equals $U\setminus V(\lambda)$ when it is a subset of $U$. By the quasi-compactness of $U$ and \itememph{b}, $E(U)$ holds. 
    
    Now let $U$ be an arbitrary affine open subset of $X$. If $x\in U$, it is contained in some affine open subset $V$ of $X$ with $E(V)$. Let $W\subseteq U\cap V$ be an affine open neighbourhood of $x$, we have $E(W)$ by the initial step. We may actually assume $W=U\setminus V(\lambda)$ with $\lambda\in \Oo_X(U)$ as such $W$ form a topology base on $U$. Thus, $U$ may be covered by $W=U\setminus V(\lambda)$ with $E(W)$. By the quasi-compactness of $U$ it has a finite covering of that type, and by \itememph{b}, $E(U)$ holds.
\end{proof}
\begin{fact*}
    $R_f/R$ is of finite type, as 
    \begin{align*}
        R[T]/\langle 1-fT\rangle_{R[T]} \simeq R_f
    \end{align*}
\end{fact*}
\begin{lem}\lbl{lem:finiteTypeOnGenerators}
    Let $R$ be a ring, $A$ an $R$-algebra, $(f_i)_{i=1}^n$ elements of $A$ such that $\langle f_1,\ldots, f_n\rangle_A = A$ (in other words, $\bigcap_{i=1}^n V(f_i) =\emptyset$ in $\Spec A$. If $A_{f_i}$ is an $R$-algebra of finite type for $1\leq i\leq n$, then $A$ is an $R$-algebra of finite type.
\end{lem}
\begin{proof}
    By our assumption there are $(g_i)_{i=1}^n\in A$ such that $1=\sum_{i=1}^n f_ig_i$. Let $A_{f_i}$ be generated by $(\alpha_{i,j})_{j=1}^m$, $\alpha_{i,j} = \frac{a_{i,j}}{f_i^\ell}\in A_{f_i}$, as an $R$-algebra. Let $\snake A\subseteq A$ be the sub-$R$-algebra of $A$ generated by $\left\{f_i\st 1\leq i\leq n\right\} \cup \left\{g_i\st 1\leq i\leq n\right\} \cup \left\{a_{i,j}\st 1\leq i\leq n, 1\leq j \leq m\right\}$. We claim $\snake A = A$. Let $x\in A$, we first show that $f_i^kx \in \snake A$ for $1\leq i \leq n$, when $k$ is large enough. In fact, 
    \begin{align*}
        x = \sum_{\beta \in \IN^m} \rho_{i,\beta} \alpha_{i,1}^{\beta_1}\cdot\ldots\cdot \alpha_{i,m}^{\beta_m}
    \end{align*}
    where $\rho_{i,\beta}\in R$ and equal to zero for all but finitely many $\beta$. When $k\geq \ell\max\left\{|\beta|\st \rho_{i,\beta}\neq 0\right\}$:
    \begin{align*}
        f_i^kx = \sum_{\beta \in \IN^m} \rho_{i,\beta}f_i^{p_{\beta,i,k}} a_{i,1}^{\beta_1}\cdot\ldots\cdot a_{i,m}^{\beta_m}\in \snake A
    \end{align*}
    establishing the claim. But if $k$ has this property
    \begin{align*}
        x &= \left(\sum_{i=1}^n f_ig_i\right)^{nk}\cdot x\\
        &= \sum_{\gamma\in\IN^n} c_\gamma \left(\prod_{i=1}^n (f_ig_i)^{\gamma_i}\right) x
    \end{align*}
    and $\prod_{i=1}^n (f_ig_i)^{\gamma_i}x\in \snake A$ as $\gamma_i\geq k$ for at least one $i$, hence $f_i^{\gamma_i}\in\snake A$, as are the other factors.
\end{proof}

\begin{proof}[Proof of Lemma~\reff{lem:finiteTypeAffine}]
    Let $E(V))$ be the property ``$\Oo_X(V)$ is of finite type over $A$'' of affine open $V\subseteq X$. By transitivity of ``of finite type'' for ring extensions and the last Fact, it has property \itememph{a} of Lemma~\reff{lem:propsOfAffine}. By Lemma~\reff{lem:finiteTypeOnGenerators}, it also has property \itememph{b}. Applying Lemma~\reff{lem:propsOfAffine} to $E$ gives Lemma~\reff{lem:finiteTypeAffine}.
\end{proof}

\begin{proof}[Proof of Proposition~\reff{prop:locFiniteMorph}]
    The implication from \itememph{a} to \itememph{b} is trivial. Now assume \itememph{b}. By \itememph{b}, $X$ may be covered by affine open $U_i$ such that $f(U_i)\subseteq V_i$ with $V_i$ affine and $\Oo_X(U_i)/\Oo_Y(U_i)$ of finite type. Let $U\subseteq X$, $V\subseteq Y$ as in \itememph{a}. Then $U$ my be covered by affine open $\snake U\subseteq U_i$. Then $\Oo_X(\snake U) /\Oo_Y(V_i)$ is of finite type. But $V_i\cap V$ may be covered by $V\setminus V(\lambda)$ and then $\Oo_X(\snake U\setminus V(f^*\lambda)) = \Oo_X(\snake U)_\lambda$ is of finite type[citation needed] over $\Oo_Y(V\setminus V(\lambda))\simeq \Oo_Y(V)_\lambda$, hence $\Oo_Y(V)$ and the assertion from Lemma~\reff{lem:finiteTypeAffine} for $U\morphism V$.
\end{proof}
