...
\end{cor}
\begin{proof}
	This follows trivially from the preceding Corollary~\reff{cor:openImmersionFiniteType} and Corollary~\reff{cor:closedImmersionFiniteType}. Or does it? Well, that depends on how \emph{locally closed immersions} are defined. We \emph{didn't} define them, but the usual notion seems to be that of a morphism of the form $j\circ i$ with $i$ an open immersion followed by $j$ a closed one. For example, The Stacks Project uses this (cf. \cite[Tag~01IO]{stacks-project}) -- but caution, their notion of closed immersions of locally ringed spaces (not preschemes though) is somewhat stronger than ours. In particular, an immersion being locally closed is \emph{stronger} than its image being locally closed.
\end{proof}
...
In this section we want to consider the following problem. Let $R$ be a discrete valuation ring with maximal (principal) ideal $\pi R$. Then $\Spec R = \{s,\eta\}$, where $s= \pi R$ is closed and $\eta = \{0\}$ is open. Thus the closed points of $\Spec R$ are not dense in $\Spec R$, and $\Spec K \morphism \Spec R$, where $K= R[\pi^{-1}]$ is the quotient field of $R$, is of finite type and maps the unique (hence closed) point of $\Spec K$ to the non-closed $\eta\in \Spec R$.

\begin{example}
    Let $X=\Spec A$, $A= R[T]$, $S= \Spec R$, then $X\morphism S = \{\eta, s\}$ is of finite type and $\pp = \langle 1 - T\pi\rangle_A$ is maximal (as $A/\pp = R[\pi^{-1}] = K$) hence closed in $X$ (but not in $\IP_S^1$ where the point $\infty$ on the fibre over $s$ is its closure) and the image of $\pp$ in $S$ is $\eta$, hence not closed. We have $\codim(\{\pp\}, X) =1$ by the principal ideal theorem but the closed points of the fibre over $s$ have codimension two, giving an example of a prescheme which is catenary and integral but not \emph{equicodimensional} (i.e. every closed point has the same codimension).
\end{example}
...
\end{fact}
\begin{proof}
    It is obvious that \itememph{a} is stronger than \itememph{b}. Now assume \itememph{b} and let $A\subseteq X$ be closed, $U\subseteq X$ open and $a\in A\cap U$ and $B= \ov{\{a\}} \subseteq A$. Then $B$ is irreducible closed hence $B\cap U$ contains a closed point $b$ of $X$ and $b\in B\cap U\subseteq A\cap U$. So \itememph{a} holds.
    
    Again assuming \itememph{b}, we want to show \itememph{c}. Let $U\subseteq X$ be open, $x\in U$ closed in $U$ and $\ov{\{x\}}= A$ be its closure in $X$. Then $A\cap U$ is the closure of $x$ in $U$, hence $A\cap U = \{x\}$ and $x$ must be closed in $X$ because $U\cap A\neq \emptyset$ would fail to contain a closed point otherwise.
    
    We now want to show the remaining implication under the stated assumption. Let $\Bb$ be such a topology base with the desired properties. Let $A\subseteq X$ be closed $A\neq \emptyset $ and $a\in A$ and $U\in \Bb$ such that $a\in U$ and there is $x\in U\cap A$ closed in $U$ hence $X$. 
\end{proof}

You should recall (not from this lecture but from Algebra II, so if you do not participate in the other lecture, you may want to read the nice script \cite{alg2}), that for any ring $R$ we define the \defemph{Jacobson radical} 
\begin{align*}
    \rad R = \bigcap_{\mm\in\mSpec R} \mm = \left\{r\in R\st 1-\rho r \in R^\times\forall \rho\in R\right\}
\end{align*}

\begin{lem}
    Let $R$ be a domain such that $\rad R = \{0\}$, then $R$ is a field or its field of quotients $K$ fails to be of finite type over $K$.
\end{lem}
\begin{proof}
    If $K$ is finite type over $R$ (say generated by $\left(\frac{z_i}{n_i}\right)_{i=1}^k$) then $K= R_f$ for some $f\in R\setminus \{0\}$ (e.g. $f = \prod_{i=1}^k n_i$) then $f\in \rad R$ because every $x\in R\setminus \{0\}$ divides some power of $f$. Hence $f$is contained in any non-zero prime ideal.
\end{proof}
\begin{prop}\lbl{prop:JacobsonRing}
    For a ring $R$, the following conditions are equivalent:
    \begin{alphanumerate}
    \item 
        $\Spec R$ satisfies the equivalent properties of Fact~\reff{fact:closedPointsDense}.
    \item  
        Any $\pp\in \Spec R$ is the intersection of the maximal ideals containing it.
    \item 
        For any $\pp\in \Spec R$, $\rad(R/\pp) = \{0\}$.
    \item  
        If $A$ is any $R$-algebra of finite type and $\mm\in\mSpec A$, then its preimage in $R$ is in $\mSpec R$.
    \end{alphanumerate}
\end{prop}
\begin{proof}
    The equivalence between \itememph{b} and \itememph{c} follows from the formula for $\rad$ recalled above.
    
    The points \itememph{a} and \itememph{c} are equivalent, which follows from the description of the topology of $\Spec R$ (as $\ov\XX = V(I)$ where $I = \sqrt{I} = \bigcap_{\pi\in \XX} \pp$). If $\qq\in \Spec R$ and $\XX \ov{\{\qq\}} \cap \mSpec R$ then $\ov{\{\qq\}} = \ov\XX$ by Property \itememph{b} of Fact~\reff{fact:closedPointsDense}. Hence $\qq= \bigcap_{\mm\in \XX} \mm$ by the bijection between the closed $A\subseteq \Spec R$ and the $I\subseteq R$ such that $I = \sqrt{I}$.
    
    Assume \itememph{b}. Let $A$ be an $R$-algebra of finite type, $\mm\in \mSpec A$, we have to show $\mm\sqcap R \in \mSpec R$. Replacing $A$ by $A/\mm$, we may assume $A$ to be a field. Replacing $R$ by $R/\pp$ with $\pp = \mm\sqcap R$ we may assume $R$ to be a subring of that field. Then $A$ contains a field $K$ of quotients of $R$ over which it is of finite type. By the Nullstellensatz $A/K$ is a finite field extension, hence $A$ is a finitely generated $K$-module. By Lemma~\reff{lem:artinTate}, $K$ is of finite type over $R$. By $\rad R = \{0\}$, $R=K$, hence all prime ideals of $R$ are maximal. 
    
    To show that \itememph{c} implies \itememph{a}, we improve Fact~\reff{fact:closedPointsDense} to Fact~1++:
    
    \begin{fact*}[Fact 1++]\lbl{fact:1++}
        Let $X$ be a topological space with a topology base $\Bb$ such that any non-empty open subset of an element $V$ of $\Bb$ contains a closed point of $U$ and such that every closed point of any  element of $\Bb$ stays closed in $X$. Then $X$ satisfies Fact~\reff{fact:closedPointsDense}\itememph{a}.
    \end{fact*}
    
    We apply this to $\Bb = \left\{\Spec R_f \st f\in R\right\}$, the $R_f$ are of finite type over $R$. 
\end{proof}
\begin{defi}[Jacobson ring]\lbl{def:JacobsonRing}
    A ring with these equivalent properties is called a \defemph{Jacobson ring}.
\end{defi}
\begin{example}
...
\end{cor}
\begin{proof}
    Let $R$ be Jacobson and $A/R$ as well as $B/A$ be of finite type and $\mm\in \mSpec B$ and $\nn = A\sqcap \mm$ then $\nn\sqcap R = \mm\sqcap R$ is a maximal ideal as $B/R$ is of finite type and $R$ is Jacobson. Hence $A/\nn$ is a subring of $B/\mm$ which is of finite type over $A/(A\sqcap \nn)$, hence it is a finite field extension of $R/\mm$ by the Nullstellensatz. $R/(\mm\sqcap R) \subseteq A/\nn \subseteq B/\mm$ where the outer terms form a finite field extension hence $A/\nn$ is a field extension and $\nn$ maximal. Hence $A$ satisfies Property \itememph{c} of Proposition~\reff{prop:JacobsonRing}
\end{proof}
\begin{defi}[Finally Franke openly admits it is actually also a Proposition. Self-awareness is the first step to betterment]\lbl{defi:JacobsonPrescheme}
    For a prescheme $X$, the following conditions are equivalent:
    \begin{alphanumerate}
    \item 
    ...
    \item 
        If $Y\morphism[f] X$ is a morphism of finite type and $y\in Y$ closed, $f(y)$ is closed in $X$. 
    \end{alphanumerate}
    A prescheme with these properties is called \defemph{Jacobson prescheme}. Any prescheme of finite type over a Jacobson prescheme is Jacobson.
\end{defi}
\begin{rem*}
...
\end{rem*}
\begin{proof}[Proof of equivalence of Definition~\reff{def:JacobsonPrescheme}]
    Assume \itememph{a}. If $A\subseteq U$ is closed and irreducible and $V\subseteq U$ open such that $V\cap A\neq \emptyset$ and $B= \ov A$ in $X$, then $A\cap V = B\cap V$ contains a point which is closed in $X$, hence $U$ and the closed points of $U$ are dense in $A$. This proves \itememph{b}.
    
    \itememph{b} implying \itememph{c} is trivial.
    
    Now assume \itememph{c}, we want to show \itememph{a}. If $U$ is open affine and $\Oo_X(U)$ is Jacobson, then $\Oo_X(U\setminus V(\lambda))$ is Jacobson for any $\lambda\in \Oo_X(U)$ as $\Oo_X(U)_\lambda/\Oo_X(U)$ is of finite type. By Fact~\reff{fact:affineSchemesTopologyBase} the affine open $U$ such that $\Oo_X(U)$ is Jacobson form a topology base and the assertion is easily seen similar to Fact~\reff{fact:1++}.
    
    If $U\subseteq X$ is open and $U\morphism X$ is of finite type hence maps closed points to closed ones if we assume \itememph{d}, Fact~\reff{fact:closedPointsDense}\itememph{c} holds which implies \itememph{a}.
    
    We assume \itememph{a} and \itememph{b} (these being equivalent) and show \itememph{d}. If $y\in Y$ is closed and there are affine $V\subseteq Y$, $U\subseteq X$ such that $f(V)\subseteq U$ then $\Oo_X(U)$ is Jacobson and $\Oo_Y(Y)$ of finite type over it hence $f(Y)$ is closed in $U$ by the proposition, hence is closed in $X$ by \itememph{a}.
\end{proof}
