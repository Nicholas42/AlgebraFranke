...
\begin{proof}
	...
	\begin{diagram}
		...
		\node (b) at (3,1.5) {$\Oo_X(a^{-1}V)\,\rlap{$=a_*\Oo_X(V)$}$};
		\node (d) at (3,0) {$\Oo_{X,k}$};
		\scriptsize
	...
	\begin{diagram}
		...
		\node (kx) at (3,0) {$\Oo_{T,t}$};
		\node (ky) at (1.5,-1.25) {$\Oo_{K,k}$};
		\node (OX) at (0,1.25) {$\Oo_X(V)$};
		\node (OT) at (3,1.25) {$\Oo_T(\tau^{-1}V)$\rlap{$\,=\tau_*\Oo_T(V)$}};
		\scriptsize
...
\end{proof}
\begin{cor}
    Let $X\doublemorphism[f][g] Y$ be a pair of morphisms in $\cat{LRS}$ and $K\morphism[\kappa] X$ be an equalizer of that pair:
    \begin{alphanumerate}
      \item 
...
\begin{rem*}
    ...
    \end{align*}
\end{rem*}
The fibre product of locally ringed spaces $X$ and $Y$ over $S$ is given by
\begin{align*}
    X\times_S Y = \left\{(x,y,\pp)\st x\in X, y\in Y, s\coloneqq \xi(x) \upsilon(y), \pp\in \Spec\left(\Oo_{X,x}\otimes_{\Oo_{S,s}}\Oo_{Y,y}\right), \alpha^{-1}(\pp) = \mm_{X,x}, \beta^{-1}(\pp) = \mm_{Y,y}\right\}
\end{align*}
where $\alpha(\lambda) =\lambda \otimes 1$ and $\beta(\theta) = 1\otimes \theta$. 

We obtain a topology base $\Omega(U,V,W,\lambda)$ where $U\subseteq X$, $V\subseteq Y$, $W\subseteq S$ such that $\xi(U) \cup \upsilon(V)\subseteq W$ and $\lambda \Oo_X(U)\otimes_{\Oo_S(W)} \Oo_Y(V)$. Its elements are triples $(x,y,\pp)$ such that $x\in U$, $y\in V$ such that $\pp$ contains the image of $\lambda$ under $\Oo_X(U) \otimes_{\Oo_S(W)} \Oo_Y(V) \morphism \Oo_{X,x}\otimes_{\Oo_{S,s}} \Oo_{Y,y}$.

The structure sheaf is given by $\Oo_{F,[(x,y,\pp)]} = (\Oo_{X,x}\otimes_{\Oo_{S,s}} \Oo_{Y,y})_\pp$. We can show the coherence condition locally, the family must come from a $\lambda$ as above.
...
\begin{defi}[Fibre product]\lbl{def:fibreProduct}
    Let $X\morphism[\xi] S\lmorphism[\upsilon] Y$ be morphisms in $\Aa$. A \defemph{fibre product} of $X$ with $Y$ over $S$ is an object $P$ with morphisms $X\lmorphism[p] P \morphism[q] Y$ satisfying $\xi p = \upsilon q$ and enjoying the universal property for such diagrams 
    
    DIAGRAM
    
    If this diagram satisfies $\xi\tau_X = \upsilon \tau_Y$ then there exists a unique morphism $T\morphism[t] P$ in $\Aa$ such that $\tau_X = pt$ and $\tau_Y = qt$. Often we will write $P = X\times_S Y$. By the typical Yoneda-style argument the fibre product is unique up to unique isomorphism.
\end{defi}
\begin{rem*}
    \begin{alphanumerate}
      \item 
      ...
      \item 
        ...
        \begin{align*}
            \Eq\left(X\times Y \doublemorphism[\xi\pi_X][\upsilon \pi_Y] S\right) \simeq X\times_S Y.
        \end{align*}
      \item 
        Let the \emph{comma category} $\Aa\morphism S$ (or $\Aa/S$) the category whose objects are the pairs $(X,\xi)$ where $X$ is an object and $X\morphism[\xi] S$ is a morphism in $\Aa$, and where a morphism $(X,\xi) \morphism (Y,\upsilon)$ is a morphism $X\morphism[f]$ in $\Aa$ such that we have $\upsilon f= \xi$ i.e. 
      ...
      \item 
        The smallest square in the diagram above defining the fibre product is called \emph{cartesian} if $P$ is a fibre product. If is also often useful to think of $p$ as morphism such that the fibre $p^{-1}(x)$ is isomorphic to $\upsilon^{-1}(\xi(x))$. Then one prefers a terminology
    	...
    	\node (d) at (2.5,0) {$S$};
    	\scriptsize
    	\draw[->] (a) -- (b) node[pos=0.5, above] {$\upsilon'$};
    	\draw[->] (a) -- (c) node[pos=0.5, left] {$\xi'$};
    	\draw[->] (c) -- (d) node[pos=0.5, above] {$\upsilon$};
    	\draw[->] (b) -- (d) node[pos=0.5, right] {$\xi$};
    \end{diagram}
        
        and calls $\upsilon'$ a \emph{base change} of $\upsilon$ with respect to $\xi$ and $\xi'$ a base change of $\xi$ with respect to $\upsilon$.
      \item 
        ...
        \end{align*}
        equipped with $(x,y)\circ(\snake x, \snake y) = (x\circ \snake x, y\circ\snake y)$ and (possibly) the induced topology from $X\times Y$.
    \end{alphanumerate}
\end{rem*}
\begin{defi}[Cofibre product] \lbl{def:cofibreProduct}
    A \defemph{dual fibre product} or \defemph{cofibre product} is a commutative diagram 
    \begin{diagram}
    	\node (a) at (0,1.5) {$B$};
    	\node (c) at (0,0) {$Y$};
    	\node (b) at (2.5,1.5) {$X$};
    	...
    	\draw[->] (c) -- (d) node[pos=0.5, above] {$k$};
    	\draw[->] (b) -- (d) node[pos=0.5, right] {$j$};
    \end{diagram}
    ...
    \begin{diagram}
    	...
    	\draw[->] (a) -- (b) node[pos=0.5, above] {};
    	\draw[->] (a) -- (c) node[pos=0.5, left] {};
    	\draw[->] (c) -- (d) node[pos=0.5, above] {};
    	\draw[->] (b) -- (d) node[pos=0.5, right] {};
    \end{diagram}
    there is a unique dotted arrow (a.k.a. a morphism) in  
    
    THIS HAPPY LITTLE DIAGRAM
    
    making that diagram commute.
\end{defi}
\begin{example}
    Let $R$ be a ring, $A$ and $B$ be $R$-algebras. Then the tensor product is an $R$-module $A\otimes_R B$ and a map 
    \begin{align*}
        A\times B &\morphism A\otimes_R B\\
        (a,b) &\longmapsto a\otimes b
    \end{align*}
    such that the map
    \begin{align*}
        \Hom_R(A\otimes_R B, T) &\morphism \Bil_R(A,B; T)\\
        \tau &\longmapsto ((a,b)\mapsto \tau(a\otimes b))
    \end{align*}
    must be bijective. Also 
    \begin{align*}
        \Hom_{\cat{Ab}} (A\otimes_R B, T)&\morphism \left\{ t\in \Bil_\IZ(A,B;T)\st (t(ra,b) = t(a,rb))\right\}\\
        \tau &\longmapsto (t\colon (a,b)\mapsto \tau(a\otimes b))
    \end{align*}
    is bijective. 
    
    Then $A\otimes_R B$ has a unique ring structure such that $(a\otimes b)\cdot(\alpha\otimes \beta) = (a\cdot \alpha)\otimes(b \cdot \beta)$. Then $A\morphism A\otimes_R \lmorphism B$ sending $a$ to $a\otimes 1$ and $b$ to $1\otimes b$ are ring morphisms such that
    \begin{diagram}
    	\node (a) at (0,1.5) {$R$};
    	\node (c) at (0,0) {$B$};
    	\node (b) at (2.5,1.5) {$A$};
    	...
    	\draw[->] (b) -- (d) node[pos=0.5, right] {};
    \end{diagram}
    is \emph{cocartesian} in the category of rings.
\end{example}
