...
\begin{defi}[Immersive morphisms] \lbl{def:immersiveMorphism}
    ...
    If $\upsilon(Y)\subseteq S$ is a closed subset, $\upsilon$ is called a \defemph{closed immersion}.
\end{defi}
...
\begin{prop}\lbl{prop:closedImmersionSheaves}
    ...
    \begin{align*}
        \Hom_{\cat{LRS}}(T,K) \isomorphism\left\{ g\in \Hom_{\cat{LRS}}(T,X) \st \Ii\subseteq \ker(\Oo_X \morphism[g^*]g_*\Oo_T)\right\}
    \end{align*}
...
\begin{prop}\lbl{prop:affineSchemeAdjunction}
    ...
    \begin{align*}
        ...
        (X\morphism[\xi] \Spec R) &\longmapsto (R\isomorphism \Oo_{\Spec R}(\Spec R) \morphism[\xi^*] \Oo_X(X))
    \end{align*}
\end{prop}
\begin{rem*}
...
\begin{cor}
    ...
    \begin{align*}
        \Spec S\doublemorphism[\Spec a][\Spec b] \Spec R\;.
    \end{align*}
\end{cor}
\begin{proof}    	
...
\begin{prop}\lbl{prop:affineSchemeAdjunctionModules}
    ...
    \begin{align*}
        \Hom_{\Oo_{\Spec R}} (\snake M, \Nn) \morphism \Hom_R\big(M, \Nn(\Spec R)\big)
    \end{align*}
...
\begin{defi}[Quasi-coherence]\lbl{def:quasicoherent}
    ...
    \begin{itemize}
        ...
        \item[$(\beta)$] 
            The morphism $\Mm(X)_\lambda \morphism \Mm(X\setminus V(\lambda))$ is an isomorphism for all $\lambda\in R$. 
        \item[$(\gamma)$]
...
\begin{proof}
    For \itememph{\alpha} $\Rightarrow$ \itememph{\beta} note that $\snake M(X\setminus V(\lambda)) \lisomorphism M_\lambda$ by Proposition~\reff{prop:RequalsGlobalSection} and its first remark.
    
    Our construction and \itememph{\beta} imply that $c$ induces an isomorphism on sections on any element of the topology base $\Bb$, hence everywhere. This shows \itememph{\beta} $\Rightarrow$ \itememph{\gamma}. 
    
    The last implication \itememph{\gamma} $\Rightarrow$ \itememph{\alpha} is obvious.
\end{proof}
\begin{cor}
...
\begin{defi}[Affine prescheme]\lbl{def:affinePrescheme}
    A prescheme $X$ is called \defemph{affine} if $X\simeq \Spec R$ in $\cat{LRS}$, for some ring $R$. An open subset $U\subseteq X$ is called \emph{affine} if $(U,\Oo_X|_U)$ is affine prescheme.
\end{defi}
\begin{rem*}
...
\end{rem*}
\begin{rem*}
    By well-known facts, $\Spec R$ is quasi-compact. Hence, any affine open subset is quasi-compact. Schemes will be preschemes with an additional separation condition implying but not equivalent to the proper the intersection of two affine open subsets is affine, hence quasi-compact. Also, recall the well-known fact $\left\{\Spec R\setminus V(f)\right\}$ is a topology base on $\Spec R$ consisting of affine open subsets. 
\end{rem*}
\begin{fact}\lbl{fact:affineSchemesTopologyBase}
    The affine open subsets of a prescheme $X$ form a topology aye on $X$. More precisely if $X=\bigcup_{\lambda\in \Lambda} U_\lambda$ is a cover of $X$ by affine open subsets $U_\lambda$, $\left\{U_\lambda\setminus V(f)\st \lambda\in \Lambda, f\in \Oo_X(U_\lambda)\right\}$ is a topology base on $X$ consisting of affine open subsets.
\end{fact}
\begin{rem*}
    In particular, if $U\subseteq X$ is open, if can be covered by affine open subsets of $X$. 
\end{rem*}
\begin{proof}
    It is sufficient to show the second part. Let $V\subseteq X$ be open. As $\left\{\Spec R\setminus V(g)\st g\in R\right\}$ is a topology base on $\Spec R$, $V\cap U_\lambda = \bigcup_{\xi\in\Xi_\lambda} U_\lambda \setminus V(f_{\lambda,\xi})$, with $f_{\lambda,\xi}\in \Oo_X(U_\lambda)$. Then $V= \bigcup_{\lambda\in\Lambda} = \bigcup_{\lambda\in \Lambda} \bigcup_{\xi\in\Xi_\lambda} U_\lambda\setminus V(f_{\lambda,\xi}$.
\end{proof}
\begin{defi}[Quasi-compact]\lbl{def:quasicompact}
    ...
    \end{alphanumerate}
    A prescheme $X$ is called quasi-seperated, if it satisfies the following equivalent conditions:
...
\end{defi}
\begin{proof}
    The union $U$ of finitely many quasi-compact open subsets $(U_i)_{i=1}^n$ of a topological space $X$ is again quasi-compact. For any cover $U= \bigcup_{\lambda\in\Lambda} V_{\lambda}$ by open subsets, each $U_i = \bigcup_{\lambda\in \Lambda} U_i\cap V_\lambda$ has a finite subcover $(U_i\cap V_{\lambda_{i,j}})_{j=1}^{n_i}$ and all these finite subcovers give us a finite subcover $U = \bigcup_{i=1}^n \bigcup_{j=1}^{n_i} U_{\lambda_{i,j}}$. 
    
    So \itememph{a} implies \itememph{b} by the definition, \itememph{b} implies \itememph{c} since $\Spec R$ is quasi-compact and \itememph{c} implies \itememph{a} by the above fact. 
    
    We obtain \itememph{d} from \itememph{e} since affine open subsets are quasi-compact and \itememph{f} from \itememph{d} by Fact \reff{fact:affineSchemesTopologyBase}. To see that \itememph{f} implies \itememph{e}, let $\Bb$ be as in \itememph{f} and $V$ and $W$ be quasi-compact open subsets. Then $V= \bigcup_{i=1}^m B_i$, $W=\bigcup_{j=1}^n C_j$ with $B_i,C_j\in \Bb$. Then $V\cap W = \bigcup_{i=1}^m \bigcup_{j=1}^n B_i\cap C_j$ is quasi compact since all $B_i\cap C_j$ are quasi-compact by assumption.
\end{proof}
\begin{fact}
    \begin{alphanumerate}
        \item
        ...
        \item   
            If $Y\morphism X$ is a morphism of preschemes which is a homeomorphism onto a closed subset of $X$ carrying the induced topology. Then $x$ being quasi-compact implies $Y$ being quasi-compact, and $X$ being quasi-separated implies $Y$ being quasi-separated.
    \end{alphanumerate}
\end{fact}
\begin{proof}
    The assertions about quasi-compactness are well-known. For quasi-separatedness in \itememph{a}, we take $\Bb=\left\{\Spec R_f\st f\in R\right\}$, which elements are quasi-compact and which is closed under intersection. For \itememph{b} take $\Bb= \left\{V\cap Y\st V\subseteq X \text{ affine}\right\}$.
\end{proof}
\begin{prop}\lbl{prop:coherentDef}
    Let $X$ be a prescheme, $\Mm$ an $\Oo_X$-module. The following conditions are equivalent:
    \begin{alphanumerate}
        \item 
            $X$ may be covered by affine subsets $U$ such that $\Mm|_U$ is quasi-coherent.
        \item 
            $X$ has a topology base consisting of affine open subsets $U$ such that $\Mm|_U$ is quasi-coherent.
        \item   
            When $U\subseteq X$ is quasi-compact, the morphism
            \begin{align}\lbl{eq:bijectiveQCQS}
                \Mm(U)_\lambda \morphism \Mm(U\setminus V(\lambda))
            \end{align}
            is, for all $\lambda\in \Oo_X(U)$, injective when $U$ is quasi-compact and bijective when $U$ is quasi-compact and quasi-seperated.
        \item 
            \eqreff{eq:bijectiveQCQS} is bijective when $U$ is quasi-compact and quasi-coherent.
        \item 
            \eqreff{eq:bijectiveQCQS} is bijective when $U$ is affine.
        \item 
            For arbitrary affine open $U$, $\Mm|_U$ is quasi-coherent.
    \end{alphanumerate}
\end{prop}
\begin{proof}
    Assume \itememph{a}. Since $\snake M|_{\Spec R_f} \simeq \snake M_f$, when $X = \bigcup_{\lambda\in\Lambda}$ with $U_\lambda$ affine and $\Mm|_{U_\lambda}$ quasi-coherent, then $\Mm|_{U_\lambda\setminus V(f)}$ is quasi-coherent. By Fact 1\itememph{b}, \itememph{b} follows.
    
    Now let \itememph{b} be given. Let $\Bb$ be a topology base of affine open subsets such that $\Mm|_U$ is quasi-coherent when $U\in\Bb$. By the definition of quasi-coherent, \eqreff{eq:bijectiveQCQS} is an isomorphism for $U\in\Bb$. Now let $V$ be a quasi-compact open subset, $f\in \Oo_X(B)$, $m=\frac{\mu}{f^\ell}\in \Mm(V)_f$ such that the image of $m$ under \eqreff{eq:bijectiveQCQS} vanishes. Then $\mu|_{U\setminus V(f)} = 0$. As $V$ is quasi-compact it can be represented $V=\bigcup_{i=1}^n V_i$ with $V_i\in\Bb$. As \eqreff{eq:bijectiveQCQS} is bijective for $V_i$, we have $k_i\in\IN$ such that $f^{k_i}(\mu|_{V_i}) = 0$ in $\Mm(V_i)$. Let $k=\max_{i=1}^n k_i$, then $f^k(\mu|_{V_i}) = 0$, hence $f^k\mu = 0$ in $\Mm(V)$ as $\Mm$ is a sheaf. Thus, $m = \frac{\mu}{f^\ell}\in M(V)_f$ is zero, which shows injectivity.
    
    To show bijectivity, let $\Bb$ be as before, $V\subseteq X$ be open, quasi-compact, quasi-seperated, and let $m\in \Mm(V\setminus V(f))$ be given. As before we have $V=\bigcup_{i=1}^n V_i$ with $V_i\in\Bb$. As \eqreff{eq:bijectiveQCQS} is an isomorphism on $V_i$ and there are only finitely many $V_i$, $m|_{V\setminus V(f)} = \frac{\mu_i}{f^k}$ with $\mu_\in \Mm(V_i)$. Let $\mu_{i,j} = \mu_i|_{V_i\cap V_j} - \mu_j|_{V_i\cap V_j}$, then $\mu_{i,j}|_{(V_i\cap V_j)\setminus V(f)} = 0$. As $V$ is quasi-separated and the $V_i$ are affine, this implies $f^{\ell_{i,j}} \mu_{i,j} = 0$ in $\Mm(V_i\cap V_j)$ by the previous step. Taking $\ell = \max\left\{ \ell_{i,j} \st 1\leq i\leq j \leq n\right\}$ we have $f^\ell\mu_u |_{V_i\cap V_j} = f^\ell\mu_j|_{V_i\cap V_j}$ and replacing $k$ by $k+\ell$, $\mu_i$ by $f^\ell \mu_i$, we achieve $\mu_i|_{V_i\cap V_j} = \mu_j|_{V_i\cap V_j}$. As $\Mm$ is a sheaf, there is $\mu\in\Mm(V)$ such that $\mu|_{V_i} = \mu_i$. Then $\frac{\mu}{f^k}|_{V\setminus V(f)} = m$ as the restriction of the left hand side to $V_i\setminus V(f)$ equals $\frac{\mu_i}{f^k} = m|_{V_i\setminus V(f)}$. 
    
    Now \itememph{c} trivially implies \itememph{d}. We obtain \itememph{e} from \itememph{d} by Fact 2, \itememph{f} form \itememph{e} by the Definition~\reff{def:quasicoherent} and \itememph{a} from \itememph{f} by Definition~\reff{def:prescheme}.
\end{proof}
\begin{cor}
    The morphism \reff{eq:bijectiveQCQS} $\Oo_X(V)_\lambda\morphism \Oo_X(V\setminus V(\lambda))$ is injective when $V$ is quasi-compact and bijective, when $V$ is quasi-compact and quasi-separated. 
\end{cor}
\begin{cor}
    When $V$ is quasi-compact and $\lambda\in \Oo_X(V)$ such that $V(\lambda) = V$ then $\lambda$ is nilpotent. Vice versa, $V(f) = V$ if $f$ is nilpotent, even if $V$ is not quasi-compact.
\end{cor}
...
\end{defi}
As kernels of module morphisms commute with localization, we have
\begin{fact}
...
\end{example*}
\begin{defi}
    Let $X\morphism[f] Y$ be a morphism between preschemes. We say that $f$:
    \begin{alphanumerate}
        \item
            a closed embedding, if it is a closed immersion in the sense of \reff{def:immersiveMorphism}.
        \item 
            quasi-compact when it satisfies the following equivalent conditions:
            \begin{itemize}
              \item 
            ...
            \end{itemize}
        \item 
            quasi-seperated if the preimage of any open subset of $Y$ is quasi-separated.
    \end{alphanumerate}
\end{defi}
\begin{rem*}
    \begin{alphanumerate}
        \item 
            The equivalence is as the proof of Definition~\reff{def:quasicompact}, which is a special case of it. 
        \item 
            $X$ is quasi-seperated iff for any quasi-compact open subset $V$, $V\monomorphism X$ is quasi-compact.
        \item 
            Open embeddings are quasi-seperated for trivial reasons.
    \end{alphanumerate}
\end{rem*}
\begin{prop}\lbl{prop:closedEmbeddingPrescheme}
    \begin{alphanumerate}
        \item 
            A closed embedding of preschemes is quasi-compact and quasi-seperated.
        \item 
            Let $X\morphism[f] Y$ be a morphism of preschemes which is quasi-compact and quasi-separated and $\Mm$ a quasi-coherent $\Oo_X$-module, then $f_*\Mm$ is a quasi-coherent $\Oo_Y$-module.
    \end{alphanumerate}
\end{prop}
\begin{proof}
    \itememph{a} follows from Fact 2.
    
    For \itememph{b} take $V\subseteq Y$ affine. Then \eqreff{eq:bijectiveQCQS} for $f_*\Mm$ and $V$ is an isomorphism by the diagram
    
    DIAGRAM
    
\end{proof}
\begin{prop}
    Let $X$ be a prescheme and $K$ a locally ringed space and $K\morphism[\iota] X$ a closed embedding in the sense of Definition~\reff{def:immersiveMorphism}. Then $K$ is a prescheme if and only if the sheaf of ideals $\Jj=\ker(\Oo_X\morphism[\iota^*]\iota_*\Oo_K)$ associated to $\iota$ by Proposition~\reff{prop:closedImmersionSheaves} is quasi-coherent.
\end{prop}
\begin{proof}
    Let $K$ be a prescheme. By Proposition~\reff{prop:closedEmbeddingPrescheme} and Fact 3, $\Jj$ is quasi-coherent. Conversely, let $\Jj$ be quasi-coherent. As the correspondence between $K$ and $\Jj$ is compatible with restriction to open subsets, it is sufficient to show that $K$ is a prescheme, when $X$ is affine. In this case ($X=\Spec R$), any quasi-coherent $\Jj\subseteq \Oo_X$ has the from $\Jj = \snake I$. But $\Spec R/I \morphism \Spec R$ is a closed embedding and our correspondence associates $\snake I$ to i. Thus, $K\simeq \Spec(R/I)$ is affine in this case.
\end{proof}
