...
\end{cor}
    ...
    \begin{rem}\lbl{rem:pullbackthingyAtStalks}
        ...
        \begin{diagram*}
        	...
        	\node (Fx) at (0,0) {$\Ff_x$};
        	\node (f*G) at (3,1.5) {$(f_*\Gg)$\rlap{$_{f(x)}$}};
        	\node (Gx) at (3,0) {$\Gg_x$};
        	\scriptsize
        ...
        \begin{diagram*}
        	...
        	\node (f*G) at (3,1.5) {$\big((gf)_*\Ff\big)$\rlap{$_{(gf)(x)}$}};
        	\node (Gx) at (3,0) {$\Ff_x$};
        	\scriptsize
...
\begin{proof}
	...
	\begin{diagram*}
		...
		\node (OT) at (2.5,1.25) {$\Oo_T(\tau^{-1}V)$\rlap{$\,=\tau_*\Oo_T(V)$}};
		\scriptsize
...
\begin{diagram*}
	...
	\node (OF) at (4.5,1.5) {$\left(\Oo_{X,x}\otimes_{\Oo_{S,s}} \Oo_{Y,y}\right)_{\pp}$\rlap{$\,=\Oo_{F,[x,y,\pp]}$}};
...
\begin{diagram*}
	...
	\node (Tensor) at (0,0) {\llap{$\Oo_{F,[\tau(t)]}=\,$}$\left(\Oo_{X,x}\otimes_{\Oo_{S,s}}\Oo_{Y,y}\right)_{\pp}$};
...
\begin{proof}
    ...
    \begin{diagram*}
    	\node (OX) at (0,0) {$\Oo_{X,x}$};
    	\node (OK) at (2,0.75) {$\Oo_{K,x}$\rlap{$\,\simeq\Oo_{K,[x]}$}};
    	\node (OL) at (2,-0.75) {$\Oo_{L,\ell}$};
    	\scriptsize 
...
\begin{proof}
	...
	\begin{center}
		...
		\begin{minipage}{0.425\textwidth}
			\centering			
			\begin{diagram*}
				\node (MXU) at (0,1.25) {$\Mm_X(U)$};
				\node (MXUV) at (3,1.25) {$\Mm_X(U\setminus V(f))$};
				\node (MXUf) at (1.5, 0) {$\Mm_X(U)_f$};
				\scriptsize
				\draw [->] (MXU) -- (MXUV);
				\draw [->] (MXU) -- (MXUf);
...
\end{prop}
\begin{rem*}
    Thus, $(\cat{Ring})^\op \xrightarrow{\Spec(-)} \cat{LRS}$ is right adjoint to $\cat{LRS} \xrightarrow{\Oo_-(-)} \cat{Ring}$.
\end{rem*}
\begin{proof}[Proof of Proposition~\reff{prop:affineSchemeAdjunction}]
    ...
    \begin{diagram*}
    	\node (R) at (0,1.5) {$R$};
    	\node (OR) at (2.5,1.5) {$\Oo_{\Spec R}(\Spec R)$};
    	\node (OX) at (5.5,1.5) {$\Oo_X(X)$};
    	\node (Rp) at (0,0) {$R_\pp$};
    	\node (ORp) at (2.5,0) {$\Oo_{\Spec R,\pp}$};
    	\node (OXx) at (5.5,0) {$\Oo_{X,x}$};
    	\scriptsize
    	\draw[->] (R) -- (Rp);
...
\end{cor}
    ...
    \begin{align}\lbl{eq:prop1.4.2surj}
        f(x) = \left\{r\in R\st x\in V(\phi(r)) \right\}
    \end{align}
    this is the preimage of $\mm_{X,x} \subseteq \Oo_{X,x}$ under $R\morphism[\phi] \Oo_X(X) \morphism \Oo_{X,x}$. Let $\Bb$ be the topology base $\left\{\Spec R_\lambda\st \lambda\in R\right\}$ of $\Spec R$. If $U\in \Bb$, we have a morphism 
    \begin{align*}
        f^*\colon \Oo_{\Spec R}(U) \simeq R_\lambda  \morphism[\phi] \Oo_X(X)_{\phi(\lambda)} \morphism \Oo_X(X\setminus V(\phi(\lambda))) = \Oo_X(f^{-1} U)
    \end{align*}
    defining a homomorphism of presheaves on $\Bb$ with target $f_*\Oo_X|_\Bb$. Sheafifying gives 
    \begin{align*}
        f^*\colon \Oo_{\Spec R}\simeq (\Oo_{\Spec R}|_\Bb)^\sh \morphism (f_*\Oo_X|_\Bb)^\sh \isomorphism f_*\Oo_X\;.
    \end{align*}
    In particular for $\lambda = 1$, we have $R\simeq  \Oo_{\Spec R}(\Spec R) \morphism[f^*] \Oo_X(X)$ equals $\phi$. The morphism induced on stalks by $f$ is thus uniquely determined morphism $R_\pp \morphism \Oo_{X,x}$ (where $\pp=f(x)$) such that its composition with $R\morphism R_\pp$ equals the map $R\morphism \Oo_X(X) \morphism \Oo_{X,x}$. By \eqreff{eq:prop1.4.2surj}, the later morphism maps $\pp$ to $\mm_{X,x}$, hence the former maps $\pp\cdot R_\pp$ to $\mm_{X,x}$, hence is local. Thus $f$ is a morphism of locally ringed spaces.
\end{proof}
\begin{rem*}
    In particular ($X=\Spec S$) any ring morphism $R\morphism[\phi] S$ defines
    \begin{align}\lbl{eq:specFunctor}
    \begin{split}
        \Spec \phi \colon \Spec S &\morphism \Spec R\\
        (\Spec \phi)(\qq) &= \phi^{-1}\qq\\
        (\Spec \phi)^*\left(\frac{r}{f}\right) &= \frac{\phi(r)}{\phi(f)} \in S_{\phi(f)} \simeq \Oo_{\Spec S}(\Spec S_f)
    \end{split}
    \end{align}
    turning $\Spec$ to a covariant functor from $\cat{Ring}^\op$ to $\cat{LRS}$ which is right adjoint to the global sections. 
\end{rem*}
\begin{cor}
    $\Spec \IZ$ is a finite object in $\cat{LRS}$. The unique morphism $X\morphism \Spec \IZ$ in $\cat{LRS}$ sends any $x\in X$ to the ideal in $\IZ$ generated by the characteristic of $k(x)$ and any fraction $\frac{a}{b}\in \Oo_{\Spec \IZ}(\IZ_b)$ to $\frac{a}{b} \in \Oo_X(X\setminus V(b))$.
\end{cor}
\begin{cor}
    If $R\doublemorphism[a][b] S$ is a pair of ring homomorphisms and $I$ the ideal $\left\langle a(r)-b(r)\st r\in R\right\rangle_S$ then 
    \begin{align*}
        \Spec S/I \morphism[\Spec \pi] \Spec S
    \end{align*}
    is an equalizer of 
    \begin{align*}
        \Spec S\doublemorphism[\Spec a][\Spec b] \Spec R\;.
    \end{align*}
    This follows from the fact that right adjoint functors preserve equalizers.
\end{cor}
\begin{cor}
    Let $R$ be a ring, $A$ and $B$ be $R$-algebras, and let 
    \begin{align*}
        A\morphism[\iota_A] A\otimes_R B \lmorphism[\iota_B] B
    \end{align*}
    given by $\iota_A(a) = a\otimes 1$ and $\iota_B(b) = 1\otimes b$. Then 
    \begin{align*}
        \Spec A \lmorphism[\Spec \iota_A] \Spec(A\otimes_R B) \morphism[\Spec \iota_B] \Spec B
    \end{align*}
    is a fibre product of $\Spec A$ with $\Spec B$ over $\Spec R$ in $\cat{LRS}$. This follows from the fact that $A\otimes_R B$ is a cofibre product of $A$ with $B$ over $R$ and the right adjoint functor $\Spec R$ preserves fibre products.
\end{cor}
\begin{prop}\lbl{prop:affineSchemeAdjunctionModules}
    ...
    \begin{align*}
        \Hom_{\Oo_{\Spec R}} (\snake M, \Nn) \morphism \Hom_R(M, \Nn(\Spec R))
    \end{align*}
    sending $\snake M \morphism[f] \Nn$ to the composition 
    \begin{align*}
        \phi\colon M\isomorphism \snake M (\Spec R) \morphism[f] \Nn(\Spec R)\;.
    \end{align*}
\end{prop}
\begin{proof}
    If $f$ and $\phi$ are as above, then we have a canonical diagram 
    \begin{diagram*}
	\node (M) at (0,1.5) {$M$};
	\node (N) at (3,1.5) {$\Nn(\Spec R)$};
	\node (Mp) at (0,0) {$M_\pp$};
	\node (Np) at (3, 0) {$\Nn_\pp$};
	\scriptsize
	\draw [->] (M) -- (Mp);
	...
	\draw [->] (N) -- (Np);
\end{diagram*}
in which the lower horizontal arrow is, in view of the universal property of $M\morphism M_\pp$ uniquely determined by $\phi$. Therefore, the morphism induced by $f$ on stalks, and hence $f$ itself, are uniquely determined by $\phi$. If $\phi$ is given and $\Bb$ denotes the topology base $\left\{\Spec R_\lambda\st \lambda\in R\right\}$ as before, we have for any such $U=\Spec R_\lambda$ a unique morphism $M_\lambda\morphism \Nn(U)$ given as the composition $M_\lambda \morphism[\phi] \Nn(\Spec R)_\lambda \morphism \Nn(U)$. Sheafifying gives
\begin{align*}
...
\end{defi}
\begin{proof}
    By Proposition~\reff{prop:RequalsGlobalSection} and its first corollary $\snake M(X\setminus V(\lambda)) \lisomorphism M_\lambda$, \itememph{b} follows from \itememph{a}.  
    
    Our construction and \itememph{b} imply that $c$ induces an isomorphism on sections on any element of the topology base $\Bb$, hence everywhere. 
    
    The last implication is obvious.
\end{proof}
\begin{cor}
    ...
    \begin{align*}
        ...
        \text{image of } \snake I \text{ under } \snake I \morphism \snake R \simeq\Oo_X &\longmapsfrom I\;.
    \end{align*}
\end{cor}


\section{Preschemes \texorpdfstring{\sout{of}}{of} and schemes}
YAY, FINALLY.

\begin{defi}[Prescheme] \lbl{def:prescheme}
    A locally ringend space $X$ is a \defemph{prescheme} if every $x\in X$ has an open neighbourhood $U$ such that the locally ringed space $(U,\Oo_X|U)$ is isomorphic to $\Spec R$ in $\cat{LRS}$, for some ring $R$. The category of preschemes is a full subcategory of $\cat{LRS}$, i.e. every morphism of locally ringed spaces between two preschemes is a morphism of preschemes.
\end{defi}
\begin{example}
    $\Spec R$ is a prescheme.
\end{example}
\begin{example}
    If $X$ is a prescheme and $U\subseteq X$ an open subset, $(U, \Oo_X|U)$ is a prescheme (called an open subprescheme of $X$).
\end{example}
\begin{example*}
    The empty set is a prescheme and is the initial object in both $\cat{Prescheme}$ and $\cat{LRS}$.
\end{example*}
\begin{defi}[Affine prescheme]\lbl{def:affinePrescheme}
    A prescheme $X$ is called \defemph{affine} if $X\simeq \Spec R$ in $\cat{LRS}$, for some ring $R$. An open subset $U\subseteq X$ is called affine if $(U,\Oo_X|_U)$ is affine prescheme.
\end{defi}
\begin{rem*}
...
\end{rem*}
\begin{rem*}
    By well-known facts, $\Spec R$ is quasi-coherent. Hence, any affine open subset is quasi-coherent. Schemes will be preschemes with an additional separation condition implying but not equivalent to the proper the intersection of two affine open subsets is affine, hence quasi-coherent. Also, recall the well-known fact $\left\{\Spec R\setminus V(f)\right\}$ is a topology base on $\Spec R$ consisting of affine open subsets. 
\end{rem*}
\begin{fact}
