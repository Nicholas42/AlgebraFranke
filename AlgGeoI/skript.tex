...
\begin{prop}\lbl{prop:equalizerLRS}
    ...
    \begin{itemize}
      ...
      \item 
        Let $\Oo_{K,[k]} = \Coeq\Big(\Oo_{Y,y} \doublemorphism[a^*][b^*] \Oo_{X,k}\Big)$ where $y=a(k) = b(k)$. The square brackets are to indicate that we do not yet know if $\Oo_{K,[k]}$ is really the stalk at $k$ of the yet-to-define sheaf $\Oo_K$ (spoiler: it is).
      \item 
...
\begin{proof}
    ...
    \begin{itemize}
      \item 
        $K$ is a topological space, $\kappa$ is continuous, $\Oo_{K,[k]}$ is a local ring (by \hyperref[fact:coequalizerOfRings]{this} basic fact about coequalizers of local ring morphisms, also see exercise sheet \#5), and $\Oo_{X,k} \morphism \Oo_{K,[k]}$ is local (it is, in fact, surjective).
      \item 
	...
	\begin{defi}[Coequalizer]
        ...
        \begin{quote}
        	 ...
        	 \begin{diagram}
        	 	...
        	 	\draw[->, bend right] (X) to node[pos=0.5, above] {$a$} (Y) ;
