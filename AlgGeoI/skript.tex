...
\begin{array}{c}
	 \tau_X(t)\in U\text{, }\tau_Y(t)\in V\text{ and the image of }\lambda\text{ in }\Oo_{X,x}\otimes_{\Oo_{S,s}}
	 \Oo_{Y,y}\\
	 \text{is } \notin (\tau^\flat)^{-1}(\mm_{T,t})\text{, where we set }s=\xi\big(\tau_X(t)\big)=\upsilon\big(\tau_Y(t)\big)
\end{array} \right\}\\
&=\left\{t\in \Theta \st \left(\text{image of }\lambda\text{ in }\Oo_{X,x}\otimes_{\Oo_{S,s}}
\Oo_{Y,y}\right) \notin (\tau^\flat)^{-1}(\mm_{T,t}) \right\}\\
&=\left\{t\in \Theta\st\left(\text{image of }\tau^\sharp\lambda\text{ in }\Oo_T(\Theta)\to \Oo_{T,t}\right)\notin\mm_{T,t}\right\}\\
