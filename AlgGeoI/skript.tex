...
\begin{proof}
	...
	Let $x\in X$ and suppose that $W\subseteq Y$ is an affine open neighbourhood of $a(x)$ and $b(x)$. We claim that $x$ has an open neighbourhood $U$ (namely $U=a^{-1}(W)\cap b^{-1}(W)$) such that $K\cap U$ is closed in $U$. This implies local closedness (if $x\in K$, take any affine open neighbourhood $W$ of $a(x)=b(x)$) and also the other assertion, cf. \hyperref[rem:localClosedness]{this} remark. To show the claim about $U$, note that
	\begin{align*}
...
\end{defi}
\begin{rem*}
	Maybe Grothendieck should have dropped the \emph{pre}fix \emph{pre} and rather have used the term \emph{scheme} instead of \emph{prescheme} in the first place, and indeed, in \emph{pre}sent days, this other notation is \emph{pre}dominating. However, Prefessor Franke \emph{pre}sumably \emph{pre}fers Grothendieck's \emph{pre}existing terminology.
\end{rem*}
\begin{fact}
	\begin{alphanumerate}
		...
		\item A morphism $X\morphism[f]Y$ is separated iff the embedding of the \emph{diagonal} 
		\begin{align*}
			\Delta=\text{image of }X\xrightarrow{(\id_X,\id_X)}X\times_YX
		\end{align*}
...
\begin{proof}
	...
	Let's prove \itememph{d}. If $f$ is separated, then $\Delta=\Eq\Big(X\times_YX\doublemorphism[p_1][p_2]X\Big)\monomorphism X\times_YX$ is a closed immersion. Conversely, if $\Delta$ is closed and $T\doublemorphism[\smash{a}][b]X$ is such that $fa=fb$, then we have a morphism $\tau\colon T\xrightarrow{\smash{(a,b)}}X\times_YX$ and 
	\begin{align*}
		K=\Eq\Big(T\doublemorphism[a][b]X\Big)\simeq T\times_{X\times_YX}\Delta\;.
	\end{align*} 
	The morphism $K\monomorphism T$ is thus a base change of $\Delta\monomorphism X\times_YX$ and hence a closed embedding by base change stability (Fact~\reff{fact:immersionsBaseChangeStable}).
...
\begin{proof}[Proof of Proposition~\reff{prop:intersectionAffine}]
    Let $U,V\subseteq X$ be affine open subsets of a scheme $X$. By Corollary~\reff{cor:fibreProductsInLRS}, their product $U\times V$ equals $p^{-1}(U) \cap q^{-1} (V)\subseteq X\times X$ (where $X\times X\doublemorphism[p][q] X$ are the projections to the factors) and is affine by Corollary~\reff{cor:affineSchemesFibreProduct}. The diagonal $\Delta=\text{image of }X\xrightarrow{\smash{(\id_X,\id_X)}}X\times X$ is closed by Fact~\reff{fact:schemeConditions}\itememph{e} and isomorphic to $X$, as $\Delta\xrightarrow{p|_\Delta=q|_\Delta}X$ is inverse to $X\morphism \Delta$. Then $\Delta\cap (U\times V)$ is affine as a closed subprescheme of the affine scheme $U\times V$ by Corollary~\reff{cor:closedSubpreschemes}. Then its preimage under the isomorphism $X\isomorphism\Delta$
    \begin{align*}
    	p|_\Delta\big(\Delta\cap (U\times V)\big)&=p|_\Delta\big(\Delta\cap p^{-1}(U)\cap q^{-1}(V)\big)\\
    	&=p|_\Delta\big(\Delta\cap p^{-1}(U)\big)\cap q|_\Delta\big(\Delta\cap q^{-1}(V)\big)=U\cap V
    \end{align*}
...
\begin{fact}\lbl{fact:qcToAffineIsQc}
    Let $X$ be quasi-compact and $Y$ affine, any morphism $X\morphism Y$ is quasi-compact.
\end{fact}
...
\begin{lem}\lbl{lem:qsmorphism}
    ...
    \item 
        The diagonal $X \xrightarrow{\Delta=(\id_X,\id_X)} X\times_Y X$ is a quasi-compact morphism.
    \end{alphanumerate}
\end{lem}
\begin{proof}
...
\end{prop}
\begin{proof}
    Assume \itememph{a} and let $U\subseteq X$ be open and $f,g\in \Oo_X(U)$ such that $fg = 0$. Then $U = V(f)\cup V(g)$ and, as $U$ is irreducible and $V(f)$ and $V(g)$ are closed in $U$, we have without loss of generality $U = V(f)$ hence $f\in\nil(\Oo_X)(U)$ by Fact~\reff{fact:nilRadical}. As $X$ is reduced, $f=0$, which shows \itememph{a} $\Rightarrow$ \itememph{b}.
...
\end{proof}
\begin{rem*}%I need to ask Franke about this.%not anymore
...
\begin{lem}\lbl{lem:propsOfAffine}
    Let $\Ee$ be a property of affine open subsets of a prescheme $X$ satisfying the following properties:
    \begin{alphanumerate}
    \item 
        $\Ee(V)$ implies $\Ee(V\setminus V(f))$ for all $f\in \Oo_X(V)$.
    \item 
        If $V$ is affine and $f_1,\ldots,f_n \in \Oo_X(V)$ such that $\bigcap_{i=1}^n V(f_i)=\emptyset$ and such that $\Ee(V\setminus V(f_i))$ for all $i$, then $\Ee(V)$ holds.
    \end{alphanumerate}
    Then the following conditions are equivalent:
    \begin{alphanumerate}
      \item[\itememph{\alpha}]
        $X$ may be covered by affine open subsets $U$ such that $\Ee$ holds for all of them.
    \item[\itememph{\beta}]
        $\Ee$ holds for arbitrary affine open subsets of $X$.
    \end{alphanumerate}
\end{lem}
\begin{proof}
    The implication \itememph{\beta} $\Rightarrow$ \itememph{\alpha} being trivial we have to show the other direction. We first have to show that $\Ee(U)$ holds when $U\subseteq V$ are affine and $\Ee(V)$ holds ($U$ and $V$ being affine open subsets). Since the $V\setminus V(\lambda)$ (for $\lambda\in \Oo_X(V)$) form a topology base on $V$, $U$ may be covered by such subsets which inherit $\Ee$ by \itememph{a}. But $V\setminus V(\lambda)$ equals $U\setminus V(\lambda)$ when it is a subset of $U$. By the quasi-compactness of $U$ and \itememph{b}, $\Ee(U)$ holds. 
    
    Now let $U$ be an arbitrary affine open subset of $X$. If $x\in U$, then $x$ is contained in some affine open subset $V$ of $X$ with $\Ee(V)$. Let $W\subseteq U\cap V$ be an affine open neighbourhood of $x$, we have $\Ee(W)$ by the initial step. We may actually assume $W=U\setminus V(\lambda)$ with $\lambda\in \Oo_X(U)$ as such $W$ form a topology base on $U$. Thus, $U$ may be covered by $W=U\setminus V(\lambda)$ with $\Ee(W)$. By the quasi-compactness of $U$ it has a finite covering of that type, and by \itememph{b}, $\Ee(U)$ holds.
\end{proof}
...
\begin{proof}[Proof of Lemma~\reff{lem:finiteTypeAffine}]
    Let $\Ee(V)$ be the property ``$\Oo_X(V)$ is of finite type over $A$'' of affine open $V\subseteq X$. If $V\subseteq X$ is affine having property $\Ee$ and $\lambda\in\Oo_X(V)$, then $\Oo_X(V\setminus V(\lambda))\simeq\Oo_X(V)_\lambda=\Oo_X(V)[\lambda^{-1}]$ is of finite type over $\Oo_X(V)$ and thus over $A$ as well, i.e. $\Ee$ has property \itememph{a} of Lemma~\reff{lem:propsOfAffine}. By Lemma~\reff{lem:finiteTypeOnGenerators}, it also has property \itememph{b}. Applying Lemma~\reff{lem:propsOfAffine} to $\Ee$ gives Lemma~\reff{lem:finiteTypeAffine}.
\end{proof}
...
\begin{example}
    Let $S= \Spec R$, $X=\IA^1_R=\Spec R[T]$ (the \emph{affine line} over $R$), and  $\IP_R^1=\Proj \left(R[T_0,T_1]\right)$. We have a diagram
    \begin{diagram*}
    	\node[ob](x) at (0,1.25) {$X$};
    	\node[ob](s) at (1.25,0) {$S$};
...
\begin{thm}[Serre] \lbl{thm:serre}
    For a Noetherian local ring $R$, with residue field $k$, the following are equivalent:
    \begin{alphanumerate}
    \item 
        $k$ admits a finite free resolution, i.e. an exact sequence
            \begin{align*}
                0 \lmorphism k \lmorphism F_0 \lmorphism F_1 \lmorphism \ldots \lmorphism F_n \lmorphism 0
            \end{align*}
        where the $F_i$ are free.
    \item 
        $k$ admits a finite flat resolution, i.e. an exact sequence
            \begin{align*}
                0 \lmorphism k \lmorphism F_0 \lmorphism F_1 \lmorphism \ldots \lmorphism F_n \lmorphism 0
            \end{align*}
    ...
    \item 
        Equality occurs in the inequality $\dim R \leq \dim (\mm/\mm^2)$.
    \end{alphanumerate}
\end{thm}
...
\begin{defi}[Normal ring]\lbl{def:normal}
    A domain is called \defemph{regular} if it is integrally closed in its field of quotients. It is clear that this is inherited by localization. 
\end{defi}
...
\begin{rem*}
    \begin{alphanumerate}
    \item   
        If $X$ is quasi-compact or Jacobson and $\Oo_{X,x}$ regular for closed $x$, then $X$ is regular. (Let $x\in X$, by Proposition~\reff{prop:closedSubsetPoint}, $\ov{\{x\}}$ contains a closed point $y$. Then $\Oo_{X,y}$ is regular as is its localization $\Oo_{X,x}$ by Theorem~\reff{thm:serre}) 
    \item   
        Note that $\left\{x\in X\st \Oo_{X,x} \text{ singular}\right\}$ may fail to be closed, despite Theorem~\reff{thm:serre}. (We think singular means not regular)
    \item 
        For a domain $A$ with field of quotients $k$, one has $A=\bigcap_{\pp\in \Spec A}  A_\pp = \bigcap_{\mm\in\mSpec} A_\mm$. Consider $r$ in this intersection and $I=\left\{a\in A\st ar\in A\right\}$ then $I$ fails to be contained in any maximal ideal of $A$, while being an ideal. It easily follows that $A$ is normal iff all $A_\mm$ (or all $A_\pp$ are. Hence, $X$ being normal is equivalent to $\Oo_X(U)$ being a normal domain for any open, affine, connected, non-empty $U$. This is equivalent to $X$ may be covered by affine open $U$ such that $\Oo_X(U)$ is a normal domain. When $X$ is quasi-compact or Jacobson, this is equivalent to $\Oo_{X,x}$ being a normal domain when $x$ is closed in $X$.
    \item 
        Let $R_k$ be the condition: $\Oo_{X,x}$ is regular when $\codim(\ov{\{x\}}, X) \leq k$. Then $R_0$ may be expressed as the regularity of $\Oo_{X,\eta}$ where $\ov\eta$ is an irreducible component of $X$, which for zero-dimensional local rings is equivalent to $\Oo_{X,\eta}$ being a field or to $\nil(\Oo_{X,\eta})=0$. By (probably) Serre, $X$ is reduced iff $X$ is $R_0$ and $\Oo_X(U)\morphism \Oo_X(V)$ are injective when $V$ is dense in $U$ (i.e. if $X$ is $S_1$). By the normality criterion of Serre, $X$ is normal iff $X$ is $R_1$ and $\Oo_X(U)\morphism \Oo_X(V)$ are injective when $V$ is dense in $U$ and $\Oo_X(U)\morphism \Oo_X(V)$ is an isomorphism if all irreducible of $U\setminus V$ are of codimension greater than $1$ (i.e. if $X$ is $S_2$). The $S_k$ are related Cohen-Macaulayness (which is $S_\infty$).
    \end{alphanumerate}
\end{rem*}
...
\begin{defi}[Finite and integral morphism]\lbl{def:finiteIntegralMorphism}
    ...
    \end{alphanumerate}
\end{defi}
The proof of equivalence in Definition~\reff{def:finiteIntegralMorphism} is by Lemma~\reff{lem:propsOfAffine} applied to the property $E(U)$:
\begin{quote}
    $\Oo_X(f^{-1}(U))$ is finite/integral over $\Oo_Y(U)$.
\end{quote}
which by $\Oo_Y(U\setminus V(\lambda))\simeq \Oo_Y(U)_\lambda$ and $\Oo_X(f^{-1}(U\setminus V(\lambda))) \simeq \Oo_X(f^{-1}(U))_\lambda$ (it is easy to see that any morphism with Property \itememph{b} of Definition~\reff{def:affineMorphism} is quasi-compact and quasi-separated) and finiteness/integrality of the ring extension is preserved in these situations and can be tested on such coverings (an $R$-module $M$ is finitely generated iff $\Spec R$ can be covered $\Spec R = \bigcup_{i=1}^n \Spec R_{f_i}$ such that $M_{f_i}$ is finitely generated).

There is a sheaf version of $\Spec$ as follows:
Let $\Aa$ be any quasi-coherent $\Oo_X$-algebra, then
\begin{align*}
    \SPEC(\Aa) = \left\{(x,\pp) \st x\in X, \pp \in \Spec(A_x) \text{ such that the preimage of }\pp\text{ under } \Oo_{X,x}\morphism \Aa_x \text{ is } \mm_{X,x}\right\}
\end{align*}

Topology base:
\begin{align*}
    \Omega(U,\lambda) = \left\{ (x,\pp) \st x\in U, \pp\not\ni (\text{image of } \lambda \text{ under } \Aa(U)\morphism \Aa_X)\right\}
\end{align*}
...
Pretender stalk:
\begin{align*}
    \Oo_{Y,[x,\pp]} \coloneqq (\Aa_x)_\pp
\end{align*}

Coherence condition:
\begin{align*}
    \Oo_Y(W) = \left\{(\alpha_{x,\pp})\in \prod_{(x,\pp)\in W} \Oo_{Y,[x,\pp]}\st \text{Every } w\in W\text{ has a neighbourhood } \Omega(U,\lambda) \text{ for which there is } a\in \Aa(U)_\lambda \text{ such that } \alpha_{x,\pp} \text{ is the image of } a\text{ under } \Aa(U)_\lambda \morphism (\Aa_x)_\lambda \morphism (\Aa_x)_\pp = \Aa_{[x,\pp]} \text{ whenever } (x,\pp)\in W\cap \Omega(U,\lambda)\right\}
\end{align*}

As in the previous parts of the lecture, one checks that the pretender stalks are canonically isomorphic to the actual stalks. Hence $Y$ is a locally ringed space. It is also easy to check that $(x,\pp)\morphism x$, $Y\morphism X$ is continuous (the preimage of $U$ is $\Omega(U, 1)$) and one has a morphism $Y\morphism X$ of locally ringed spaces, $p(x,\pp) = x$, $p^* f$ are the imaged of $f$ under $\Oo_X(U) \morphism \Oo_{X,x} \morphism \Aa_x \morphism (\Aa_x)_\pp$. One then notes that $p^{-1} U\simeq \SPEC(\Aa|_U)$. This reduces the question of $\SPEC(\Aa)$ being a preschme to the case where $X=\Spec R$ is affine. In this case, one has $\Aa= \snake A$ where $A$ is an $R$-algebra, and it is natural to assume 
\begin{align*}
    \SPEC(\Aa)&\isomorphism \Spec A\\
    (\qq\sqcap R, \qq \cdot A_x) &\longmapsfrom \qq\\
    (x,\pp)  &\longmapsto (\text{the preimage of }\pp\text{ under } A\morphism A_x \simeq \Aa_x)\;.
\end{align*}
One has $(\Oo_{\SPEC \Aa})_{(x,\pp)} = (\Aa_x)_\pp \simeq (A_x)_\pp \simeq A_\qq \simeq (\Oo_{\Spec A})_\qq$ and the identification of the coherence conditions is also easy.

\begin{prop}
    Let $S$ be a prescheme, $\Aa$ any quasi-coherent $\Oo_S$-algebra, $X\morphism[\xi] S$ any $S$-prescheme. Then one has a bijection
    \begin{align*}
        \Hom_{\cat{S-PreSch}} (X, \SPEC(A)) \simeq \Hom_{\cat{\Oo_S-Alg}} (\Aa, \xi_* \Oo_X)
    \end{align*}
    sending $X\morphism[f] \Spec \Aa$ to $\Aa \simeq p_* \Oo_{\Spec \Aa} \morphism[p_*(f^*)] p_*f_* \Oo_X = \xi_* \Oo_X$. ($p\colon:\SPEC \Aa \morphism S$).
\end{prop}
\begin{proof}
    As 
    \begin{align*}
        U&\longmapsto \Hom_{\cat{U-PreSch}} (\xi^{-1}(U), p^{-1}(U)) = \Hom_{\cat{S-PreSch}}(\xi^{-1}(U), p^{-1}(U))\\
        U&\longmapsto \Hom_{\cat{\Oo_U-Alg}} (\Aa_U, \xi_*\Aa|_U) = \Hom_{\cat{\Oo_U-Alg}}(\Aa|_I, (\xi|_{\xi^{-1}(U)})_* \Oo_{\xi^{-1}}(U))\;.
    \end{align*}
    are easily seen to be sheaves of sets on $S$, it is sufficient to check this on the affine open subsets of $S$ where it follows from the similar adjunction in the end of 1.4.
\end{proof}

\begin{lem}
    A morphism of preschemes is affine if and only if it is quasi-compact and quasi-separated and the canonical morphism
    \begin{align}
        X\morphism \SPEC(f_*\Oo_X)\lbl{eq:isomAffine}
    \end{align}
    (corresponding to $\id_{f_*\Oo_X}$ under the previous adjunction) is an isomorphism.
\end{lem}
\begin{proof}
    Assume \itememph{b} from Definition~\reff{def:affineMorphism}. Over the affine open $U$ such that $V= f^{-1}(U)$ is affine, we have
    
    DIAGRAM
    
    where the rightward arrows are isomorphisms. Hence \eqreff{eq:isomAffine} is an iso over open $U$ such that $f^{-1}(U)$ is affine (and f is quasi-compact and quasi-separated over such $U$). If $X$ may be covered by such $U$ the conditions hold globally.
    
    We obtain \itememph{a} from this by similar arguments. 
\end{proof}
\begin{cor}
    The clauses of affine and of finite morphisms are stable under composition and base-change and are base-local.
\end{cor}
\begin{prop}
    If $X\morphism[f] Y$ is finite and $A$ a closed subset of $X$, then $f(A)$ is closed in $Y$.
\end{prop}
\begin{proof}
    The question is local with respect to $Y$, hence we may assume that $Y$ is affine, e.g. $\Spec R$. The morphism being affine, $X$ is also affine, e.g. $X= \Spec S$, where $S$ is an $R$-algebra and a finitely generated $R$-module. If $A=V(I)$ we have $A\simeq \Spec(S/I)$. Replacing $S$ by $S/I$, we may assume that $A=X$ and have to show $f(X)$ is closed. Let $J = \ker(R\morphism S)$, then $f(X)\subseteq V(J)$ and it is sufficient to show equality. We may replace $R$ by $R/J$ and assume $R\morphism S$ to be injective. By a well-know result of integral ring extensions, $\Spec S\morphism \Spec R$ is surjective in this case. 
\end{proof}

\begin{rem}
    In fact, integrality is sufficient for the asserted and this proof is valid.
\end{rem}

\begin{defi}[Universally closed morphism]\lbl{def:univClosed}
    A morphism $X\morphism Y$ is closed if $f(A)\subseteq Y$ is closed whenever $A$ is closed in $Y$. It is \defemph{universally closed} if all its base-changes $\snake X = X\otimes_Y \snake Y \morphism \snake Y$ are closed.
\end{defi}
\begin{rem}
    \begin{alphanumerate}
    \item 
...
\begin{example}
    ...
    \end{align*}
    The structure sheaf has stalks $\Oo_{P,[\pp]} \simeq (R_\pp)_0$ and $\Oo_P(U)$ is the set of all $(\phi_\pp)\in \prod_{\pp\in U} \Oo_{P,[\pp]}$ which locally on open subseteq of the form $P\setminus V(\lambda)$ ($\lambda$ homogenous) must be of the form where $\pp$ is the image of $f$ under $(R_\lambda)_0 \morphism (R_\pp)_0$ where $f\in (R_\lambda)_0$. 
    
    One shows that $P\setminus V(\lambda) \simeq \Spec((R_\lambda)_0)$ when $\lambda$ has positive degree. One can use this to derive $P\times_{\Spec R_0} \Spec S \simeq \Proj(R\otimes_{R_0} S)$. Note that $\Proj$ is only functional with respect to $R\morphism S$ such that $S_+$ is generated by the image of $R_+$ as an ideal.
\end{example}
\begin{example}
    Let $R_0$ be any ring, $A$ an $R_0$-algebra and $R_k = A$ for $k>0$ with the obvious ring operations. Then $P\setminus V(\lambda) \simeq \Spec((R_\lambda)_0$ may be applied where $\lambda = 1\in A$ viewed in degree $1$ and shows $\Proj(R)\simeq \Spec A$ as a $\Spec R_0$-prescheme. In particular, $\Proj(R)\morphism \Spec R_0$ may easily fail to be closed.
\end{example}
If we assume that $R_1$ is a finitely generated $R_0$-module and $R_+$ generated by $R_1$ as an ideal (or $R$ by $R_1$ as $R_0$-algebra), then the sequence of ideals 
\begin{align*}
    I_k = \Ann_{R_0}\left(\bigoplus_{\ell\leq k} R_\ell\right) = \Ann_{R_0} (R_k)
\end{align*}
is ascending. Let $I_\infty = \bigcup_{k=1}^\infty I_k$.

\begin{prop}
    In this situation, the image of $\Proj$ in $\Spec R_0$ equals $V(I_\infty)$.
\end{prop}
\begin{proof}
    ...
    \begin{align*}
        ...
        &= V(\Ann(M))
    \end{align*}
    by well-known facts from commutative Algebra (which you should be familiar with, otherwise, you may want to think about attending Algebra $\mathrm{II}$ instead). The second to last step is done by applying the Lemma of Nakayama, Azumaya and Krull (cf. \cite[Proposition~1.2.3]{alg2}). Hence 
    \begin{align*}
        V(I_\infty) = \left\{\pp\in \Spec R_0\st (R_k\otimes_{R_0} \KK(\pp))\neq 0 \text{ for arbitrarily large } k\right\}
    \end{align*}
    but $\Proj(R\otimes_{R_0} \KK(\pp)) = P\times_{\Spec R_0} \Spec(\KK(\pp))$ which as a set is the fibre of $P\morphism \Spec R_0$ over $\pp$, by the description of a fibre product with the immersive morphism $\Spec \KK(\pp) \morphism R_0$. Hence one hast to show, that for any finitely generated algebra $\snake R$ over a field $k$ generated by $\snake R_1$ as a $k$-algebra $\Proj(R)\neq 0$ if and only if $\snake R_k \neq 0$ for arbitrarily large $k$. But then there must be $f\in \snake R_1$ which is not nilpotent hence $\snake R_f \neq 0$ and $\Spec(\snake R_f)_0 \simeq \Proj(\snake R\setminus V(f))$ is not empty. If $\snake R_k = 0$ for large $k$ then $\snake R_+ = \sqrt{0}$ and $\Proj(\snake R) = \emptyset$.
\end{proof}
\begin{cor}
    The morphism $\Proj(R) \morphism \Spec R_0$ is closed under the above assumption.
\end{cor}
\begin{proof}
    We have seen that its image is closed and the same holds for the image of $V(I) \simeq \Proj(R/I)$ for any homogenous ideal $I$.
\end{proof}
\begin{rem}
    In fact, it is sufficient to assume $R_+$ to be finitely generated.
\end{rem}
\begin{rem}
    Closedness of a morphism $X\morphism Y$ being base-local, universal closedness already follows when all base-changes of $f$ by $\Spec A \morphism Y$ are closed.
\end{rem}
\begin{cor}
...
\begin{example}
    \begin{align*}
        \IP^n(R) = \Proj(R[X_1,\ldots, X_n])
    \end{align*}
\end{example}
\begin{rem}
    There seems to be no universally adopted notion of ``projective morphism''. Grothendieck has a sheaf-$\mathbf{Proj}$ (obtained from $\Proj$ like $\SPEC$ from $\Spec$) and defines an $S$-prescheme $X$ (or the morphism $X\morphism[\xi] S$ to be projetive iff $X= \mathbf{Proj}(\Rr)$ where $\Rr$ is a quasi-coherent $\IN$-graded $\Oo_S$-algebra such that $\Rr_1(U)$ is a finitely generated $\Oo_S(U)$-module when $U$ is affine, and generated $\Rr_+(U)$ in that case.
    
    This class is not base-local. One could require this locally or require that locally $\xi^{-1}(U) \subseteq \IP^n(U)$ ($U$ affine, $n$ depending on $U$).
\end{rem}
...
\end{prop}
\begin{proof}
    Let $U_i\subseteq X$ be affine open subsets covering $X$, $U_i\simeq \Spec A_i$ where $A_i\simeq R[X_1,\ldots, X_{n_i}]/I_i$ giving a closed embedding $U_i\morphism \IA^{n_i}(R)$. Let $U = \bigcap_i U_i$, we have embeddings 
    \begin{align*}
        U\morphism X\times \prod_i \IA^{n_i}(R)\morphism X\times\prod_i \IP^{n_i}(R)\;.
    \end{align*}
    Let $\snake X$ be the closure of the image of $U$ under that embedding (reduced subprescheme), then $\snake X\morphism X$ has the desired properties (e.g. is an isomorphism over $U$) and $\snake X\morphism \prod_i \IP^{n_i}(R)$ (projection to the second factor) is a closed embedding.
\end{proof}
