...
\begin{proof}[Proof of Proposition~\reff{prop:locFiniteMorph}]
    ...
    \end{diagram*}
     ...
     At the price of possibly making $f^*\lambda$ another generator, $\Oo_X(U_{i,j})_{f^*\lambda}$ is of finity type over $\Oo_Y(V)$ if it is over $\Oo_Y(V)_\lambda$ and the assertion follows from Lemma~\reff{lem:finiteTypeAffine} for $U\morphism V\simeq\Spec\Oo_Y(V)$.
\end{proof}
