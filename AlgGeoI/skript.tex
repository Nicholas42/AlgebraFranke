...
\end{proof}
\begin{cor}
    There is a canonical bijection from the quasi-coherent ideals $\Jj$ on $X=\Spec R$ to the ideals $I\subseteq R$ by 
    \begin{align*}
    \left\{
    \begin{array}{c}
...
\begin{fact}
    Let $X$ be a topological space, $\Rr$ a sheaf of rings on it. Let $U\subseteq X$ be open. Then the following conditions to $r\in\Rr(U)$ are equivalent:
    \begin{alphanumerate}
    \item   
...
\end{fact}
\begin{proof}
    If $U$ is quasi-compact, this follows from corollary 1.5.2. The question being local, the assertion follows in general.
\end{proof}
...
\end{prop}
\begin{proof}
    First, we show that $\nil(\Oo_X)$ is quasi-coherent. It suffices to show that $\nil(\Oo_X) = \snake{\nil(R)}$ when $X=\Spec R$. Since these open subsets form a topology base, it is sufficient to show $(\nil(\Oo_X))(\Spec R_f) = \nil(R)_f$. As $\Spec R_f$ is quasi-compact the left hand side is $\nil(R_f)$ hence we have to show $\nil(R)_f \isomorphism \nil(R_f)$. Injectivity follows from the injectivity of $\nil(R)\morphism R$. For surjectivity, let $\frac{r}{f^k}\in \nil(R_f)$, then $\frac{r^m}{f^{km}} = 0$ in $R_f$ for appropriate $m$, hence $f^\ell r^m = 0$ in $R$ for some $\ell\in\IN$, and $(rf^\ell) = 0$ in $R$, hence $f^\ell r\in \nil(R)$ but $\frac{r}{f^k}=\frac{f^\ell r}{f^{\ell+k}}\in \nil(R)_f$.
    
    Assume \itememph{a}. If $Y\subseteq X$ is a closed subprescheme containing all points of $X$, it is given by a quasi-coherent sheaf of ideals $I$. For $x\in X=Y$, we have $\Oo_{Y,x}= \Oo_{X,x}/I_x$ hence for any $f\in \Oo_X(U)$ we have $V(f) = V(f|_{U\cap Y})$. For $f\in I(U)$, $f|_{Y\cap U} = 0$ hence $V(f) = U$ hence $f\in (\nil(\Oo_X))(U)$ hence $f=0$ by \itememph{a}. Hence $I=0$ and $X=Y$.
    
    In the other direction, one can easily see, that $\nil(\Oo_X) \neq 0$ defines a closed subprescheme containing all the points of $X$.
\end{proof}
...
\end{rem}
\begin{fact}
    Let $X$ be a reduced prescheme, $Y$ a scheme and $X\doublemorphism[f][g] Y$ two morphisms of preschemes coinciding on a dense open subset $U\subseteq X$. Then they coincide on $X$. In fact, $Y$ being a scheme can be weakend to the existence of a separated morphism $Y\morphism[\upsilon] S$ such that $\upsilon f = \upsilon g$.
\end{fact}
\begin{proof}
    Under our assumptions on $Y$, the equalizer $\Eq\left(X\doublemorphism[f][g] Y\right)$ is a closed subprescheme of $X$ containing all points of $U$, hence of $X$. By Proposition~\reff{prop:reduced} $f$ and $g$ coincide on $X$.
\end{proof}
\begin{rem}
    It is easy to see that the scheme property of a prescheme $Y$ or the separateness of the morphisms $Y\morphism[\upsilon] S$, can be characteristized by the validity of the above assertion for reduced $X$. In fact, it is sufficient to take $X=\Spec R$ where $R$ is a domain, or a general valuation ring, or, when $Y$ is Noetherian, a discrete valuation ring.
\end{rem}
\begin{prop}
    \begin{alphanumerate}
    \item
        ...
        \begin{align*}
            ...
            \left\{(U,f)/_\sim \in \Oo_{X,x}\st y\in V(f)\right\} &\longmapsfrom y\;.
        \end{align*}
    ...
    \item
        The morphism $\Spec (\Oo_{X,x}) \morphism X$ defined by 
        \begin{align*}
            \Spec(\Oo_{X,x} ) = \Spec(\Oo_X(U)_{\pp_x}) \morphism \Spec(\Oo_X(U)) = U \morphism X
        \end{align*}
        for an affine open neighbourhood $U$ of $x$ ($\pp_x\in \Spec(\Oo_X(U))$ being the ideal of $f\in \Oo_X(U)$ such that $x\in V(f)$) is independent of $U$ and defines the bijection of \itememph{a} on the level of points.
    \end{alphanumerate}
\end{prop}
\begin{proof}
    For \itememph{a}, we may assume $X$ to be affine because any affine open neighbourhood of $x$ contains all $y$ occuring on the right hand side. When $X=\Spec R$, and $x$ is given by $\pp$ and $y$ by $\qq$ then $\ov{\{y\}} = V(\qq)$ hence $\qq$ defines an element of the right hand side iff $\qq\supseteq \pp$. In this case, the image of $y$ in the left hand side $\Spec(\Oo_{X,x})\simeq \Spec R_\pp$ is the set of all $\frac{r}{f}\in R_\pp$ such that $\qq R_f$ is an element of the vanishing set of $\frac{r}{f}$ in $R_f$, hence $\frac{r}{f}\in \qq R_f$, hence the ideal of $R_\pp$ associated to $y$ is $\qq R_\pp$. This is indeed a bijection onto $\Spec R_\pp$, the inverse map being given by $\Spec R_\pp \morphism \Spec R$, $\gamma\mapsto \qq= \gamma\sqcap R$.
    
    Now we prove \itememph{b}. The fact that the map on points defined by that morphism is the inverse to $\lmorphism$ in \itememph{a} follows from the last remark. In particular, it is unique on the level of points. On the level of stalks of sections of the structure sheaf it is given by the canonical morphism (for a $y\in X$ such that $x\in\ov{\{y\}}$)
    \begin{align*}
        \Oo_{X,x} = \colimit[U\ni x] \Oo_X(U) \morphism \colimit[V\ni y] \Oo_X(U)
    \end{align*}
    mapping the complement of the prime ideal defined by $y$ to units, hence defining $(\Oo_{X,x})_\qq\morphism \Oo_{X,y}$ ($\qq$ being the image of $y$ under $\lmorphism$) by the universal property of the localization. This is the $(\Oo_{X,x})_\qq \simeq (\Oo_{\Spec \Oo_{X,x}})_\qq \morphism \Oo_{X,y}$ defined by the morphisms of \itememph{b}.
\end{proof}
