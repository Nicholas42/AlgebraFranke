...
\begin{defi}[Immersive morphisms] \lbl{def:immersiveMorphism}
    ...
    If $\upsilon(Y)\subseteq S$ is a closed subset, $\upsilon$ is called a \defemph{closed immersion}. If it is open, $\upsilon$ is an \defemph{open immersion}.
\end{defi}
...
\end{cor}
\begin{rem*}
	 In particular, the base change of a closed immersion is itself closed and likewise the base change of an open immersion is open again.
\end{rem*}
...
\begin{fact}\lbl{fact:qcmorphism}
    ...
    \item 
        $X$ may be covered by affine open subsets such that $f^{-1}(U)$ is quasi-compact.
    \end{alphanumerate}
\end{fact}
\begin{proof}
...
\begin{proof}[Proof of Proposition~\reff{prop:locFiniteMorph}]
    The implication \itememph{a} $\Rightarrow$ \itememph{b} is trivial. Now assume \itememph{b}. By \itememph{b}, $X$ may be covered by affine open $U_i$ such that $f(U_i)\subseteq V_i$ with $V_i\subseteq Y$ affine and $\Oo_X(U_i)/\Oo_Y(U_i)$ of finite type. Let $U\subseteq X$, $V\subseteq Y$ as in \itememph{a}. Then $U$ my be covered by affine open subsets $U=\bigcup_{i,j}U_{i,j}$ such that $U_{i,j}\subseteq U_i$. Then $\Oo_X(U_{i,j}) /\Oo_Y(V_i)$ is of finite type by Lemma~\reff{lem:finiteTypeAffine}. But $V_i\cap V$ may be covered by $V\setminus V(\lambda)$ and then $\Oo_X(U_{i,j}\setminus V(f^*\lambda)) \simeq \Oo_X(U_{i,j})_{f^*\lambda}$ is of finite type over $\Oo_Y(V\setminus V(\lambda))\simeq \Oo_Y(V)_\lambda$. 
    
    Indeed, we have $V(f^*\lambda)=f^{-1}(V(\lambda))$ by Definition~\reff{def:localMorphisms}\itememph{b}, moreover by transitivity of the restriction morphisms in $\Oo_X$ and the defining property of the morphism of sheaves $\Oo_Y\morphism[f^*]f_*\Oo_X$ we obtain a commutative diagram
    \begin{diagram*}
    	\node[ob] (OY) at (0,1.5) {$\Oo_Y(V_i)$};
    	\node[ob] (OYl) at (4,1.5) {$\Oo_Y\big(V\setminus V(\lambda)\big)$};
    	\node[ob] (OX) at (0,0) {$\Oo_X(f^{-1}V_i)$};
    	\node[ob] (OXl) at (4,0) {$\Oo_X\big(f^{-1}V\setminus V(f^*\lambda)\big)$};
    	\node[ob] (OXU) at (2,-1.25) {$\Oo_X\big(U_{i,j}\setminus V(f^*\lambda)\big)$};
    	\scriptsize
    	\draw[->] (OY) -- (OX) node[pos=0.5, left] {$f^*$};
    	\draw[->] (OYl) -- (OXl) node[pos=0.5, right] {$f^*$};
    	\draw[->] (OY) -- (OYl);
    	\draw[->] (OX) -- (OXl);
    	\draw[->] (OX) -- (OXU);
    	\draw[->] (OXl) -- (OXU);
    \end{diagram*}
     In particular, that $\Oo_X(U_{i,j})_{f^*\lambda}$ is of finity type over $\Oo_Y(V)_\lambda$ follows from the fact that it is of finite type over $\Oo_Y(V_i)$, which again is due to $\Oo_X(U_{i,j})_{f^*\lambda}=\Oo_X(U_{i,j})[(f^*\lambda)^{-1}]$ being of finite type over $\Oo_X(U_{i,j})$ being of finite type over $\Oo_Y(V_i)$. 
     
     At the price of possibly making $f^*\lambda$ another generator, $\Oo_X(U_{i,j})_{f^*\lambda}$ is of finite type over $\Oo_Y(V)$ if it is over $\Oo_Y(V)_\lambda$ and the assertion follows from Lemma~\reff{lem:finiteTypeAffine} for $U\morphism V\simeq\Spec\Oo_Y(V)$.
     
     
     
     Alternative proof: Let $U_i$, $V_i\supseteq f(U_i)$ be affine such that $\Oo(U_i)/\Oo(V_i)$ is of finite type.
     
     Step 1: It is possible to cover $f^{-1}(V)$ by affine $\snake U_i$ for which there is an affine $\snake V_i$ such that $f(\snake U_i) \subseteq \snake V_i\subseteq V$ and $\Oo_X(\snake U_i)/\Oo_X(\snake V_i)$ is of finite type. Indeed, if $x\in f^{-1}(V)$ then $x\in U_i$ and as open subsets of this form form a topology base there exists $\lambda \in \Oo_Y(V_i)$ such that $f(x)\not\in V(\lambda)$ and $\snake V_i = V_i\setminus V(\lambda) \subseteq V$. Then, with $\snake U_i = U\setminus V(f^*(\lambda))$, we have $x\in\snake U_i$, $\Oo_X(\snake U_i)\simeq \Oo_X(U_i)_\lambda$ is of finite type over $\Oo_X(U_i)$ hence $\Oo_Y(V_i)$, hence $\Oo_Y(\snake V_i)$.
     
     Step 2: It is possible to do the same thing with $\snake V_i = V\setminus V(\lambda)$. Indeed, open subsets of this form being a topology base, it is possible to find $\lambda_{i,j}\in \Oo_Y(V)$ such that $V\setminus V(\lambda_{i,j}$ are subsets of $\snake V_i$ covering it. Then the $\snake U_{i,j} = \snake U_i\setminus V(f^*(\lambda_{i,j}))$ cover $\snake U_i$ and $\Oo_X(\snake U_{i,j}) \simeq \Oo_X(\snake U_i)_\lambda$ is of finite type over $\Oo_X(\snake U_i)$ hence $\Oo_Y(\snake V_i)$ hence $\Oo_Y(V\setminus V(\lambda_{i,j}))$.
     
     Step 3: In step 2, $\Oo_Y(\snake V_i) \simeq \Oo_Y(V)_\lambda$ is of finite type over $\Oo_X(V)$. Hence $\Oo_X(\snake U_i)/\Oo_X(V)$ is of finite type and an application of the Lemma to $f^{-1}(V)\morphism V$ gives $\Oo_X(U)/\Oo_X(V)$ finite type.
\end{proof}

\begin{defi}[Morphism of (locally) finite type]\lbl{def:morphismFiniteType}
    A morphism is of \defemph{locally finite type} if it satisfies these equivalent conditions. It is of \defemph{finite type} if it is quasi-compact and of locally finite type.
\end{defi}

\begin{fact}
    If $Y=\Spec R$, the structure of a $Y$-prescheme on $X$ is equivalent to the structure of a sheaf of $R$-algebras on $\Oo_X$ and $X$ is locally of finite type over $Y$ iff the $\Oo_X(U)$ for affine $U$ are $R$-algebras of finite type, and it is of finite type, iff it is in addition quasi-compact.
\end{fact}
\begin{fact}
...
\end{fact}
\begin{proof}
    Since the class of quasi-compact morphisms have these properties, it suffices to prove them for morphisms of locally finite type. 
    
    If $W\subseteq Y$ is open, it is cleat that $f^{-1}(W)\morphism W$ inherits the property \itememph{a} from Proposition~\reff{prop:locFiniteMorph} from $f$. Conversely, let $Y$ be covered by $W_i$ for which $f^{-1}(W_i)\morphism W_i$ is of locally finite type. Then $f^{-1}(W_i)$ may be covered by $U$ as in property \itememph{b} of Proposition~\reff{prop:locFiniteMorph}. Hence $X$ may be covered this way. This proves base-locality.
    
    By the transitivity of ``finite type'' for ring extensions, property \itememph{a} of Proposition~\reff{prop:locFiniteMorph} is stable under composition.
    
    By base-locality, it is sufficient to consider the base-change of $X\morphism \Spec A$ by $\Spec B\morphism \Spec A$. Then $X\morphism \Spec A$ satisfies the equivalent conditions of Lemma~\reff{lem:finiteTypeAffine}. It is possible to cover $X$ by affine $U_i\simeq \Spec(C_i)$ and $X\times_{\Spec A} \Spec B$ is covered by the $U_i\times_{\Spec A} \Spec B \simeq \Spec(C_i\otimes_A B)$ and $C_i\otimes_A B/B$ is of finite type, being generated by the $c_{i,j}\otimes 1$ over $B$ if the $(c_{i,j})_{j=1}^{n_i}$ generate $C_i/A$.
\end{proof}
\begin{rem*}
    Being of locally finite type is also local in the sens that it is inherited by $U\morphism[f] V$ form $X\morphism[f] Y$ if $U$ and $V$ are open with $f(U)\morphism V$ and that it holds for $f$ if $X$ may be covered by such open subsets $U$ (for which an appropriate $V$ exists).
\end{rem*}
\begin{rem*}
    $\id_X$ is of finite type as Proposition~\reff{prop:locFiniteMorph}\itememph{b} obviously holds.
\end{rem*}
\begin{cor}
    If $U\subseteq V$ is an inclusion of affine open subsets of a prescheme, $\Oo_X(U)/\Oo_X(V)$ is of finite type.
\end{cor}
\begin{cor}
...
\end{cor}
\begin{cor}
    Any locally closed immersion of locally finite type.
\end{cor}
...
\begin{prop}\lbl{prop:locNoetherian}
    ...
    \begin{alphanumerate}
    \item 
        For any quasi-compact open $U\subseteq X$, any ascending sequence $\Jj_0\subseteq \Jj_1\subseteq\ldots \subseteq \Oo_X|_U$ of quasi-coherent sheaves of ideals on $U$ terminates at some $i$ wih $\Jj_i= \Jj_k$, when $k\geq i$.
    \item 
...
\end{prop}
\begin{proof}
    Since there is a bijection between quasi-coherent sheaves of ideals on $U=\Spec A$ and the ideals in $A$, $A$ is Noetherian iff \itememph{a} holds for $U=\Spec A$. Thus \itememph{b} and \itememph{c} as well as \itememph{d} and \itememph{e} are equivalent. The implications from \itememph{a} to \itememph{b} and from \itememph{b} to \itememph{e} are trivial.  If \itememph{e} holds, then $X$ may be covered by affine open $U_i$ for which $\Oo_X(U_i)$ is Noetherian. the $\Oo_X(U_i\setminus V(\lambda)) \simeq \Oo_X(U_i)_\lambda$ are also Noetherian, hence the $U_i\setminus V(\lambda)$ satisfy \itememph{a}. By Fact~\reff{fact:affineSchemesTopologyBase} they form a topology base. Hence any quasi-compact open $U$ may be covered by finitely many such $U_i\setminus V(\lambda)$. Since the sequence $(\Jj)_{i\in\IN}$ stabilizes on each of these finitely many subsets, it stabilizes on $U$.
\end{proof}
...
Let $X$ be a Noetherian $T_1$-space and $X^* = X\cup H $, where \begin{align*}
    = \left\{\eta_Z\st Z\subset X \text{ irreducible, closed}, \#(Z)> 1\right\}\;,
\end{align*}
the two sets are assumed to be disjoint and where $\eta_Z\neq \eta_Y$ when $Z\neq Y$. For open $U\subseteq X$, let 
\begin{align*}
    U^* &= U \cup \left\{\eta_Z\in H\st Z\cap U \neq \emptyset, \#(Z) > 1\right\} \\
    &= U\cup \left\{\eta_Y \st Y\subseteq U\text{ closed, irreducible}, \#(Y) > 1\right\}\;.
\end{align*}
We have $(U\cap V)^* = U^* \cap V^*$, $\bigcup_{i\in I} U_i^* = (\bigcup_{i\in I} U_i)^*$ by the base properties of irreducibility. Thus, there is a topology base on $X^*$ by the open subsets $U^*$. As the posets of $X$ and $X^*$ are isomorphic, the presheaf categories on $X$ and $X^*$ are isomorphic and the validity of the sheaf axiom turns out to be preserved. 

\begin{fact}
    We have an equivalence of categories between the Noetherian $T_1$-spaces and the the Noetherian sober spaces such that the closed points are dense in any closed subsets. 
    \begin{align*}
        X =\left\{\text{closed points of } Y\right\} &\lmorphism Y\\
        X&\morphism Y= X^*
    \end{align*}
\end{fact}
If $k=\ov k$ and $X\morphism \Spec k$ is a morphism of finite type, then a point $x\in X$ is closed iff it is closed in some affine open subsets which is equivalent to $k(x) = k$. And every $f\in \Oo_X(U)$ defines a function from the closed points of $U$ to $k$ by mapping $x\in X$ to the image of $f$ under
\begin{align*}
    \Oo_X(U)\morphism \Oo_{X,x} \morphism \Oo_{X,x}/\mm_x \simeq k
\end{align*}

\begin{thm}
    We have an equivalence of categories between the category of prevarieties over $k$ and the integral preschemes $X/k$ of finite type by
    \begin{align*}
        Y &\morphism Y^*\\
        Y = \left\{\text{closed points of } X\right\} &\lmorphism X
    \end{align*}
    with the above sheaf of $k$-valued functions. Here $Y^*$ is equipped with the unique sheaf of rings on $Y^*$ corresponding to $\Oo_Y$ under the above isomorphism of categories.
\end{thm}
